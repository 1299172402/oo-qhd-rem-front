    ...
    function ($rootScope, $scope, $http, $modal, $timeout, $window, editorManager) {
      ...
      $scope.editorFactory.promise.then(() => {
        ...
        const addGroup = function (groupName, groupArray) {
          const group = { name: groupName, items: [], paletteItems: [], groups: [], visible: true };
        ...
        /*
             StencilSet items
             */
        const data = editorManager.getStencilData();
        ...
        {
          const {role} = data.rules.morphingRules[i];
        ...
        for (let stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
          ...
          const stencilItem = {'id': data.stencils[stencilIndex].id,
          ...
          for (var i = 0; i < data.stencils[stencilIndex].roles.length; i++) {
            ...
            } else if (stencilRole === 'sequence_start') {
              stencilItem.canConnect = true;
        ...
        editorManager.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, (event) => {
          ...
          if (shapes && shapes.length == 1) {
            ...
            const a = editorManager.getCanvas().node.getScreenCTM();
                ...
                if (zoom !== 100) {
	              ...
	              if (additionalIEZoom === 1) {
	                   absoluteXY.y = absoluteXY.y - jQuery("#canvasSection").offset().top + 5;
	                         absoluteXY.x -= jQuery("#canvasSection").offset().left;
        ...
        if (!$rootScope.stencilInitialized) {
          FLOWABLE.eventBus.addListener(FLOWABLE.eventBus.EVENT_TYPE_HIDE_SHAPE_BUTTONS, (event) => {
            jQuery('.Oryx_button').each((i, obj) => {
          ...
          FLOWABLE.eventBus.addListener(FLOWABLE.eventBus.EVENT_TYPE_SHOW_VALIDATION_POPUP, (event) => {
            // Method to open validation dialog
            const showValidationDialog = function() {
          ...
          FLOWABLE.eventBus.addListener(FLOWABLE.eventBus.EVENT_TYPE_NAVIGATE_TO_PROCESS, (event) => {
        ...
        $scope.quickAddItem = function(newItemId) {
          $scope.safeApply(() => {
            ...
            if (shapes && shapes.length == 1) {
              ...
              const option = {type: $scope.currentSelectedShape.getStencil().namespace() + newItemId, namespace: $scope.currentSelectedShape.getStencil().namespace()};
      ...
      $scope.dropCallback = function (event, ui) {
        ...
        if ($scope.dragCanContain) {
          ...
          if ($scope.quickMenu) {
            ...
            const commandClass = ORYX.Core.Command.extend({
              construct(option, dockedShape, canAttach, position, facade){
                this.option = option;
                this.docker = null;
      ...
      $scope.dragCallback = function (event, ui) {
        ...
        if ($scope.dragModeOver != false) {
          ...
          if (additionalIEZoom !== 1) {
            coord.x /= additionalIEZoom;
            coord.y /= additionalIEZoom;
          }
          ...
          var parentCandidate = aShapes.reverse().find((candidate) => (candidate instanceof ORYX.Core.Canvas
      ...
      $scope.dragCallbackQuickMenu = function (event, ui) {
        ...
        if ($scope.dragModeOver != false) {
          ...
          if (stencil.type() === "node")  {
            		...
            		const canContain = editorManager.getRules().canContain({containingShape:candidate, containedStencil:stencil});
...
FLOWABLE.CreateCommand = ORYX.Core.Command.extend({
  construct(option, currentReference, position, facade){
    this.option = option;
  ...
  execute(){
    ...
    if (this.currentReference && this.position) {
      ...
      const {containedStencil} = this;
      const {connectedShape} = this;
      ...
      if (this.shape instanceof ORYX.Core.Node){
        (this.shape.dockers||[]).each((docker)=> {
