...
var Class = (function() {
  ...
  function subclass() {}
  function create() {
    let parent = null; const properties = $A(arguments);
  ...
  function addMethods(source) {
    ...
    for (let i = 0, {length} = properties; i < length; i++) {
      const property = properties[i]; let value = source[property];
    ...
    case 'object':
      ...
      if (_class === ARRAY_CLASS) {
        ...
        for (var i = 0, {length} = keys; i < length; i++) {
          var key = keys[i]; var str = Str(key, value, stack);
...
Object.extend(Function.prototype, (function() {
  ...
  function update(array, args) {
    const arrayLength = array.length; let {length} = args;
  ...
  function bind(context) {
    ...
    const nop = function() {};
    const __method = this; const args = slice.call(arguments, 1);
  ...
  function curry() {
    ...
    const __method = this; const args = slice.call(arguments, 0);
    return function() {
      const a = merge(args, arguments);
...
Object.extend(String.prototype, (function() {
  ...
  function gsub(pattern, replacement) {
    let result = ''; let source = this; let match;
...
const Enumerable = (function() {
  ...
  function eachSlice(number, iterator, context) {
    let index = -number; const slices = []; const array = this.toArray();
  ...
  function zip() {
    let iterator = Prototype.K; const args = $A(arguments);
...
function $A(iterable) {
  ...
  let length = iterable.length || 0; const results = new Array(length);
...
(function() {
  ...
  function intersect(array) {
    return this.uniq().findAll((item) => array.indexOf(item) !== -1);
  }
  ...
  function indexOf(item, i) {
    ...
    const array = Object(this); const length = array.length >>> 0;
  ...
  function concat(_) {
    const array = []; const items = slice.call(arguments, 0); let item; let n = 0;
    items.unshift(this);
  ...
  function map(iterator) {
    ...
    const results = []; const context = arguments[1]; let n = 0;
  ...
  function filter(iterator) {
    ...
    const object = Object(this);
    const results = []; const context = arguments[1]; let value;
...
var Hash = Class.create(Enumerable, (function() {
  ...
  function _each(iterator, context) {
    ...
    for (const key in this._object) {
      const value = this._object[key]; const pair = [key, value];
      pair.key = key;
      pair.value = value;
  ...
  function toQueryString() {
    return this.inject([], (results, pair) => {
      const key = encodeURIComponent(pair.key); const values = pair.value;
...
Ajax.Request = Class.create(Ajax.Base, {
  ...
  setRequestHeaders() {
    ...
    if (typeof this.options.requestHeaders === 'object') {
      ...
      if (Object.isFunction(extras.push))
        for (let i = 0, {length} = extras; i < length; i += 2)
  ...
  respondToReadyState(readyState) {
    const state = Ajax.Request.Events[readyState]; const response = new Ajax.Response(this);
...
(function(GLOBAL) {
  ...
  const {tags} = INSERTION_TRANSLATIONS;
  ...
  function replace_IE(element, content) {
    ...
    content = Object.toHTML(content);
    const parent = element.parentNode; const tagName = parent.tagName.toUpperCase();
  ...
  function insert(element, insertions) {
    ...
    for (const position in insertions)
  ...
  function getContentFromAnonymousElement(tagName, html, force) {
    let t = INSERTION_TRANSLATIONS.tags[tagName]; let div = DIV;
  ...
  function purgeCollection_IE(elements) {
    let i = elements.length; let element; let uid;
    while (i--) {
      element = elements[i];
  ...
  function immediateDescendants(element) {
    const results = []; let child = $(element).firstChild;
  ...
  function siblings(element) {
    ...
    const next = nextSiblings(element);
  ...
  function adjacent(element) {
    element = $(element);
    const expressions = SLICE.call(arguments, 1).join(', ');
    const siblings = Element.siblings(element); const results = [];
  ...
  function writeAttribute(element, name, value) {
    element = $(element);
    let attributes = {}; const table = ATTRIBUTE_TRANSLATIONS.write;
  ...
  let classProp = 'className'; let forProp = 'for';
  ...
  function setStyle(element, styles) {
    element = $(element);
    const elementStyle = element.style; let match;
  ...
  function retrieve(element, key, defaultValue) {
    if (!(element = $(element))) return;
    const storage = getStorage(element); let value = storage.get(key);
  ...
  const Methods = {}; const {ByTag} = Element.Methods;
...
(function() {
  ...
  function getPixelValue(value, property, context) {
    ...
    const isPercentage = value.include('%'); const isViewport = (context === document.viewport);

    if (/\d/.test(value) && element && element.runtimeStyle && !(isPercentage && isViewport)) {
      const style = element.style.left; const rStyle = element.runtimeStyle.left;
    ...
    if (element && isPercentage) {
      ...
      const decimal = toDecimal(value); let whole = null;
  ...
  Element.Layout = Class.create(Hash, {
    ...
    _begin() {
      ...
      const position = getRawStyle(element, 'position'); let width = element.offsetWidth;
      ...
      const positionedWidth = element.offsetWidth; let newWidth;
      if (width && (positionedWidth === width)) {
        newWidth = getContentWidth(element, context);
      } else if (position === 'absolute' || position === 'fixed') {
        newWidth = getContentWidth(element, context);
      } else {
        const parent = element.parentNode; const pLayout = $(parent).getLayout();
  ...
  function viewportOffset(forElement) {
    let valueT = 0; let valueL = 0; const docBody = document.body;
  ...
  function makePositioned(element) {
    element = $(element);
    const position = Element.getStyle(element, 'position'); const styles = {};
  ...
  function undoPositioned(element) {
    ...
    const madePositioned = storage.get('prototype_made_positioned');
  ...
  function makeClipping(element) {
    ...
    const storage = Element.getStorage(element);
    const madeClipping = storage.get('prototype_made_clipping');
  ...
  function undoClipping(element) {
    element = $(element);
    const storage = Element.getStorage(element);
    const overflow = storage.get('prototype_made_clipping');
  ...
  function clonePosition(element, source, options) {
    ...
    source  = $(source);
    element = $(element);
    let p; let delta; let layout; const styles = {};
...
Prototype.Selector = (function() {
  ...
  function find(elements, expression, index) {
    index = index || 0;
    const {match} = Prototype.Selector; const {length} = elements; let matchIndex = 0; let i;
...
(function( window ) {
  ...
  function Sizzle( selector, context, results, seed ) {
    ...
    let i; let groups; let old; let nid; let newContext; let newSelector;
  ...
  support.sortDetached = assert(( div1 ) => div1.compareDocumentPosition( document.createElement("div") ) & 1);
...
var Form = {
  ...
  serializeElements(elements, options) {
    ...
    let key; let value; let submitted = false; const {submit} = options; let accumulator; let initial;
...
Form.Methods = {
  ...
  getElements(form) {
    ...
    let element; const results = []; const serializers = Form.Element.Serializers;
  ...
  request(form, options) {
    ...
    let params = options.parameters; let action = form.readAttribute('action') || '';
...
Form.Element.Serializers = (function() {
  ...
  function select(element, value) {
    ...
    let opt; let currentValue; const single = !Object.isArray(value);
  ...
  function selectMany(element) {
    var values; const {length} = element;
...
(function(GLOBAL) {
  ...
  function findElement(event, expression) {
    let element = _element(event); const selector = Prototype.Selector;
  ...
  function getOrCreateRegistryFor(element, uid) {
    ...
    if (Object.isUndefined(uid))
      uid = getUniqueElementID(element);
    if (!CACHE[uid]) CACHE[uid] = { element };
    return CACHE[uid];
  }
  ...
  function unregister(element, eventName, handler) {
    ...
    let i = entries.length; let entry;
    while (i--) {
  ...
  function stopObserving(element, eventName, handler) {
    ...
    const eventNameGiven = !Object.isUndefined(eventName);
  ...
  function stopObservingElement(element) {
    const uid = getUniqueElementID(element); const registry = GLOBAL.Event.cache[uid];
    if (!registry) return;
...
if (!document.getElementsByClassName) document.getElementsByClassName = function(instanceMethods){
  ...
  instanceMethods.getElementsByClassName = Prototype.BrowserFeatures.XPath ?
    function(element, className) {
      ...
      className = className.toString().strip();
      const elements = []; const classNames = (/\s/.test(className) ? $w(className) : null);
      ...
      for (var i = 0, child, cn; child = nodes[i]; i++) {
        if (child.className && (cn = ` ${  child.className  } `) && (cn.include(className) ||
                    (classNames && classNames.all((name) => !name.toString().blank() && cn.include(` ${  name  } `)))))
...
Element.ClassNames.prototype = {
  ...
  _each(iterator, context) {
    this.element.className.split(/\s+/).select((name) => name.length > 0)._each(iterator, context);
  },
