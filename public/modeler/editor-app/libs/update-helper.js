...
var UpdateHelper = Class.create({
  ...
  getStack() {
    try {
      ...
      const match = (e.stack || '').match(this.Regexp);
      if (match) {
        return match.reject((path) => (/(prototype|unittest|update_helper)\.js/).test(path)).join("\n");
      }  return ''; 
    }
  },
...
const prototypeUpdateHelper = new UpdateHelper([
  {
    methodName: 'display',
    namespace: Toggle,
...

  {
    ...
    type: 'error',
    condition() { return arguments.length > 1 && !Object.isNumber(arguments[1]) }
  },
...

  {
    ...
    condition() { return !arguments.length }
  },
...
(function() {
  const __properties = Object.keys(Hash.prototype).concat(['_object', '__properties']);
  ...
  function notify(property, value) {
    const message = messages[arguments.length == 1 ? 'getting' : 'setting'].evaluate({
      property,
  ...
  function checkProperties(hash) {
    ...
    const current = Object.keys(hash);
  ...
  $w('merge update').each((name) => {
    Hash.prototype[name] = Hash.prototype[name].wrap(function(proceed, object) {
      for (const prop in object) defineSetters(this, prop);
      return proceed(object);
    });
  });

  $H(Hash.prototype).each((method) => {
    const {key} = method;
