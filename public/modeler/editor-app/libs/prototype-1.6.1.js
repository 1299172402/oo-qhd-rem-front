...
Object.extend(String.prototype, (function() {
  ...
  function camelize() {
    const parts = this.split('-'); const len = parts.length;
...
(function() {
  ...
  function concat() {
    const array = slice.call(this, 0); let item;
    for (let i = 0, {length} = arguments; i < length; i++) {
      item = arguments[i];
...
Element.Methods = {
  ...
  insert(element, insertions) {
    ...
    let content; let insert; let tagName; let childNodes;
  ...
  inspect(element) {
    ...
    $H({'id': 'id', 'className': 'class'}).each((pair) => {
      const property = pair.first(); const attribute = pair.last();
  ...
  writeAttribute(element, name, value) {
    element = $(element);
    let attributes = { }; const t = Element._attributeTranslations.write;
...
if (Prototype.Browser.Opera) {
  ...
  Element.Methods.setOpacity = function(element, value) {
    ...
    let filter = element.getStyle('filter'); const {style} = element;
    if (value == 1 || value === '') {
  ...
  if (Prototype.BrowserFeatures.ElementExtensions) {
    (function() {
      function _descendants(element) {
        const nodes = element.getElementsByTagName('*'); const results = [];
...
if ('outerHTML' in document.documentElement) {
  Element.Methods.replace = function(element, content) {
    ...
    if (Element._insertionTranslations.tags[tagName]) {
      ...
      if (nextSibling)
        fragments.each((node) => { parent.insertBefore(node, nextSibling) });
      else
        fragments.each((node) => { parent.appendChild(node) });
    }
...
Element._getContentFromAnonymousElement = function(tagName, html) {
  let div = new Element('div'); const t = Element._insertionTranslations.tags[tagName];
  if (t) {
...
Element.addMethods = function(methods) {
  const F = Prototype.BrowserFeatures; const T = Element.Methods.ByTag;
...
(function(viewport) {
  const B = Prototype.Browser; const doc = document; let element; const property = {};
...
Element.addMethods({
  ...
  retrieve(element, key, defaultValue) {
    if (!(element = $(element))) return;
    const hash = Element.getStorage(element); let value = hash.get(key);
...
var Selector = Class.create({
  ...
  compileMatcher() {
    ...
    const c = Selector.criteria; let le; let p; let m; const len = ps.length; let name;
  ...
  compileXPathMatcher() {
    ...
    const x = Selector.xpath; let le; let m; const len = ps.length; let name;
  ...
  findElements(root) {
    root = root || document;
    let e = this.expression; let results;

    switch (this.mode) {
    case 'selectorsAPI':
      if (root !== document) {
        var oldId = root.id; let id = $(root).identify();
        id = id.replace(/([\.:])/g, "\\$1");
        e = `#${  id  } ${  e}`;
      }
  ...
  match(element) {
    ...
    let e = this.expression; const ps = Selector.patterns; const as = Selector.assertions;
    let le; let p; let m; const len = ps.length; var name;
    ...
    let match = true; var name; let matches;
...
Object.extend(Selector, {
  ...
  xpath: {
    ...
    pseudos: {
      ...
      'not': function(m) {
        ...
        const x = Selector.xpath; let le; let v; const len = p.length; let name;
      ...
      nth(fragment, m) {
        let mm; let formula = m[6]; let predicate;
  ...
  handlers: {
    ...
    tagName(nodes, root, tagName, combinator) {
      ...
      const results = []; const h = Selector.handlers;
      if (nodes) {
        if (combinator) {
    ...
    id(nodes, root, id, combinator) {
      const targetNode = $(id); const h = Selector.handlers;
    ...
    attr(nodes, root, attr, value, operator, combinator) {
      if (!nodes) nodes = root.getElementsByTagName("*");
      if (nodes && combinator) nodes = this[combinator](nodes);
      const handler = Selector.operators[operator]; const results = [];
      for (var i = 0, node; node = nodes[i]; i++) {
  ...
  pseudos: {
    ...
    nth(nodes, formula, root, reverse, ofType) {
      ...
      if (formula == 'even') formula = '2n+0';
      if (formula == 'odd')  formula = '2n+1';
      const h = Selector.handlers; const results = []; const indexed = []; let m;
      h.mark(nodes);
      for (var i = 0, node; node = nodes[i]; i++) {
    ...
    'not': function(nodes, selector, root) {
      const h = Selector.handlers; let selectorType; let m;
  ...
  matchElements(elements, expression) {
    const matches = $$(expression); const h = Selector.handlers;
    h.mark(matches);
...
var Form = {
  ...
  serializeElements(elements, options) {
    ...
    let key; let value; let submitted = false; const {submit} = options;
...
(function() {
  ...
  function stopObserving(element, eventName, handler) {
    ...
    if (eventName && !handler) {
      ...
      registry.each( (pair) => {
        const eventName = pair.key; const responders = pair.value;
