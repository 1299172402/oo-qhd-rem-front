...
var FLOWABLE_EDITOR_TOUR = {
  ...
  /*
     ...
     */
  gettingStarted($scope, $translate, $q, useLocalStorage) {
    let userName;
    ...
    $q.all([
      $translate('TOUR.WELCOME-TITLE', {userName}), $translate('TOUR.WELCOME-CONTENT'),
  ...
  /*
     ...
     */
  sequenceFlowBendpoint($scope, $translate, $q, useLocalStorage) {
  ...
  _buildStepTemplate (addPrevButton, addNextButton, addEndTourButton, optionalForcedWidth, image) {
  ...
  _buildOnNextFunction(selector) {
    return function () {
      jQuery(selector).each((i, obj) => {
        obj.style.display = 'block';
      })
    };
  },

  _buildOnEndFunction(selectors) {
    return function () {
      for (let elementsToResetIndex = 0; elementsToResetIndex < selectors.length; elementsToResetIndex++) {
        jQuery(selectors[elementsToResetIndex]).each((i, obj) => {
          obj.style.display = 'block';
        });
      }
    }
