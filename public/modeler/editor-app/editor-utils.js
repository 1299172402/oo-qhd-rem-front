...
EDITOR.UTIL = {
  ...
  /**
     ...
     */
  collectPropertiesFromPrecedingElements (startElement, propertyType) {
  ...
  /**
     ...
     * to the return result list.
     */
  collectElementsFromPrecedingElements (startElement, stencilId) {
    const visitedElements = [];
  ...
  collectShapes (shapes, stencilId) {
  ...
  collectSortedElementsFromPrecedingElements (startElement) {
    const visitedElements = [];
    const collectedElements = [];
  ...
  _visitShapeAndCollectShape(shape, stencilId, collectedShapes) {
  ...
  _visitElementAndCollectProperty (element, propertyType, visitedElementsArray, collectedProperties) {
    ...
    const childShapesIterator = element.getChildShapes();
    if (childShapesIterator) {
      for (var i = 0; i < childShapesIterator.length; i++) {
        const childShape = childShapesIterator[i];
  ...
  _visitElementAndCollectElement (element, stencilId, visitedElementsArray, collectedElements) {
  ...
  _visitElementAndCollectAllElement (element, visitedElementsArray, collectedElements) {
    ...
    const parent = element.getParentShape();
    if (parent) {
      try {
        const incomingParentShapesIterator = parent.getIncomingShapes();
        if (incomingParentShapesIterator) {
          for (var i = 0; i < incomingParentShapesIterator.length; i++) {
            const incomingParentShape = incomingParentShapesIterator[i];
  ...
  /**
     ...
     */
  getPropertyFromParent (element, propertyType) {
    if (element.parent) {
  ...
  _getPropertyFromParent (parentElement, propertyType) {
  ...
  /**
     ...
     */
  extractSortedStepsOfType (allElements, propertyType) {

    const filteredSteps = [];
    const sortedSteps = EDITOR.UTIL.extractSortedSteps(allElements);
  ...
  extractSortedSteps(allElements) {
  ...
  getOutgoingElements(element, elementsMap, sortedSteps, onlySteps) {
    ...
    if (element && elementsMap) {
      ...
      } else if (onlySteps && EDITOR.UTIL._elementIsStep(element)) {
        element.id = element.resourceId;
        sortedSteps.push(element);
      } else if (!onlySteps) {
        element.id = element.resourceId;
