...
angular.module('flowableModeler').controller('FlowableEventListenersPopupCtrl',
  ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    ...
    $scope.listenerDetailsChanged = function () {
      const listener = $scope.selectedListener;
      if (listener.events) {
        let eventText = '';
      ...
      else if ($scope.selectedListener.className !== '') {
    ...
    $scope.removeListener = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.eventListeners.indexOf(selectedItems[0]);
    ...
    $scope.moveListenerUp = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.eventListeners[index];
          $scope.eventListeners.splice(index, 1);
          $timeout(() => {
