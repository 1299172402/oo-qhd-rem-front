...
angular.module('flowableModeler').controller('FlowableDataPropertiesPopupCtrl',
  ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
      ...
      for (let i = 0; i < $scope.dataProperties.length; i++) {
			    const dataProperty = $scope.dataProperties[i];
			    if (dataProperty.enumValues && dataProperty.enumValues.length > 0) {
				    for (let j = 0; j < dataProperty.enumValues.length; j++) {
					    const enumValue = dataProperty.enumValues[j];
    ...
    const idPromise = $translate('PROPERTY.DATAPROPERTIES.ID');
    const namePromise = $translate('PROPERTY.DATAPROPERTIES.NAME');
    const typePromise = $translate('PROPERTY.DATAPROPERTIES.TYPE');
    const valuePromise = $translate('PROPERTY.DATAPROPERTIES.VALUE');

    $q.all([idPromise, namePromise, typePromise, valuePromise]).then((results) => {
      ...
      $scope.gridOptions.onRegisterApi = function (gridApi) {
        ...
        gridApi.selection.on.rowSelectionChanged($scope, (row) => {
          ...
          if ($scope.selectedProperty && $scope.selectedProperty.enumValues) {
            ...
            for (let i = 0; i < $scope.selectedProperty.enumValues.length; i++) {
    ...
    $scope.removeProperty = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.dataProperties.indexOf(selectedItems[0]);
    ...
    $scope.movePropertyUp = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.dataProperties[index];
          $scope.dataProperties.splice(index, 1);
          $timeout(()=> {
    ...
    $scope.removeEnumValue = function() {
      const selectedItems = $scope.enumGridApi.selection.getSelectedRows();
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.enumValues.indexOf(selectedItems[0]);
    ...
    $scope.moveEnumValueUp = function() {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.enumValues.indexOf(selectedItems[0]);
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.enumValues[index];
          $scope.enumValues.splice(index, 1);
          $scope.selectedProperty.enumValues.splice(index, 1);
          $timeout(() => {
