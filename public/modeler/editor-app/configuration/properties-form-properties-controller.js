...
angular.module('flowableModeler').controller('FlowableFormPropertiesPopupCtrl',
  ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
      ...
      for (let i = 0; i < $scope.formProperties.length; i++) {
			    const formProperty = $scope.formProperties[i];
			    if (formProperty.enumValues && formProperty.enumValues.length > 0) {
				    for (let j = 0; j < formProperty.enumValues.length; j++) {
					    const enumValue = formProperty.enumValues[j];
    ...
    const idPromise = $translate('PROPERTY.FORMPROPERTIES.ID');
    const namePromise = $translate('PROPERTY.FORMPROPERTIES.NAME');
    const typePromise = $translate('PROPERTY.FORMPROPERTIES.TYPE');

    $q.all([idPromise, namePromise, typePromise]).then((results) => {
    ...
    $scope.removeProperty = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.formProperties.indexOf(selectedItems[0]);
    ...
    $scope.movePropertyUp = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.formProperties[index];
          $scope.formProperties.splice(index, 1);
          $timeout(()=> {
