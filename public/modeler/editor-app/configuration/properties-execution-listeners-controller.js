...
angular.module('flowableModeler').controller('FlowableExecutionListenersPopupCtrl',
  ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
      ...
      for (let i = 0; i < $scope.executionListeners.length; i++) {
        const executionListener = $scope.executionListeners[i];
    ...
    const eventPromise = $translate('PROPERTY.EXECUTIONLISTENERS.EVENT');
    const implementationPromise = $translate('PROPERTY.EXECUTIONLISTENERS.FIELDS.IMPLEMENTATION');
    const namePromise = $translate('PROPERTY.EXECUTIONLISTENERS.FIELDS.NAME');

    $q.all([eventPromise, implementationPromise, namePromise]).then((results) => {
    ...
    $scope.removeListener = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.executionListeners.indexOf(selectedItems[0]);
    ...
    $scope.moveListenerUp = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.executionListeners[index];
          $scope.executionListeners.splice(index, 1);
          $timeout(() => {
