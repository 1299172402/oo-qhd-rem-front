...
angular.module('flowableModeler').controller('FlowableAssignmentPopupCtrl',
  [ '$rootScope', '$scope', '$translate', '$http', 'UserService', 'GroupService', function($rootScope, $scope, $translate, $http, UserService, GroupService) {
    ...
    $scope.$watch('popup.groupFilter', () => {
      $scope.updateGroupFilter();
    });

    $scope.$watch('popup.filter', () => {
      $scope.updateFilter();
    });
    ...
    $scope.updateFilter = function() {
      if ($scope.popup.oldFilter == undefined || $scope.popup.oldFilter != $scope.popup.filter) {
        ...
        if ($scope.popup.filter !== null && $scope.popup.filter !== undefined) {
          UserService.getFilteredUsers($scope.popup.filter).then((result) => {
            const filteredUsers = [];
            for (let i=0; i<result.data.length; i++) {
              const filteredUser = result.data[i];

              let foundCandidateUser = false;
              if ($scope.popup.assignmentObject.idm.candidateUsers !== null && $scope.popup.assignmentObject.idm.candidateUsers !== undefined) {
                for (let j=0; j<$scope.popup.assignmentObject.idm.candidateUsers.length; j++) {
                  const candidateUser = $scope.popup.assignmentObject.idm.candidateUsers[j];
    ...
    $scope.updateGroupFilter = function() {
      if ($scope.popup.oldGroupFilter == undefined || $scope.popup.oldGroupFilter != $scope.popup.groupFilter) {
        ...
        GroupService.getFilteredGroups($scope.popup.groupFilter).then((result) => {
    ...
    $scope.confirmUser = function(user) {
      if (!user) {
        ...
        const users = $scope.popup.userResults;
      ...
      if (user) {
        if ($scope.assignmentOption.id == "user") {
          $scope.popup.assignmentObject.idm.assignee = user;
        } else if ($scope.assignmentOption.id == "users") {

          // Only add if not yet part of candidate users
          let found = false;
          if ($scope.popup.assignmentObject.idm.candidateUsers) {
            for (let i = 0; i < $scope.popup.assignmentObject.idm.candidateUsers.length; i++) {
    ...
    $scope.confirmGroup = function(group) {
      if (!group) {
        // Selection is done with keyboard, use selection index
        const groups = $scope.popup.groupResults;
      ...
      if (group) {
        ...
        if ($scope.popup.assignmentObject.idm.candidateGroups) {
          for (let i = 0; i < $scope.popup.assignmentObject.idm.candidateGroups.length; i++) {
    ...
    $scope.removeCandidateUser = function(user) {
      const users = $scope.popup.assignmentObject.idm.candidateUsers;
      let indexToRemove = -1;
    ...
    $scope.removeCandidateGroup = function(group) {
      const groups = $scope.popup.assignmentObject.idm.candidateGroups;
      let indexToRemove = -1;
    ...
    $scope.save = function () {
      ...
      if ($scope.popup.assignmentObject.type === 'static') { // IDM
