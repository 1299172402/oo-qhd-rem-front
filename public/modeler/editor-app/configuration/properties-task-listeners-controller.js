...
angular.module('flowableModeler').controller('FlowableTaskListenersPopupCtrl',
  ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
      ...
      for (let i = 0; i < $scope.taskListeners.length; i++) {
        const taskListener = $scope.taskListeners[i];
    ...
    const eventPromise = $translate('PROPERTY.TASKLISTENERS.EVENT');
    const implementationPromise = $translate('PROPERTY.TASKLISTENERS.FIELDS.IMPLEMENTATION');
    const namePromise = $translate('PROPERTY.TASKLISTENERS.FIELDS.NAME');
    ...
    $scope.removeListener = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        const index = $scope.taskListeners.indexOf(selectedItems[0]);
    ...
    $scope.moveListenerUp = function () {
      ...
      if (selectedItems && selectedItems.length > 0) {
        ...
        if (index != 0) { // If it's the first, no moving up of course
          const temp = $scope.taskListeners[index];
          $scope.taskListeners.splice(index, 1);
          $timeout(() => {
