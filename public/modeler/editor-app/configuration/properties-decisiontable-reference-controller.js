...
angular.module('flowableModeler').controller('FlowableDecisionTableReferencePopupCtrl', ['$rootScope', '$scope', '$http', '$location', 'editorManager',
  function($rootScope, $scope, $http, $location, editorManager) {
    ...
    $scope.$parent.$on('modal.hide.before', () => {
      $scope.close();
      $scope.$parent.$apply();
    });
    ...
    $scope.open = function() {
      if ($scope.selectedDecisionTable) {
        ...
        let json = editorManager.getModel();
    ...
    $scope.createDecisionTable = function() {
      ...
      const stencilSetId = $scope.model.decisionTable.stencilSet;
      ...
      $http({
        ...
        success((data, status, headers, config) => {
          ...
          const newDecisionTableId = data.id;
          $scope.property.value = {
    ...
    $scope.resetCurrent = function () {
      for (let i = 0, found = false; i < $scope.decisionTables.length && found === false; i++) {
        const table = $scope.decisionTables[i];
