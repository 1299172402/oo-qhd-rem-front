...
angular.module('flowableModeler').controller('ProcessNavigatorController',['editorManager', '$scope',function(editorManager, $scope){
  ...
  $scope.showSubProcess = function(child){
    const flowableShapes = editorManager.getChildShapeByResourceId(child.resourceId);
  ...
  FLOWABLE.eventBus.addListener(FLOWABLE.eventBus.EVENT_TYPE_EDITOR_READY, (event)=> {
