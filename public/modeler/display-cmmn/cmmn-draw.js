...
{
  const rect = paper.rect(planModel.x, planModel.y, planModel.width, planModel.height);
  ...
  if (planModel.name)
  {
    const planModelName = paper.text(planModel.x + 14, planModel.y + (planModel.height / 2), planModel.name).attr({
	        "text-anchor" : "middle",
	        "font-family" : "Arial",
	        "font-size" : "12",
	        "fill" : "#000000"
...
function _drawSubProcess(element)
{
  ...
  const strokeColor = _cmmnGetColor(element, MAIN_STROKE_COLOR);
...
function _drawTask(element)
{
  ...
  // Stroke
  const strokeColor = _cmmnGetColor(element, ACTIVITY_STROKE_COLOR);
...
function _drawMilestone(element)
{
  ...
  const rect = paper.rect(element.x, element.y, width, height, 24);
  rectAttrs['stroke-width'] = strokeWidth;
...
function _drawStage(element)
{
  ...
  const rect = paper.rect(element.x, element.y, width, height, 16);
  rectAttrs['stroke-width'] = strokeWidth;
...
function _drawPlanModel(element)
{
  ...
  const path1 = paper.path("M20 55 L37 34 L275 34 L291 55");
  path1.attr({
    "opacity": 1,
    "stroke": strokeColor,
    "fill": "#ffffff"
  ...
  const planModelHeader = paper.set();
...
function _drawEntryCriterion(element)
{
  ...
  // Fill
  const gatewayFillColor = WHITE_FILL_COLOR;

  // Opacity
  const gatewayOpacity = 1.0;
