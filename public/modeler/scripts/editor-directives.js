  ...
  function ($rootScope, $timeout, $modal, $http, $templateCache, $translate, RecursionHelper, FormBuilderService) {
    return {
      ...
      link ($scope, $element, attributes) {
        ...
        const templateUrl = 'views/popover/formfield-edit-popover.html';
        ...
        $scope.openFieldPopover = function () {
          ...
          const originalFormElementType = $scope.formElement.type;
          let originalDisplayFieldType;
          ...
          var deregisterHideListener = $scope.$on('modal.hide', () => {
        ...
        $scope.confirmNewOption = function ($event) {
          if ($scope.newOption.name) {
            const {options} = $scope.formElement;
        ...
        $scope.removeOption = function (index) {
          ...
          if ($scope.formElement.options == 1) {
            ...
            let isPresent = false;
            for (let i = 0; i < $scope.formElement.options.length; i++) {
        ...
        // Readonly field
        $scope.$watch('formElement.params.field', (newValue, oldValue) => {
...
angular.module('flowableModeler').directive('storeCursorPosition', ['$rootScope', '$timeout', '$popover', '$http', '$templateCache', function ($rootScope, $timeout, $popover, $http, $templateCache) {
  return {
    ...
    link ($scope, $element, attributes) {
      $element.on('click change keypress', () => {
...
angular.module('flowableModeler').
  directive('editorInputCheck', () => ({
    require: 'ngModel',
    link (scope, element, attrs, modelCtrl) {

      modelCtrl.$parsers.push((inputValue) => {

        const transformedInput = inputValue.replace(/[&\/\\#,+~%.'":*?<>{}()$@;]/g, '');
  ...
  directive('hotAutoDestroy',["hotRegisterer",function(hotRegisterer) {
    return {
      ...
      link (scope, element, attr){
        element.on("$destroy", () => {
          try{
            const hotInstance = hotRegisterer.getInstance(attr.hotId);
            hotInstance.destroy();
          }
          catch(er){
            console.log(er);
