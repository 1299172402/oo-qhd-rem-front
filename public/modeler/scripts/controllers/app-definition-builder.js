...
angular.module('flowableModeler')
  .controller('ModelsIncludedPopupCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout', '$location', function ($rootScope, $scope, $translate, $http, $timeout, $location) {
    ...
    $scope.selectModel = function(model) {
      const index = $scope.popup.selectedModels.indexOf(model.id);
      if (index >= 0) {
      ...
      const modelArray = [];
      for (let i = 0; i < $scope.popup.models.data.length; i++) {
        if ($scope.popup.selectedModels.indexOf($scope.popup.models.data[i].id) >= 0) {
          const selectedModel = $scope.popup.models.data[i];
          const summaryModel = {
            id: selectedModel.id,
    ...
    $scope.selectCmmnModel = function(model) {
      const index = $scope.popup.selectedCmmnModels.indexOf(model.id);
      if (index >= 0) {
      ...
      const modelArray = [];
      for (let i = 0; i < $scope.popup.cmmnModels.data.length; i++) {
        if ($scope.popup.selectedCmmnModels.indexOf($scope.popup.cmmnModels.data[i].id) >= 0) {
          const selectedModel = $scope.popup.cmmnModels.data[i];
          const summaryModel = {
            id: selectedModel.id,
            name: selectedModel.name,
            version: selectedModel.version,
