...
angular.module('flowableModeler')
  .controller('DecisionTableEditorController', ['$rootScope', '$scope', '$q', '$translate', '$http', '$timeout', '$location', '$modal', '$route', '$routeParams', 'DecisionTableService',
    ...
    function ($rootScope, $scope, $q, $translate, $http, $timeout, $location, $modal, $route, $routeParams, DecisionTableService,
      ...
      let hotDecisionTableEditorInstance;
      const hitPolicies = ['FIRST', 'ANY', 'UNIQUE', 'PRIORITY', 'RULE ORDER', 'OUTPUT ORDER', 'COLLECT'];
      const stringOperators = ['==', '!=', 'IS IN', 'IS NOT IN'];
      const numberOperators = ['==', '!=', '<', '>', '>=', '<=', 'IS IN', 'IS NOT IN'];
      const booleanOperators = ['==', '!='];
      const dateOperators = ['==', '!=', '<', '>', '>=', '<=', 'IS IN', 'IS NOT IN'];
      const collectionOperators = ['ANY OF', 'NONE OF', 'ALL OF', 'NOT ALL OF', '==', '!='];
      const allOperators = ['==', '!=', '<', '>', '>=', '<=', 'ANY OF', 'NONE OF', 'ALL OF', 'NOT ALL OF', 'IS IN', 'IS NOT IN'];
      ...
      Object.keys(collectOperators).forEach((key) => {
      ...
      $scope.doAfterModifyColWidth = function (width, col) {
        if ($scope.model.columnDefs[col] && $scope.model.columnDefs[col].width) {
          const settingsWidth = $scope.model.columnDefs[col].width;
      ...
      $scope.doAfterOnCellMouseDown = function (event, coords, TD) {
        ...
        } else if (coords && coords.row !== undefined) {
          $timeout(() => {
      ...
      $scope.doAfterRender = function (isForced) {
        ...
        if (hitPolicyHeaderElement) {
          const element = document.querySelector("thead > tr > th:first-of-type");
          if (element) {
      ...
      const textValidator = function(value, callback) {
        callback(true);
      };
      ...
      var isCorrespondingCollectionOperator = function (row, prop) {
        const operatorCol = getCorrespondingOperatorCell(row, prop);
        const operatorCellMeta = hotDecisionTableEditorInstance.getCellMeta(row, operatorCol);

        let isCollectionOperator = false;
        if (isOperatorCell(operatorCellMeta)) {
          const operatorValue = hotDecisionTableEditorInstance.getDataAtCell(row, operatorCol);
      ...
      var disableCorrespondingOperatorCell = function (row, prop) {
        ...
        const currentEditor = hotDecisionTableEditorInstance.getCellEditor(row, operatorCol);
      ...
      var getCorrespondingOperatorCell = function (row, prop) {
        const currentCol = hotDecisionTableEditorInstance.propToCol(prop);
        if (currentCol < 1) {
          return;
        }
        const operatorCol = currentCol - 1;
        return operatorCol;
      };
      ...
      const createNewInputExpression = function (inputExpression) {
      ...
      const createNewOutputExpression = function (outputExpression) {
        let newOutputExpression;
        if (outputExpression) {
          newOutputExpression = {
            id: _generateColumnId(),
      ...
      $scope.addNewInputExpression = function (inputExpression, insertPos) {
        const newInputExpression = createNewInputExpression(inputExpression);
        ...
        $scope.model.rulesData.forEach((rowData) => {
      ...
      $scope.addNewOutputExpression = function (outputExpression, insertPos) {
        const newOutputExpression = createNewOutputExpression(outputExpression);
      ...
      $scope.removeInputExpression = function (expressionPos) {
        const removedElements = $scope.currentDecisionTable.inputExpressions.splice(expressionPos, 1);
      ...
      $scope.removeOutputExpression = function (expressionPos) {
        const removedElements = $scope.currentDecisionTable.outputExpressions.splice(expressionPos, 1);
        removePropertyFromGrid(removedElements[0].id, 'output');
        $scope.evaluateDecisionHeaders($scope.currentDecisionTable);
      };

      $scope.openInputExpressionEditor = function (expressionPos, newExpression) {
        const editTemplate = 'views/popup/decision-table-edit-input-expression.html';
        ...
        } else if (expressionPos >= $scope.model.startOutputExpression) {
      ...
      $scope.openOutputExpressionEditor = function (expressionPos, newExpression) {
        const editTemplate = 'views/popup/decision-table-edit-output-expression.html';
      ...
      $scope.openHitPolicyEditor = function () {
        const editTemplate = 'views/popup/decision-table-edit-hit-policy.html';
      ...
      const _loadDecisionTableDefinition = function (modelId) {
        DecisionTableService.fetchDecisionTableDetails(modelId).then((decisionTable) => {
          ...
          let hitPolicyHeaderString = `<div class="hit-policy-header"><a onclick="triggerHitPolicyEditor()">${  $rootScope.currentDecisionTable.hitIndicator.substring(0, 1)}`;
      ...
      const composeInputOperatorColumnDefinition = function (inputExpression) {
        const expressionPosition = $scope.currentDecisionTable.inputExpressions.indexOf(inputExpression);
      ...
      const composeInputExpressionColumnDefinition = function (inputExpression) {
        const expressionPosition = $scope.currentDecisionTable.inputExpressions.indexOf(inputExpression);
        ...
        if (inputExpression.entries && inputExpression.entries.length > 0) {
          const entriesOptionValues = inputExpression.entries.slice(0, inputExpression.entries.length);
      ...
      const composeOutputColumnDefinition = function (outputExpression) {
        const expressionPosition = $scope.currentDecisionTable.outputExpressions.indexOf(outputExpression);
        ...
        if (outputExpression.entries && outputExpression.entries.length > 0) {
          const entriesOptionValues = outputExpression.entries.slice(0, outputExpression.entries.length);
          columnDefinition.type = 'dropdown';
          columnDefinition.source = entriesOptionValues;
          columnDefinition.strict = true;
      ...
      $scope.evaluateDecisionHeaders = function (decisionTable) {
        ...
        const element = document.querySelector("thead > tr > th:first-of-type > div > a");
        if (element) {
        ...
        let inputExpressionCounter = 0;
        if (decisionTable.inputExpressions && decisionTable.inputExpressions.length > 0) {
          decisionTable.inputExpressions.forEach((inputExpression) => {
            const inputOperatorColumnDefinition = composeInputOperatorColumnDefinition(inputExpression);
            ...
            const inputExpressionColumnDefinition = composeInputExpressionColumnDefinition(inputExpression);
          ...
          const inputExpression = createNewInputExpression();
        ...
        if (decisionTable.outputExpressions && decisionTable.outputExpressions.length > 0) {
          decisionTable.outputExpressions.forEach((outputExpression) => {
          ...
          const outputExpression = createNewOutputExpression();
      ...
      var createDefaultRow = function () {
        const defaultRow = {};
        $scope.model.columnDefs.forEach((columnDefinition) => {
      ...
      const evaluateDecisionGrid = function (decisionTable) {
        const tmpRuleGrid = [];

        // rows
        if (decisionTable.rules && decisionTable.rules.length > 0) {
          decisionTable.rules.forEach((rule) => {

            // rule data
            const tmpRowValues = {};
            for (let i = 0; i < Object.keys(rule).length; i++) {
              const id = Object.keys(rule)[i];

              $scope.model.columnDefs.forEach((columnDef) => {
...
angular.module('flowableModeler')
  .controller('DecisionTableInputConditionEditorCtlr', ['$rootScope', '$scope', 'hotRegisterer', '$timeout', function ($rootScope, $scope, hotRegisterer, $timeout) {
    ...
    const getEntriesValues = function (entriesArrayOfArrays) {
      const newEntriesArray = [];
      // remove last value
      entriesArrayOfArrays.pop();

      entriesArrayOfArrays.forEach((entriesArray) => {
    ...
    const createEntriesValues = function (entriesArray) {
      const localCopy = entriesArray.slice(0);
      const entriesArrayOfArrays = [];
    ...
    const deleteRowRenderer = function (instance, td, row) {
      td.innerHTML = '';
