...
angular.module('flowableModeler')
  .controller('DecisionTableDetailsCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams','$modal', '$timeout', '$popover', 'DecisionTableService', 'hotRegisterer',
    function ($rootScope, $scope, $translate, $http, $location, $routeParams, $modal, $timeout, $popover, DecisionTableService, hotRegisterer) {
      ...
      let hotReadOnlyDecisionTableEditorInstance;
      const hitPolicies = ['FIRST', 'ANY', 'UNIQUE', 'PRIORITY', 'RULE ORDER', 'OUTPUT ORDER', 'COLLECT'];
      const operators = ['==', '!=', '<', '>', '>=', '<=', 'ANY OF', 'NONE OF', 'ALL OF', 'NOT ALL OF', 'IS IN', 'IS NOT IN'];
      let columnIdCounter = 0;
      const dateFormat = 'YYYY-MM-DD';

      const variableUndefined = $translate.instant('DECISION-TABLE-EDITOR.EMPTY-MESSAGES.NO-VARIABLE-SELECTED');
      ...
      $scope.toggleFavorite = function() {
        ...
        $http({method: 'PUT', url: FLOWABLE.APP_URL.getModelUrl($scope.model.latestModelId), data}).
          success((data, status, headers, config) => {
      ...
      $scope.doAfterRender = function () {
        ...
        if (element) {
          ...
          const newElement = angular.element(`<div class="hit-policy-header"><a onclick="triggerHitPolicyEditor()">${  $scope.currentDecisionTable.hitIndicator.substring(0, 1)  }</a></div>`);
      ...
      var isCorrespondingCollectionOperator = function (row, prop) {
        ...
        const operatorCellMeta = hotReadOnlyDecisionTableEditorInstance.getCellMeta(row, operatorCol);

        let isCollectionOperator = false;
        if (isOperatorCell(operatorCellMeta)) {
          const operatorValue = hotReadOnlyDecisionTableEditorInstance.getDataAtCell(row, operatorCol);
      ...
      var disableCorrespondingOperatorCell = function (row, prop) {
        ...
        const currentEditor = hotReadOnlyDecisionTableEditorInstance.getCellEditor(row, operatorCol);
      ...
      var getCorrespondingOperatorCell = function (row, prop) {
        const currentCol = hotReadOnlyDecisionTableEditorInstance.propToCol(prop);
        if (currentCol < 1) {
          return;
        }
      ...
      const _loadDecisionTableDefinition = function (modelId, historyId) {
        DecisionTableService.fetchDecisionTableDetails(modelId, historyId).then((decisionTable) => {
      ...
      const setGridValues = function (key, type) {
