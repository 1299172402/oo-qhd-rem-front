...
Polyline.prototype = {
  ...
  rebuildPath(){
    ...
    for(let i = 0; i < this.getAnchorsCount(); i++){
      ...
      let targetX = anchor.x; let targetY = anchor.y;
      if (i>0 && i < this.getAnchorsCount()-1) {
        // get new x,y
        const cx = anchor.x; const cy = anchor.y;
        ...
        const dx0 = (cx - targetX) / 3;
        const dy0 = (cy - targetY) / 3;
        var ax = cx - dx0;
        var ay = cy - dy0;
					
        const dx1 = (cx - nextSrcX) / 3;
        const dy1 = (cy - nextSrcY) / 3;
        var bx = cx - dx1;
        var by = cy - dy1;
