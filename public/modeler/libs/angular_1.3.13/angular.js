 ...
 */
  ...
  function minErr(module, ErrorConstructor) {
    ...
    return function() {
      ...
      const prefix = `[${  module ? `${module  }:` : ''  }${code  }] `;
      const template = arguments[1];
      const templateArgs = arguments;

      let message; let i;

      message = prefix + template.replace(/\{\d+\}/g, (match) => {
        const index = +match.slice(1, -1); let arg;
 ...
 */
  ...
  const escapeForRegexp = function(s) {
    ...
    if (!destination) {
      ...
      if (source) {
        if (isArray(source)) {
          ...
          const emptyObject = Object.create(Object.getPrototypeOf(source));
 ...
 */
  function equals(o1, o2) {
    ...
    const t1 = typeof o1; const t2 = typeof o2; let length; let key; let keySet;
    if (t1 == t2) {
      if (t1 == 'object') {
        if (isArray(o1)) {
 ...
 */
  function parseKeyValue(/** string */keyValue) {
    const obj = {}; let key_value; let key;
 ...
 */
  ...
  function getNgAttribute(element, ngAttr) {
    let attr; let i; const ii = ngAttrPrefixes.length;
 ...
 */
  ...
  let bindJQueryFired = false;
  let skipDestroyOnNextJQueryCleanData;
  function bindJQuery() {
 ...
 */
  ...
  let jqId = 1;
  const addEventListenerFn = function(element, type, fn) {
    element.addEventListener(type, fn, false);
  };
  const removeEventListenerFn = function(element, type, fn) {
 ...
 */
  ...
  forEach({
        ...
        if (!eventFns) {
          ...
          // http://www.quirksmode.org/js/events_mouse.html#link8
            ...
            jqLiteOn(element, MOUSE_EVENT_MAP[type], function(event) {
              const target = this; const related = event.relatedTarget;
    ...
    replaceWith(element, replaceNode) {
      let index; const parent = element.parentNode;
    ...
    after(element, newElement) {
      let index = element; const parent = element.parentNode;
    ...
    triggerHandler(element, event, extraParameters) {
      ...
      let dummyEvent; let eventFnsCopy; let handlerArgs;
 ...
 */
  ...
  function createInjector(modulesToLoad, strictDi) {
    ...
    const path = [];
    const loadedModules = new HashMap([], true);
    const providerCache = {
      $provide: {
    ...
    const providerInjector = (providerCache.$injector =
          createInternalInjector(providerCache, (serviceName, caller) => {
    ...
    var instanceInjector = (instanceCache.$injector =
          createInternalInjector(instanceCache, (serviceName, caller) => {
            const provider = providerInjector.get(serviceName + providerSuffix, caller);
    ...
    function createInternalInjector(cache, factory) {
      ...
      // Object creation: http://jsperf.com/create-constructor/2
        const instance = Object.create((isArray(Type) ? Type[Type.length - 1] : Type).prototype || null);
        const returnedValue = invoke(Type, instance, locals, serviceName);
 ...
 */
   ...
   */
    this.$get = ['$window', '$location', '$rootScope', function($window, $location, $rootScope) {
      ...
      function scroll() {
        const hash = $location.hash(); let elm;
   ...
   */
    ...
    this.$get = ['$$q', '$$asyncCallback', '$rootScope', function($$q, $$asyncCallback, $rootScope) {
      ...
      function runAnimationPostDigest(fn) {
        let cancelFn; const defer = $$q.defer();
      ...
      function resolveElementClasses(element, classes) {
        ...
        const hasClasses = createMap();
        forEach((element.attr('class') || '').split(/\s+/), (className) => {
          hasClasses[className] = true;
        });

        forEach(classes, (status, className) => {
          const hasClass = hasClasses[className];
      ...
      function applyStyles(element, options) {
        if (angular.isObject(options)) {
          const styles = extend(options.from || {}, options.to || {});
          element.css(styles);
        }
      }
     ...
     */
       ...
       */
        ...
        $$addClassImmediately(element, className, options) {
          element = jqLite(element);
       ...
       */
        ...
        $$removeClassImmediately(element, className, options) {
          element = jqLite(element);
          className = !isString(className)
            ? (isArray(className) ? className.join(' ') : '')
            : className;
       ...
       */
        setClass(element, add, remove, options) {
          const self = this;
          const STORAGE_KEY = '$$animateClasses';
          let createdCache = false;
          element = jqLite(element);

          let cache = element.data(STORAGE_KEY);
          if (!cache) {
            cache = {
              classes: {},
              options
          ...
          if (createdCache) {
            cache.promise = runAnimationPostDigest((done) => {
              const cache = element.data(STORAGE_KEY);
              ...
              if (cache) {
                const classes = resolveElementClasses(element, cache.classes);
                if (classes) {
        ...
        $$setClassImmediately(element, add, remove, options) {
 ...
 */
   ...
   */
    self.cookies = function(name, value) {
      let cookieLength; let cookieArray; let cookie; let i; let index;
 ...
 */
  ...
  function $CompileProvider($provide, $$sanitizeUriProvider) {
    ...
    const ALL_OR_NOTHING_ATTRS = makeMap('ngSrc,ngSrcset,src,srcset');
   ...
   */
    ...
    this.$get = [
       ...
       */
          $set(key, value, writeAttr, attrName) {
            ...
            const aliasedKey = getAliasedAttrName(node, key);
            let observer = key;
            let nodeName;
            ...
            $$observers && forEach($$observers[observer], (fn) => {
              try {
                fn(value);
              } catch (e) {
                $exceptionHandler(e);
       ...
       */
        ...
        const NG_ATTR_BINDING = /^ngAttr[A-Z]/;
        ...
        compile.$$addScopeInfo = debugInfoEnabled ? function $$addScopeInfo($element, scope, isolated, noTemplate) {
          const dataName = isolated ? (noTemplate ? '$isolateScopeNoTemplate' : '$isolateScope') : '$scope';
     ...
     */
        function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective,
          previousCompileContext) {
          const linkFns = [];
          let attrs; let directives; let nodeLinkFn; let childNodes; let childLinkFn; let linkFnFound; let nodeLinkFnFound;
          ...
          function compositeLinkFn(scope, nodeList, $rootElement, parentBoundTranscludeFn) {
            let nodeLinkFn; let childLinkFn; let node; let childScope; let i; let ii; let idx; let childBoundTranscludeFn;
            let stableNodeList;
            ...
            for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes,
              ...
              if (isNgAttr = NG_ATTR_BINDING.test(ngAttrName)) {
                name = name.replace(PREFIX_REGEXP, '')
                  .substr(8).replace(/_(.)/g, (match, letter) => letter.toUpperCase());
              }
     ...
     */
        function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn,
          ...
          function getControllers(directiveName, require, $element, elementControllers) {
            let value; let retrievalMethod = 'data'; let optional = false;
          ...
          function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
            let i; let ii; let linkFn; let controller; let isolateScope; let elementControllers; let transcludeFn; let $element;
            let attrs;
            ...
            if (newIsolateScopeDirective) {
              ...
              const isolateScopeController = controllers && controllers[newIsolateScopeDirective.name];
              let isolateBindingContext = isolateScope;
              if (isolateScopeController && isolateScopeController.identifier &&
              newIsolateScopeDirective.bindToController === true) {
                isolateBindingContext = isolateScopeController.instance;
              }

              forEach(isolateScope.$$isolateBindings = newIsolateScopeDirective.$$isolateBindings, (definition, scopeName) => {
                const {attrName} = definition;
                const {optional} = definition;
                const {mode} = definition; // @, =, or &
                let lastValue;
                let parentGet; let parentSet; let compare;
     ...
     */
        ...
        function compileTemplateUrl(directives, $compileNode, tAttrs,
          ...
          $templateRequest($sce.getTrustedResourceUrl(templateUrl))
            .then((content) => {
              let compileNode; let tempTemplateAttrs; let $template; let childBoundTranscludeFn;
     ...
     */
        ...
        function addAttrInterpolateDirective(node, directives, value, name, allOrNothing) {
          const trustedContext = getTrustedContext(node, name);
          allOrNothing = ALL_OR_NOTHING_ATTRS[name] || allOrNothing;

          let interpolateFn = $interpolate(value, true, trustedContext, allOrNothing);
 ...
 */
   ...
   */
     ...
     */
      ...
      //                     expression, if any.
        let instance; let match; let constructor; let identifier;
        later = later === true;
        ...
        // publicly.
        // Object creation: http://jsperf.com/create-constructor/2
          const controllerPrototype = (isArray(expression) ?
 ...
 */
  function parseHeaders(headers) {
    const parsed = createMap(); let key; let val; let i;
     ...
     */
        function $http(requestConfig) {
          ...
          function executeHeaderFns(headers) {
            let headerContent; const processedHeaders = {};
          ...
          function mergeHeaders(config) {
            ...
            let defHeaderName; let lowercaseDefHeaderName; let reqHeaderName;
 ...
 */
  ...
  // TODO(vojta): fix the signature
    ...
    // - adds and immediately removes script elements from the document
      let script = rawDocument.createElement('script'); let callback = null;
        ...
        if (!mustHaveExpression || expressions.length) {
          ...
          return extend((context) => {
            ...
            try {
              ...
              const newErr = $interpolateMinErr('interr', "Can't interpolate: {0}\n{1}", text,
                err.toString());
          ...
          // all of these properties are undocumented for now
            ...
            expressions,
            $$watchDelegate(scope, listener, objectEquality) {
              let lastValue;
 ...
 */
  ...
  function parseAppUrl(relativeUrl, locationObj) {
    ...
    const match = urlResolve(relativeUrl);
 ...
 */
   ...
   */
    this.$$compose = function() {
      const search = toKeyValue(this.$$search);
      const hash = this.$$hash ? `#${  encodeUriSegment(this.$$hash)}` : '';
 ...
 */
  ...
  forEach([LocationHashbangInHtml5Url, LocationHashbangUrl, LocationHtml5Url], (Location) => {
   ...
   */
    ...
    this.$get = ['$rootScope', '$browser', '$sniffer', '$rootElement', '$window',
      function($rootScope, $browser, $sniffer, $rootElement, $window) {
        ...
        // update browser
        $rootScope.$watch(() => {
          const oldUrl = trimEmptyHash($browser.url());
          const newUrl = trimEmptyHash($location.absUrl());
          const oldState = $browser.state();
          const currentReplace = $location.$$replace;
          const urlOrStateChanged = oldUrl !== newUrl ||
        ($location.$$html5 && $sniffer.history && oldState !== $location.$$state);
          ...
          if (initializing || urlOrStateChanged) {
            ...
            $rootScope.$evalAsync(() => {
              ...
              const {defaultPrevented} = $rootScope.$broadcast('$locationChangeStart', newUrl, oldUrl,
                $location.$$state, oldState);
 ...
 */
  ...
  const OPERATORS = extend(createMap(), {
 ...
 */
  ...
  Parser.prototype = {
    ...
    binaryFn(left, op, right, isBranching) {
      const fn = OPERATORS[op];
    ...
    filter(inputFn) {
      const fn = this.$filter(this.consume().text);
      let argsFn;
      let args;
      ...
      const inputs = [inputFn].concat(argsFn || []);

      return extend((self, locals) => {
        const input = inputFn(self, locals);
        if (args) {
          args[0] = input;
    ...
    assignment() {
      ...
      if ((token = this.expect('='))) {
        ...
        right = this.ternary();
        return extend((scope, locals) => left.assign(scope, right(scope, locals), locals), {
          inputs: [left, right]
        });
      }
      return left;
    ...
    fieldAccess(object) {
      ...
      return extend((scope, locals, self) => {
        const o = self || object(scope, locals);
    ...
    objectIndex(obj) {
      ...
      return extend((self, locals) => {
        const o = obj(self, locals);
        const i = indexFn(self, locals);
        let v;
        ...
        assign(self, value, locals) {
          const key = ensureSafeMemberName(indexFn(self, locals), expression);
          // prevent overwriting of Function.constructor which would break ensureSafeObject check
          let o = ensureSafeObject(obj(self, locals), expression);
    ...
    functionCall(fnGetter, contextGetter) {
      const argsFn = [];
      ...
      const args = argsFn.length ? [] : null;

      return function $parseFunctionCall(scope, locals) {
        const context = contextGetter ? contextGetter(scope, locals) : isDefined(contextGetter) ? undefined : scope;
        const fn = fnGetter(scope, locals, context) || noop;
    ...
    arrayDeclaration() {
      ...
      return extend((self, locals) => {
        const array = [];
        for (let i = 0, ii = elementFns.length; i < ii; i++) {
    ...
    object() {
      ...
      return extend((self, locals) => {
        const object = {};
        for (let i = 0, ii = valueFns.length; i < ii; i++) {
  ...
  function setter(obj, locals, path, setValue, fullExp) {
    ...
    const element = path.split('.'); let key;
    for (let i = 0; element.length > 1; i++) {
      key = ensureSafeMemberName(element.shift(), fullExp);
      let propertyObj = (i === 0 && locals && locals[key]) || obj[key];
      if (!propertyObj) {
        propertyObj = {};
        obj[key] = propertyObj;
      }
  ...
  const getterFnCacheDefault = createMap();
  const getterFnCacheExpensive = createMap();
 ...
 */
  ...
  function getterFn(path, options, fullExp) {
    ...
    if (options.csp) {
      ...
      /* jshint -W054 */
      let evaledFnGetter = new Function('s', 'l', 'eso', 'fe', code); // s=scope, l=locals, eso=ensureSafeObject
      /* jshint +W054 */
 ...
 */
  function $ParseProvider() {
    const cacheDefault = createMap();
    const cacheExpensive = createMap();
    ...
    this.$get = ['$filter', '$sniffer', function($filter, $sniffer) {
      ...
      return function $parse(exp, interceptorFn, expensiveChecks) {
        let parsedExpression; let oneTime; let cacheKey;
        ...
        case 'string':
          ...
          if (!parsedExpression) {
            ...
            const parseOptions = expensiveChecks ? $parseOptionsExpensive : $parseOptions;
      ...
      function inputsWatchDelegate(scope, listener, objectEquality, parsedExpression) {
        ...
        if (inputExpressions.length === 1) {
          let oldInputValue = expressionInputDirtyCheck; // init to something unique so that equals check fails
      ...
      function oneTimeWatchDelegate(scope, listener, objectEquality, parsedExpression) {
        let unwatch; let lastValue;
        return unwatch = scope.$watch((scope) => parsedExpression(scope), function oneTimeListener(value, old, scope) {
          lastValue = value;
          if (isFunction(listener)) {
      ...
      function constantWatchDelegate(scope, listener, objectEquality, parsedExpression) {
        let unwatch;
        return unwatch = scope.$watch((scope) => parsedExpression(scope), function constantListener(value, old, scope) {
          if (isFunction(listener)) {
            listener.apply(this, arguments);
          }
          unwatch();
      ...
      function addInterceptor(parsedExpression, interceptorFn) {
        ...
        const watchDelegate = parsedExpression.$$watchDelegate;

        const regularWatch =
          watchDelegate !== oneTimeLiteralWatchDelegate &&
          watchDelegate !== oneTimeWatchDelegate;

        const fn = regularWatch ? function regularInterceptedExpression(scope, locals) {
          const value = parsedExpression(scope, locals);
          return interceptorFn(value, scope, locals);
        } : function oneTimeInterceptedExpression(scope, locals) {
          const value = parsedExpression(scope, locals);
          const result = interceptorFn(value, scope, locals);
 ...
 */
   ...
   */
    ...
    Promise.prototype = {
      ...
      "finally": function(callback, progressBack) {
        return this.then((value) => handleCallback(value, true, callback), (error) => handleCallback(error, false, callback), progressBack);
      }
    };
    ...
    function processQueue(state) {
      let fn; let promise; let pending;
    ...
    Deferred.prototype = {
      ...
      notify(progress) {
        const callbacks = this.promise.$$state.pending;
   ...
   */
    ...
    const makePromise = function makePromise(value, resolved) {
      const result = new Deferred();
      if (resolved) {
  ...
  function $RootScopeProvider() {
     ...
     */
      ...
      */
       ...
       */
          $watchGroup(watchExpressions, listener) {
            ...
            forEach(watchExpressions, (expr, i) => {
              const unwatchFn = self.$watch(expr, (value, oldValue) => {
                newValues[i] = value;
       ...
       */
          $watchCollection(obj, listener) {
            ...
            function $watchCollectionInterceptor(_value) {
              newValue = _value;
              let newLength; let key; let bothNaN; let newItem; let oldItem;
       ...
       */
          $digest() {
            let watch; let value; let last;
            let watchers;
            let length;
            let dirty; let ttl = TTL;
            let next; let current; const target = this;
            const watchLog = [];
            let logIdx; let logMsg; let asyncTask;
       ...
       */
          $evalAsync(expr, locals) {
            ...
            asyncQueue.push({scope: this, expression: expr, locals});
          },
       ...
       */
          $broadcast(name, args) {
            ...
            const listenerArgs = concat([event], arguments, 1);
            let listeners; let i; let length;
 ...
 */
  function $$SanitizeUriProvider() {
    ...
    let imgSrcSanitizationWhitelist = /^\s*((https?|ftp|file|blob):|data:image\/)/;
    ...
    this.$get = ['$templateCache', '$http', '$q', function($templateCache, $http, $q) {
      function handleRequestFn(tpl, ignoreRequestError) {
        ...
        if (isArray(transformResponse)) {
          transformResponse = transformResponse.filter((transformer) => transformer !== defaultHttpResponseTransform);
          ...
          forEach(bindings, (binding) => {
            ...
            if (dataBinding) {
              forEach(dataBinding, (bindingName) => {
                if (opt_exactMatch) {
                  const matcher = new RegExp(`(^|\\s)${  escapeForRegexp(expression)  }(\\s|\\||$)`);
 ...
 */
  ...
  function deepCompare(actual, expected, comparator, matchAgainstAnyProp, dontMatchWholeObject) {
    ...
    // if ANY of it's items matches `expected`
      return actual.some((item) => deepCompare(item, expected, comparator, matchAgainstAnyProp));
    }
 ...
 */
  ...
  const DATE_FORMATS_SPLIT = /((?:[^yMdHhmsaZEw']+)|(?:'(?:[^']|'')*')|(?:E+|y+|M+|d+|H+|h+|m+|s+|a|Z|w+))(.*)/;
 ...
 */
  ...
  function orderByFilter($parse) {
    return function(array, sortPredicate, reverseOrder) {
      ...
      sortPredicate = sortPredicate.map((predicate) => {
        let descending = false; let get = predicate || identity;
 ...
 */
  ...
  function FormController(element, attrs, $scope, $animate, $interpolate) {
    ...
    const parentForm = form.$$parentForm = element.parent().controller('form') || nullFormCtrl;
   ...
   */
    form.$removeControl = function(control) {
      ...
      forEach(form.$pending, (value, name) => {
        form.$setValidity(name, null, control);
      });
      forEach(form.$error, (value, name) => {
        form.$setValidity(name, null, control);
      });
      forEach(form.$$success, (value, name) => {
        form.$setValidity(name, null, control);
      });
        ...
        // Setup initial state of the control
          ...
          return {
            ...
            // if `action` attr is not present on the form, prevent the default action (submission)
              ...
              const parentFormCtrl = controller.$$parentForm;
              let alias = controller.$name;

              if (alias) {
                setter(scope, null, alias, controller, alias);
                attr.$observe(attr.name ? 'name' : 'ngForm', (newValue) => {
...
*/
  ...
  function createDateInputType(type, regexp, parseDate, format) {
    return function dynamicDateInputType(scope, element, attr, ctrl, $sniffer, $browser, $filter) {
      ...
      baseInputType(scope, element, attr, ctrl, $sniffer, $browser);
      const timezone = ctrl && ctrl.$options && ctrl.$options.timezone;
      let previousDate;
      ...
      ctrl.$parsers.push((value) => {
        ...
        // contains some different data format!
          const parsedDate = parseDate(value, previousDate);
          if (timezone === 'UTC') {
      ...
      ctrl.$formatters.push((value) => {
        ...
        if (isValidDate(value)) {
          ...
          if (previousDate && timezone === 'UTC') {
            const timezoneOffset = 60000 * previousDate.getTimezoneOffset();
  ...
  function badInputChecker(scope, element, attr, ctrl) {
    ...
    const nativeValidation = ctrl.$$hasNativeValidators = isObject(node.validity);
    if (nativeValidation) {
      ctrl.$parsers.push((value) => {
  ...
  var ngBindHtmlDirective = ['$sce', '$parse', '$compile', function($sce, $parse, $compile) {
    return {
      ...
      compile: function ngBindHtmlCompile(tElement, tAttrs) {
        ...
        const ngBindHtmlWatch = $parse(tAttrs.ngBindHtml, (value) => (value || '').toString());
  ...
  var ngIfDirective = ['$animate', function($animate) {
    return {
      ...
      link($scope, $element, $attr, ctrl, $transclude) {
        let block; let childScope; let previousElements;
  ...
  var ngListDirective = function() {
    return {
      ...
      // to access the ngList attribute, which doesn't pre-trim the attribute
        const ngList = element.attr(attr.$attr.ngList) || ', ';
 ...
 *
 */
  const NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', '$animate', '$timeout', '$rootScope', '$q', '$interpolate',
    function($scope, $exceptionHandler, $attr, $element, $parse, $animate, $timeout, $rootScope, $q, $interpolate) {
      ...
      const parsedNgModel = $parse($attr.ngModel);
      const parsedNgModelAssign = parsedNgModel.assign;
      let ngModelGet = parsedNgModel;
      let ngModelSet = parsedNgModelAssign;
      ...
      this.$$setOptions = function(options) {
        ...
        if (options && options.getterSetter) {
          const invokeModelGetter = $parse(`${$attr.ngModel  }()`);
          const invokeModelSetter = $parse(`${$attr.ngModel  }($$$p)`);

          ngModelGet = function($scope) {
            let modelValue = parsedNgModel($scope);
   ...
   */
      this.$validate = function() {
        ...
        const viewValue = ctrl.$$lastCommittedViewValue;
        // Note: we use the $$rawModelValue as $modelValue might have been
        // set to undefined during a view -> model update that found validation
        // errors. We can't parse the view here, since that could change
        // the model although neither viewValue nor the model on the scope changed
        const modelValue = ctrl.$$rawModelValue;

        // Check if the there's a parse error, so we don't unset it accidentially
        const parserName = ctrl.$$parserName || 'parse';
        const parserValid = ctrl.$error[parserName] ? false : undefined;

        const prevValid = ctrl.$valid;
        const prevModelValue = ctrl.$modelValue;

        const allowInvalid = ctrl.$options && ctrl.$options.allowInvalid;

        ctrl.$$runValidators(parserValid, modelValue, viewValue, (allValid) => {
      ...
      this.$$runValidators = function(parseValid, modelValue, viewValue, doneCallback) {
        ...
        const localValidationRunId = currentValidationRunId;
        ...
        function processParseErrors(parseValid) {
          const errorKey = ctrl.$$parserName || 'parse';
          if (parseValid === undefined) {
            setValidity(errorKey, null);
          } else {
            setValidity(errorKey, parseValid);
            if (!parseValid) {
              forEach(ctrl.$validators, (v, name) => {
                setValidity(name, null);
              });
              forEach(ctrl.$asyncValidators, (v, name) => {
                setValidity(name, null);
              });
              return false;
            }
        ...
        function processSyncValidators() {
          ...
          forEach(ctrl.$validators, (validator, name) => {
        ...
        function processAsyncValidators() {
          ...
          let allValid = true;
          forEach(ctrl.$asyncValidators, (validator, name) => {
   ...
   */
      this.$commitViewValue = function() {
        const viewValue = ctrl.$viewValue;
      ...
      this.$$parseAndValidate = function() {
        ...
        let parserValid = isUndefined(modelValue) ? undefined : true;

        if (parserValid) {
          for (let i = 0; i < ctrl.$parsers.length; i++) {
        ...
        ctrl.$$runValidators(parserValid, modelValue, ctrl.$$lastCommittedViewValue, (allValid) => {
          if (!allowInvalid) {
            // Note: Don't check ctrl.$valid here, as we could have
            // external validators (e.g. calculated on the server),
            // that just call $setValidity and need the model value
      ...
      this.$$writeModelToScope = function() {
        ...
        forEach(ctrl.$viewChangeListeners, (listener) => {
          try {
            listener();
          } catch (e) {
            $exceptionHandler(e);
   ...
   */
      ...
      $scope.$watch(() => {
        const modelValue = ngModelGet($scope);
        ...
        if (modelValue !== ctrl.$modelValue) {
          ...
          const formatters = ctrl.$formatters;
      ...
      // Setup initial state of the control
        ...
        return {
          pre: function ngModelPreLink(scope, element, attr, ctrls) {
            ...
            const formCtrl = ctrls[1] || nullFormCtrl;
          ...
          post: function ngModelPostLink(scope, element, attr, ctrls) {
            ...
            if (modelCtrl.$options && modelCtrl.$options.updateOn) {
              element.on(modelCtrl.$options.updateOn, (ev) => {
  ...
  var ngRepeatDirective = ['$parse', '$animate', function($parse, $animate) {
    ...
    return {
      ...
      compile: function ngRepeatCompile($element, $attr) {
        ...
        let trackByExpGetter; let trackByIdExpFn; let trackByIdArrayFn; let trackByIdObjFn;
        ...
        return function ngRepeatLink($scope, $element, $attr, ctrl, $transclude) {
          ...
          $scope.$watchCollection(rhs, (collection) => {
            ...
            let key; let value; // key/value of iteration
            let trackById;
            let trackByIdFn;
            let collectionKeys;
      ...
      link(scope, element, attr, ngSwitchController) {
        ...
        scope.$watch(watchExpr, (value) => {
          ...
          for (i = 0, ii = selectedScopes.length; i < ii; ++i) {
            ...
            const promise = previousLeaveAnimations[i] = $animate.leave(selected);
 ...
 */
  ...
  var selectDirective = ['$compile', '$parse', function($compile,   $parse) {
    ...
    const nullModelCtrl = {$setViewValue: noop};
    // jshint maxlen: 100
    ...
    return {
      ...
      // if ngModel is not defined, we don't need to do anything
        ...
        const {multiple} = attr;
        const optionsExp = attr.ngOptions;
        let nullOption = false; // if false, user will not be able to select it (used by ngOptions)
        let emptyOption;
        let renderScheduled = false;
        // we can't just jqLite('<option>') since jqLite is not smart enough
        // to create it in <select> and IE barfs otherwise.
        const optionTemplate = jqLite(document.createElement('option'));
        const optGroupTemplate =jqLite(document.createElement('optgroup'));
        const unknownOption = optionTemplate.clone();
        ...
        function setupAsOptions(scope, selectElement, ctrl) {
          ...
          const selectAsFn = selectAs ? $parse(selectAs) : null;
          ...
          const track = match[8];
          const trackFn = track ? $parse(match[8]) : null;
          let trackKeysCache = {};
          // This is an array of array of existing option groups in DOM.
          // We try to reuse these if possible
          // - optionGroupsCache[0] is the options with no option group
          // - optionGroupsCache[?][0] is the parent: either the SELECT or OPTGROUP element
          const optionGroupsCache = [[{element: selectElement, label:''}]];
          ...
          if (multiple) {
            scope.$watchCollection(() => ctrl.$modelValue, scheduleRendering);
          }
          ...
          function selectionChanged() {
            scope.$apply(() => {
              const collection = valuesFn(scope) || [];
              let viewValue;
              if (multiple) {
                viewValue = [];
                forEach(selectElement.val(), (selectedKey) => {
                  selectedKey = trackFn ? trackKeysCache[selectedKey] : selectedKey;
                  viewValue.push(getViewValue(selectedKey, collection[selectedKey]));
                });
              } else {
                const selectedKey = trackFn ? trackKeysCache[selectElement.val()] : selectElement.val();
          ...
          function createIsSelectedFn(viewValue) {
            ...
            if (multiple) {
              if (trackFn && isArray(viewValue)) {
                ...
                for (let trackIndex = 0; trackIndex < viewValue.length; trackIndex++) {
                // tracking by key
        ...
        */
          ...
          function render() {
            ...
            let optionGroup;
            let option;
            let existingParent; let existingOptions; let existingOption;
            const viewValue = ctrl.$viewValue;
            const values = valuesFn(scope) || [];
            const keys = keyName ? sortedKeys(values) : values;
            let key;
            let value;
            let groupLength; let length;
            let groupIndex; let index;
            const labelMap = {};
            let selected;
            const isSelected = createIsSelectedFn(viewValue);
    ...
    return {
      ...
      compile(element, attr) {
        ...
        return function(scope, element, attr) {
          ...
          let selectCtrl = parent.data(selectCtrlName) ||
  ...
  var patternDirective = function() {
    return {
      ...
      link(scope, elm, attr, ctrl) {
        ...
        let regexp; const patternExp = attr.ngPattern || attr.pattern;
        attr.$observe('pattern', (regex) => {
