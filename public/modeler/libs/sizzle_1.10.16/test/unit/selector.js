...
test("element", () => {
  ...
  let form; let all; let good; let i; let obj1; let lengthtest;
  let siblingTest; let siblingNext; let iframe; let iframeDoc; let html;
...
test("broken", () => {
  ...
  const broken = function( name, selector ) {
    ...
    }, ( e ) => e.message.indexOf("Syntax error") >= 0, `${name  }: ${  selector}` );
  };
...
test("child and adjacent", () => {
  ...
  let siblingFirst; let en; let nothiddendiv;
...
test("attributes", () => {
  ...
  let opt; let input; let attrbad; let div;
...
test("pseudo - (first|last|only)-(child|of-type)", () => {
  ...
  const secondChildren = jQuery("p:nth-child(2)").before("<div></div>");
...
test("pseudo - misc", () => {
  ...
  jQuery.each( [ "button", "submit", "reset" ], ( i, type ) => {
    const els = jQuery(
...
test("pseudo - position", () => {
  ...
  // jQuery #12526
  const context = jQuery("#qunit-fixture").append("<div id='jquery12526'></div>")[0];
...
test("pseudo - form", () => {
  ...
  const extraTexts = jQuery("<input id=\"impliedText\"/><input id=\"capitalText\" type=\"TEXT\">").appendTo("#form");
...
test("pseudo - :lang", () => {
  ...
  var assertMatch = function( text, elem, selector ) {
    ok( Sizzle.matchesSelector( elem, selector ), `${text  } match ${  selector}` );
  };
  var assertNoMatch = function( text, elem, selector ) {
  ...
  }, ( e ) => e.message.indexOf("Syntax error") >= 0, ":lang value must be a valid identifier" );
...
asyncTest( "Iframe dispatch should not affect Sizzle, see jQuery #13936", 1, () => {
  let loaded = false;
  let thrown = false;
...
test("matchesSelector", () => {
  ...
  const el = document.getElementById("simon1");
