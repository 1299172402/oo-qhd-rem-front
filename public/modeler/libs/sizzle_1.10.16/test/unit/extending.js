...
test("custom pseudos", () => {
  ...
  Sizzle.selectors.filters.aristotlean = Sizzle.selectors.createPseudo(() => function( elem ) {
    return !!elem.id;
  });
  t( "Custom element filter", "#foo :aristotlean", [ "sndp", "en", "yahoo", "sap", "anchor2", "simon" ] );
  delete Sizzle.selectors.filters.aristotlean;

  Sizzle.selectors.filters.endswith = Sizzle.selectors.createPseudo(( text ) => function( elem ) {
  ...
  Sizzle.selectors.setFilters.second = Sizzle.selectors.createPseudo(() => Sizzle.selectors.createPseudo(( seed, matches ) => {
    if ( seed[1] ) {
      matches[1] = seed[1];
      seed[1] = false;
    }
  ...
  Sizzle.selectors.setFilters.slice = Sizzle.selectors.createPseudo(( argument ) => {
    const bounds = argument.split(":");
    return Sizzle.selectors.createPseudo(( seed, matches ) => {
      let i = bounds[1];
...
test("backwards-compatible custom pseudos", () => {
  expect( 3 );
  ...
  Sizzle.selectors.setFilters.podium = function( elements, argument ) {
    const count = argument == null || argument === "" ? 3 : +argument;
...
test("custom attribute getters", () => {
  expect( 2 );

  const original = Sizzle.selectors.attrHandle.hreflang;
  const selector = "a:contains('mark')[hreflang='http://diveintomark.org/en']";

  Sizzle.selectors.attrHandle.hreflang = function( elem, name ) {
    const href = elem.getAttribute("href");
    const lang = elem.getAttribute( name );
