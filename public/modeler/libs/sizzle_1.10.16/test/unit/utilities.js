...
test("Sizzle.attr (HTML)", () => {
  testAttr();
});

test("Sizzle.attr (XML)", () => {
  testAttr( jQuery.parseXML("<root/>") );
});

test("Sizzle.contains", () => {
  expect( 16 );

  const container = document.getElementById("nonnodes");
  const element = container.firstChild;
  const text = element.nextSibling;
  const nonContained = container.nextSibling;
...
if ( jQuery("<svg xmlns='http://www.w3.org/2000/svg' version='1.1' height='1' width='1'><g/></svg>")[0].firstChild ) {
  test("Sizzle.contains in SVG (jQuery #10832)", () => {
    expect( 4 );
...
test("Sizzle.uniqueSort", () => {
  expect( 14 );

  function Arrayish( arr ) {
    let i = this.length = arr.length;
    while ( i-- ) {
      this[ i ] = arr[ i ];
    }
  }
  ...
  const detached1 = document.createElement("p");
  const detached2 = document.createElement("ul");
  const detachedChild = detached1.appendChild( document.createElement("a") );
  const detachedGrandchild = detachedChild.appendChild( document.createElement("b") );
  ...
  jQuery.each( tests, ( label, test ) => {
    const length = test.length || test.input.length;
