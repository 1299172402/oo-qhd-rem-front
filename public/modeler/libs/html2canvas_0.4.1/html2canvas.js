...
(function(window, document, undefined){
  ...
  _html2canvas.Util.parseBackgroundImage = function (value) {
    ...
    let c; let mode = 0; let numParen = 0; let quote; let args;
  ...
  _html2canvas.Util.resizeBounds = function( current_width, current_height, target_width, target_height, stretch_mode ){
    ...
    let output_width; let output_height;
  ...
  function backgroundBoundsFactory( prop, el, bounds, image, imageIndex, backgroundSize ) {
    ...
    if (bgposition[0].toString().indexOf("%") !== -1){
      ...
      if(bgposition[0] === 'auto') {
        left = image.width;
      } else if (/contain|cover/.test(bgposition[0])) {
        const resized = _html2canvas.Util.resizeBounds(image.width, image.height, bounds.width, bounds.height, bgposition[0]);
        left = resized.width;
 ...
 */
    Generate.parseGradient = function(css, bounds) {
      let gradient; let i; const len = reGradients.length; let m1; let stop; let m2; let m2Len; let step; let m3; let tl; let tr; let br; let bl;
    ...
    Generate.Gradient = function(src, bounds) {
      ...
      if(gradient) {
        ...
        case 'ellipse':
          ...
          var ctxRadial = canvasRadial.getContext('2d');
          var ri = Math.max(gradient.rx, gradient.ry);
          var di = ri * 2;
  ...
  _html2canvas.Parse = function (images, options) {
    ...
    const element = (( options.elements === undefined ) ? document.body : options.elements[0]); // select body by default
    let numDraws = 0;
  ...
  `.${  pseudoHide  }-after:after { content: "" !important; display: none !important; }`;
    ...
    function listItemText(element, type) {
      let currentIndex = elementIndex(element); let text;
      switch(type){
      case "decimal":
        text = currentIndex;
        break;
    ...
    function clipBounds(src, dst){
      const x = Math.max(src.left, dst.left);
      const y = Math.max(src.top, dst.top);
      const x2 = Math.min((src.left + src.width), (dst.left + dst.width));
      const y2 = Math.min((src.top + src.height), (dst.top + dst.height));
    ...
    function getBorderRadiusData(element) {
      return ["TopLeft", "TopRight", "BottomRight", "BottomLeft"].map((side) => getCSS(element, `border${  side  }Radius`));
    }

    const getCurvePoints = (function(kappa) {
    ...
    function parseBorders(element, bounds, borders){
      ...
      const borderRadius = getBorderRadiusData(element);
    ...
    function renderBackgroundRepeat(ctx, image, backgroundPosition, bounds) {
      const offsetX = Math.round(bounds.left + backgroundPosition.left);
      const offsetY = Math.round(bounds.top + backgroundPosition.top);
    ...
    function resizeImage(image, bounds) {
      ...
      let ctx; const canvas = doc.createElement('canvas');
      canvas.width = bounds.width;
      canvas.height = bounds.height;
    ...
    function getBounds(element, transform) {
      const bounds = (transform.matrix) ? Util.OffsetBounds(element) : Util.Bounds(element);
  ...
  _html2canvas.Preload = function( options ) {
    ...
    const doc = element.ownerDocument;
    const domImages = element.getElementsByTagName('img'); // Fetch images of the present element only
    const imgLen = domImages.length;
    const link = doc.createElement("a");
    const supportCORS = (function( img ){
    ...
    function createRenderQueue(parseQueue) {
      ...
      rootContext = (function buildStackingContext(rootNode) {
        ...
        function insert(context, node, specialParent) {
          const zi = (node.zIndex.zindex === 'auto') ? 0 : Number(node.zIndex.zindex);
          let contextForChildren = context; // the stacking context for children
          const {isPositioned} = node.zIndex;
          const {isFloated} = node.zIndex;
          const stub = {node};
          let childrenDest = specialParent; // where children without z-index should be pushed into
  ...
  _html2canvas.Util.Support = function (options, doc) {
    ...
    function supportRangeBounds() {
      let r; let testElement; let rangeBounds; let rangeHeight; let support = false;
    ...
    return function(parsedData, options, document, queue, _html2canvas) {
      ...
      if (options.elements.length === 1) {
        ...
        // crop image to the bounds of selected (single) element
          ...
          const imgData = canvas.getContext("2d").getImageData(bounds.left, bounds.top, bounds.width, bounds.height);
