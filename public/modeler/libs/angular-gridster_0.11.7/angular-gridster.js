  ...
  angular.module('gridster', [])
      ...
      function(gridsterConfig, $timeout) {
        ...
        this.setOptions = function(options) {
          ...
          if (!this.margins || this.margins.length !== 2) {
            ...
            for (let x = 0, l = this.margins.length; x < l; ++x) {
        ...
        this.autoSetItemPosition = function(item) {
          ...
          for (let rowIndex = 0; rowIndex < this.maxRows; ++rowIndex) {
            for (let colIndex = 0; colIndex < this.columns; ++colIndex) {
              ...
              const items = this.getItems(rowIndex, colIndex, item.sizeX, item.sizeY, item);
        ...
        this.getItems = function(row, column, sizeX, sizeY, excludeItems) {
          ...
          for (let h = 0; h < sizeY; ++h) {
            for (let w = 0; w < sizeX; ++w) {
              const item = this.getItem(row + h, column + w, excludeItems);
        ...
        this.removeItem = function(item) {
          for (let rowIndex = 0, l = this.grid.length; rowIndex < l; ++rowIndex) {
            const columns = this.grid[rowIndex];
            if (!columns) {
              continue;
            }
        ...
        this.putItem = function(item, row, column, ignoreItems) {
          ...
          if (item.oldRow !== null && typeof item.oldRow !== 'undefined') {
            const samePosition = item.oldRow === row && item.oldColumn === column;
            const inGrid = this.grid[row] && this.grid[row][column] === item;
            ...
            const oldRow = this.grid[item.oldRow];
        ...
        this.swapItems = function(item1, item2) {
          ...
          const item1Row = item1.row;
          const item1Col = item1.col;
          item1.row = item2.row;
          item1.col = item2.col;
          item2.row = item1Row;
          item2.col = item1Col;
        ...
        this.moveOverlappingItems = function(item, ignoreItems) {
          ...
          const overlappingItems = this.getItems(
            item.row,
            item.col,
            item.sizeX,
            item.sizeY,
        ...
        this.moveItemsDown = function(items, newRow, ignoreItems) {
          ...
          const topRows = {};
          let item; let i; let l;
          // calculate the top rows in each column
          for (i = 0, l = items.length; i < l; ++i) {
            item = items[i];
            const topRow = topRows[item.col];
          ...
          for (i = 0, l = items.length; i < l; ++i) {
            item = items[i];
            const rowsToMove = newRow - topRows[item.col];
        ...
        this.floatItemUp = function(item) {
          ...
          while (rowIndex > -1) {
            const items = this.getItems(rowIndex, colIndex, sizeX, sizeY, item);
            if (items.length !== 0) {
              break;
            }
            bestRow = rowIndex;
        ...
        this.updateHeight = function(plus) {
          let maxHeight = this.minRows;
          plus = plus || 0;
          for (let rowIndex = this.grid.length; rowIndex >= 0; --rowIndex) {
            const columns = this.grid[rowIndex];
            if (!columns) {
              continue;
            }
        ...
        this.unifiedInput = function(target, startEvent, moveEvent, endEvent) {
          ...
          const numberOfKeys = function(theObject) {
            if (Object.keys) {
          ...
          const computeDocumentToElementDelta = function(theElement) {
            let elementLeft = 0;
            let elementTop = 0;
            const oldIEUserAgent = navigator.userAgent.match(/\bMSIE\b/);

            for (let offsetElement = theElement; offsetElement != null; offsetElement = offsetElement.offsetParent) {
          ...
          let documentToTargetDelta = computeDocumentToElementDelta(target);
          ...
          var doEvent = function(theEvtObj) {
            ...
            const pointerList = theEvtObj.changedTouches ? theEvtObj.changedTouches : [theEvtObj];
            for (let i = 0; i < pointerList.length; ++i) {
              const pointerObj = pointerList[i];
              ...
              if (typeof pointerObj.pageX === 'undefined') {
                ...
                if (pointerObj.srcElement.offsetParent === target && document.documentMode && document.documentMode === 8 && pointerObj.type === 'mousedown') {
                  ...
                  let sx = -2;
                  let sy = -2; // adjust for old IE's 2-pixel border
                  for (let scrollElement = pointerObj.srcElement; scrollElement !== null; scrollElement = scrollElement.parentNode) {
              ...
              const {pageX} = pointerObj;
              const {pageY} = pointerObj;
          ...
          let contentZooming; let msTouchAction;
          ...
          this.enable = function() {
            ...
            if (window.navigator.msPointerEnabled) {
              ...
              useSetReleaseCapture = true;
              target.attachEvent('onmousedown', () => {
                doEvent(window.event);
                window.event.returnValue = false;
                return false;
              });
              target.attachEvent('onmousemove', () => {
                doEvent(window.event);
                window.event.returnValue = false;
                return false;
              });
              target.attachEvent('onmouseup', () => {
                doEvent(window.event);
                window.event.returnValue = false;
                return false;
              });
          ...
          compile() {
            ...
            return function(scope, $elem, attrs, gridster) {
              ...
              const refresh = function() {
                ...
                for (let rowIndex = 0, l = gridster.grid.length; rowIndex < l; ++rowIndex) {
                  const columns = gridster.grid[rowIndex];
                  if (!columns) {
                    continue;
                  }
              ...
              scope.$watch('config.draggable', () => {
                $rootScope.$broadcast('gridster-draggable-changed');
              }, true);

              scope.$watch('config.resizable', () => {
              ...
              scope.$watch('gridster.movingItem', () => {
                gridster.updateHeight(gridster.movingItem ? gridster.movingItem.sizeY : 0);
              });

              let prevWidth = $elem[0].offsetWidth || parseInt($elem.css('width'), 10);

              function resize() {
                const width = $elem[0].offsetWidth || parseInt($elem.css('width'), 10);
              ...
              scope.$watch(() => $elem[0].offsetWidth || parseInt($elem.css('width'), 10), resize);
              ...
              $rootScope.dropCallback = function (event, ui) {
                $timeout(() => {
                  ...
                  const offset = jQuery($elem[0]).offset();
                  const row = gridster.pixelsToRows(event.clientY - offset.top, true) - 1;
                  const column = gridster.pixelsToColumns(event.clientX - offset.left, true) - 1;
                  if (row >= 0 && column >= 0) {
    ...
    .controller('GridsterItemCtrl', function() {
      ...
      this.setSize = function(key, value, preventMove) {
        key = key.toUpperCase();
        const camelCase = `size${  key}`;
        const titleCase = `Size${  key}`;
        if (value === '') {
          return;
        }
        ...
        let max = key === 'X' ? this.gridster.columns : this.gridster.maxRows;
        ...
        const changed = (this[camelCase] !== value || (this[`old${  titleCase}`] && this[`old${  titleCase}`] !== value));
    ...
    .factory('GridsterDraggable', ['$document', '$timeout', '$window',
      function($document, $timeout, $window) {
        function GridsterDraggable($el, scope, gridster, item, itemOptions) {
          ...
          let elmX; let elmY; let elmW; let elmH;
          ...
          const maxTop = 9999;
          const minLeft = 0;
          const realdocument = $document[0];

          let originalCol; let originalRow;
          const inputTags = ['select', 'input', 'textarea', 'button'];
          ...
          function mouseMove(e) {
            ...
            const maxLeft = gridster.curWidth - 1;
          ...
          function drag(event) {
            ...
            const hasCallback = gridster.draggable && gridster.draggable.drag;
            const {scrollSensitivity} = gridster.draggable;
            const {scrollSpeed} = gridster.draggable;

            const row = gridster.pixelsToRows(elmY);
            const col = gridster.pixelsToColumns(elmX);

            const itemsInTheWay = gridster.getItems(row, col, item.sizeX, item.sizeY, item);
            const hasItemsInTheWay = itemsInTheWay.length !== 0;

            if (gridster.swapping === true && hasItemsInTheWay) {
              const boundingBoxItem = gridster.getBoundingBox(itemsInTheWay);
              const sameSize = boundingBoxItem.sizeX === item.sizeX && boundingBoxItem.sizeY === item.sizeY;
              const sameRow = boundingBoxItem.row === row;
              const sameCol = boundingBoxItem.col === col;
              const samePosition = sameRow && sameCol;
              const inline = sameRow || sameCol;
              ...
              if (sameSize && itemsInTheWay.length === 1) {
                ...
                const emptyRow = item.row <= row ? item.row : row + item.sizeY;
                const emptyCol = item.col <= col ? item.col : col + item.sizeX;
                const rowOffset = emptyRow - boundingBoxItem.row;
                const colOffset = emptyCol - boundingBoxItem.col;

                for (let i = 0, l = itemsInTheWay.length; i < l; ++i) {
                  const itemInTheWay = itemsInTheWay[i];

                  const itemsInFreeSpace = gridster.getItems(
      ...
      function() {
        function GridsterResizable($el, scope, gridster, item, itemOptions) {
          ...
          function ResizeHandle(handleClass) {
            ...
            let originalWidth; let originalHeight;
            let savedDraggable;
            ...
            function resize(e) {
              const oldRow = item.row;
              const oldCol = item.col;
              const oldSizeX = item.sizeX;
              const oldSizeY = item.sizeY;
              const hasCallback = gridster.resizable && gridster.resizable.resize;
              ...
              const isChanged = item.row !== oldRow || item.col !== oldCol || item.sizeX !== oldSizeX || item.sizeY !== oldSizeY;
          ...
          const handles = [];
          let handlesOpts = gridster.resizable.handles;
          if (typeof handlesOpts === 'string') {
            handlesOpts = gridster.resizable.handles.split(',');
          }
          let enabled = false;

          for (let c = 0, l = handlesOpts.length; c < l; c++) {
          ...
          this.enable = function() {
            ...
            for (let c = 0, l = handles.length; c < l; c++) {
              handles[c].enable();
            }
            enabled = true;
          };
      ...
      function($parse, GridsterDraggable, GridsterResizable) {
        return {
          ...
          link(scope, $el, attrs, controllers) {
            const optionsKey = attrs.gridsterItem;
            let options;

            const gridster = controllers[0];
            const item = controllers[1];

            // bind the item's position properties
            if (optionsKey) {
              const $optionsGetter = $parse(optionsKey);
            ...
            const aspects = ['minSizeX', 'maxSizeX', 'minSizeY', 'maxSizeY', 'sizeX', 'sizeY', 'row', 'col'];
            const $getters = {};

            const aspectFn = function(aspect) {
              let key;
              ...
              // initial set
              const val = $getters[aspect](scope);
              if (typeof val === 'number') {
                item[aspect] = val;
              }
            };

            for (let i = 0, l = aspects.length; i < l; ++i) {
              aspectFn(aspects[i]);
            }
            ...
            scope.$watch(() => `${item.row  },${  item.col}`, positionChanged);

            function sizeChanged() {
              const changedX = item.setSizeX(item.sizeX, true);
              if (changedX && $getters.sizeX && $getters.sizeX.assign) {
                $getters.sizeX.assign(scope, item.sizeX);
              }
              const changedY = item.setSizeY(item.sizeY, true);
            ...
            scope.$watch(() => `${item.sizeY  },${  item.sizeX  }|${  item.minSizeX  },${  item.maxSizeX  },${  item.minSizeY  },${  item.maxSizeY}`, sizeChanged);

            const draggable = new GridsterDraggable($el, scope, gridster, item, options);
            const resizable = new GridsterResizable($el, scope, gridster, item, options);

            scope.$on('gridster-draggable-changed', () => {
              draggable.toggle(!gridster.isMobile && gridster.draggable && gridster.draggable.enabled);
            });
            scope.$on('gridster-resizable-changed', () => {
              resizable.toggle(!gridster.isMobile && gridster.resizable && gridster.resizable.enabled);
            });
            scope.$on('gridster-resized', () => {
              resizable.toggle(!gridster.isMobile && gridster.resizable && gridster.resizable.enabled);
            });
            scope.$watch(() => gridster.isMobile, () => {
              resizable.toggle(!gridster.isMobile && gridster.resizable && gridster.resizable.enabled);
              draggable.toggle(!gridster.isMobile && gridster.draggable && gridster.draggable.enabled);
            });
