...
function make_xlsx_lib(XLSX){
  ...
  const set_cp = function(cp) { current_codepage = cp; set_ansi(cp); };
  ...
  const _getchar = function _gc1(x) { return String.fromCharCode(x); };
  const _getansi = function _ga1(x) { return String.fromCharCode(x); };
  const DENSE = null;
  const DIF_XL = true;
...
*/
  ...
  function safegetzipfile(zip, file) {
    ...
    for(let i=0; i<k.length; ++i) {
      const n = k[i].replace(/^Root Entry[\/]/,"").toLowerCase();
  ...
  function zipentries(zip) {
    ...
    for(let i = 0; i < k.length; ++i) if(k[i].slice(-1) != '/') o.push(k[i].replace(/^Root Entry[\/]/, ""));
    return o.sort();
  }
...
*/
  ...
  const fix_read_opts = fix_opts_func([
  ...
  const fix_write_opts = fix_opts_func([
