...
function make_xlsx_lib(XLSX){
  XLSX.version = '0.15.6';
  let current_codepage = 1200; let current_ansi = 1252;
  ...
  const set_ansi = function(cp) { if(VALID_ANSI.indexOf(cp) == -1) return; current_ansi = CS2CP[0] = cp; };
  function reset_ansi() { set_ansi(1252); }

  let set_cp = function(cp) { current_codepage = cp; set_ansi(cp); };
  function reset_cp() { set_cp(1200); reset_ansi(); }

  function char_codes(data) { const o = []; for(let i = 0, len = data.length; i < len; ++i) o[i] = data.charCodeAt(i); return o; }

  function utf16leread(data) {
    const o = [];
    for(let i = 0; i < (data.length>>1); ++i) o[i] = String.fromCharCode(data.charCodeAt(2*i) + (data.charCodeAt(2*i+1)<<8));
    return o.join("");
  }
  function utf16beread(data) {
    const o = [];
    for(let i = 0; i < (data.length>>1); ++i) o[i] = String.fromCharCode(data.charCodeAt(2*i+1) + (data.charCodeAt(2*i)<<8));
    return o.join("");
  }

  let debom = function(data) {
    const c1 = data.charCodeAt(0); const c2 = data.charCodeAt(1);
  ...
  let _getchar = function _gc1(x) { return String.fromCharCode(x); };
  let _getansi = function _ga1(x) { return String.fromCharCode(x); };
  if(typeof cptable !== 'undefined') {
  ...
  const Base64 = (function make_b64(){
    ...
    return {
      encode(input) {
        let o = "";
        let c1=0; let c2=0; let c3=0; let e1=0; let e2=0; let e3=0; let e4=0;
  ...
  function s2ab(s) {
    ...
    const buf = new ArrayBuffer(s.length); const view = new Uint8Array(buf);
    for (let i=0; i!=s.length; ++i) view[i] = s.charCodeAt(i) & 0xFF;
    return buf;
  }

  function a2s(data) {
    if(Array.isArray(data)) return data.map((c) => String.fromCharCode(c)).join("");
    const o = []; for(let i = 0; i < data.length; ++i) o[i] = String.fromCharCode(data[i]); return o.join("");
  }
  ...
  let bconcat = function(bufs) { return [].concat.apply([], bufs); };

  const chr0 = /\u0000/g; const chr1 = /[\u0001-\u0006]/g;
  /* ssf.js (C) 2013-present SheetJS -- http://sheetjs.com */
  /* jshint -W041 */
  const SSF = ({});
  const make_ssf = function make_ssf(SSF){
    SSF.version = '0.10.3';
    function _strrev(x) { let o = ""; let i = x.length-1; while(i>=0) o += x.charAt(i--); return o; }
    function fill(c,l) { let o = ""; while(o.length < l) o+=c; return o; }
    function pad0(v,d){const t=`${v}`; return t.length>=d?t:fill('0',d-t.length)+t;}
    function pad_(v,d){const t=`${v}`;return t.length>=d?t:fill(' ',d-t.length)+t;}
    function rpad_(v,d){const t=`${v}`; return t.length>=d?t:t+fill(' ',d-t.length);}
    function pad0r1(v,d){const t=`${Math.round(v)}`; return t.length>=d?t:fill('0',d-t.length)+t;}
    function pad0r2(v,d){const t=`${v}`; return t.length>=d?t:fill('0',d-t.length)+t;}
    const p2_32 = Math.pow(2,32);
    function pad0r(v,d){if(v>p2_32||v<-p2_32) return pad0r1(v,d); const i = Math.round(v); return pad0r2(i,d); }
    ...
    function frac(x, D, mixed) {
      const sgn = x < 0 ? -1 : 1;
      let B = x * sgn;
      let P_2 = 0; let P_1 = 1; let P = 0;
      let Q_2 = 1; let Q_1 = 0; let Q = 0;
      let A = Math.floor(B);
      while(Q_1 < D) {
        A = Math.floor(B);
        P = A * P_1 + P_2;
    ...
    function parse_date_code(v,opts,b2) {
      ...
      let date = (v|0); let time = Math.floor(86400 * (v - date)); let dow=0;
      let dout=[];
    ...
    const general_fmt_num = (function make_general_fmt_num() {
      const gnr1 = /\.(\d*[1-9])0+$/; const gnr2 = /\.0*$/; const gnr4 = /\.(\d*[1-9])0+/; const gnr5 = /\.0*[Ee]/; const gnr6 = /(E[+-])(\d)$/;
      function gfn2(v) {
        const w = (v<0?12:11);
        let o = gfn5(v.toFixed(12)); if(o.length <= w) return o;
      ...
      return function general_fmt_num(v) {
        const V = Math.floor(Math.log(Math.abs(v))*Math.LOG10E); let o;
    ...
    function write_date(type, fmt, val, ss0) {
      let o=""; let ss=0; let tt=0; let {y} = val; let out; let outl = 0;
      switch(type) {
      ...
      if(outl > 0) return pad0(out, outl); return "";
    }
    /* jshint +W086 */
    function commaify(s) {
      const w = 3;
      if(s.length <= w) return s;
      let j = (s.length % w); let o = s.substr(0,j);
    ...
    var write_num = (function make_write_num(){
      ...
      function write_num_pct(type, fmt, val){
        const sfmt = fmt.replace(pct1,""); const mul = fmt.length - sfmt.length;
      ...
      function write_num_exp(fmt, val){
        ...
        if(fmt.match(/^#+0.0E\+0$/)) {
          ...
          let period = fmt.indexOf("."); if(period === -1) period=fmt.indexOf('E');
      ...
      function write_num_f1(r, aval, sign) {
        const den = parseInt(r[4],10); const rr = Math.round(aval * den); const base = Math.floor(rr/den);
        const myn = (rr - base*den); const myd = den;
      ...
      function rnd(val, d) { const dd = Math.pow(10,d); return `${Math.round(val * dd)/dd}`; }
      function dec(val, d) {
      ...
      function write_num_flt(type, fmt, val) {
        ...
        let o;
        let r; let ri; let ff; const aval = Math.abs(val); const sign = val < 0 ? "-" : "";
        ...
        if((r = fmt.match(/^([#0?]+)\.([#0]+)$/))) {
          ...
          ri = o.indexOf(".");
          const lres = fmt.indexOf(".") - ri; const rres = fmt.length - o.length - lres;
    ...
    function eval_fmt(fmt, v, opts, flen) {
      const out = []; let o = ""; let i = 0; let c = ""; let lst='t'; let dt; let j; let cc;
      let hr='H';
      /* Tokenize */
      while(i < fmt.length) {
        switch((c = fmt.charAt(i))) {
        ...
        case '\\': var w = fmt.charAt(++i); var t = (w === "(" || w === ")") ? w : 't';
          out[out.length] = {t, v:w}; ++i; break;
        case '_': out[out.length] = {t:'t', v:" "}; i+=2; break;
        case '@': /* Text Placeholder */
          out[out.length] = {t:'T', v}; ++i; break;
        case 'B': case 'b':
    ...
    function choose_fmt(f, v) {
      let fmt = split_fmt(f);
      let l = fmt.length; const lat = fmt[l-1].indexOf("@");
      if(l<4 && lat>-1) --l;
  ...
  function dateNF_fix(str, dateNF, match) {
    let Y = -1; let m = -1; let d = -1; let H = -1; let M = -1; let S = -1;
    (dateNF.match(dateNFregex)||[]).forEach((n, i) => {
    ...
    let datestr = (`${(`${  Y>=0?Y: new Date().getFullYear()}`).slice(-4)  }-${  (`00${  m>=1?m:1}`).slice(-2)  }-${  (`00${  d>=1?d:1}`).slice(-2)}`);
    if(datestr.length == 7) datestr = `0${  datestr}`;
    if(datestr.length == 8) datestr = `20${  datestr}`;
    const timestr = (`${(`00${  H>=0?H:0}`).slice(-2)  }:${  (`00${  M>=0?M:0}`).slice(-2)  }:${  (`00${  S>=0?S:0}`).slice(-2)}`);
  ...
  var CFB = (function _CFB(){
    ...
    function namecmp(l, r) {
      const L = l.split("/"); const R = r.split("/");
...
*/
    ...
    function parse_dos_date(buf) {
      ...
      const val = new Date();
      const d = ymd & 0x1F; ymd >>>= 5;
      const m = ymd & 0x0F; ymd >>>= 4;
      val.setMilliseconds(0);
      val.setFullYear(ymd + 1980);
      val.setMonth(m-1);
      val.setDate(d);
      const S = hms & 0x1F; hms >>>= 5;
      const M = hms & 0x3F; hms >>>= 6;
      val.setHours(hms);
      val.setMinutes(M);
      val.setSeconds(S<<1);
      return val;
    ...
    function parse_extra_field(blob) {
      ...
      while(blob.l <= blob.length - 4) {
        ...
        const sz = blob.read_shift(2); const tgt = blob.l + sz;
        const p = {};
        switch(type) {
    ...
    function parse(file, options) {
      ...
      for(let q = -1, j = 0; j < 109; ++j) { /* 109 = (512 - blob.l)>>>2; */
      ...
      const files = {}; const Paths = []; const FileIndex = []; const FullPaths = [];
    ...
    function sectorify(file, ssz) {
      ...
      const sectors = [];
      for(let i=1; i < nsectors; ++i) sectors[i-1] = file.slice(i*ssz,(i+1)*ssz);
    ...
    function build_full_paths(FI, FP, Paths) {
      let i = 0; let L = 0; let R = 0; let C = 0; let j = 0; const pl = Paths.length;
      const dad = []; const q = [];
    ...
    function get_mfat_entry(entry, payload, mini) {
      const {start} = entry; let {size} = entry;
    ...
    function sleuth_fat(idx, cnt, sectors, ssz, fat_addrs) {
      ...
      if(idx === ENDOFCHAIN) {
        ...
        const sector = sectors[idx]; const m = (ssz>>>2)-1;
        if(!sector) return;
        for(let i = 0; i < m; ++i) {
    ...
    function get_sector_list(sectors, start, fat_addrs, ssz, chkd) {
      const buf = []; const buf_chain = [];
      if(!chkd) chkd = [];
      const modulus = ssz - 1; let j = 0; let jj = 0;
      for(j=start; j>=0;) {
        chkd[j] = true;
        buf[buf.length] = j;
    ...
    function make_sector_list(sectors, dir_start, fat_addrs, ssz) {
      const sl = sectors.length; const sector_list = ([]);
      const chkd = []; let buf = []; let buf_chain = [];
      const modulus = ssz - 1; let i=0; let j=0; let k=0; let jj=0;
      for(i=0; i < sl; ++i) {
        buf = ([]);
    ...
    function read_directory(dir_start, sector_list, sectors, Paths, nmfs, files, FileIndex, mini) {
      let minifat_store = 0; const pl = (Paths.length?2:0);
      const sector = sector_list[dir_start].data;
      let i = 0; let namelen = 0; let name;
      for(; i < sector.length; i+= 128) {
        ...
        const ctime = blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2);
        if(ctime !== 0) o.ct = read_date(blob, blob.l-8);
        const mtime = blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2) + blob.read_shift(2);
    ...
    function init_cfb(cfb, opts) {
      const o = opts || {}; const root = o.root || "Root Entry";
    ...
    function seed_cfb(cfb) {
      ...
      const p = new_buf(4); p[0] = 55; p[1] = p[3] = 50; p[2] = 54;
    ...
    function rebuild_cfb(cfb, f) {
      ...
      const now = new Date(1987, 1, 19); let j = 0;
      const data = [];
    ...
    function _write(cfb, options) {
      ...
      const L = (function(cfb){
        let mini_size = 0; let fat_size = 0;
        ...
        const L =  [1, difat_cnt, fat_cnt, mfat_cnt, dir_cnt, fat_size, mini_size, 0];
    ...
    function find(cfb, path) {
      const UCFullPaths = cfb.FullPaths.map((x) => x.toUpperCase());
      const UCPaths = UCFullPaths.map((x) => { const y = x.split("/"); return y[y.length - (x.slice(-1) == "/" ? 2 : 1)]; });
      let k = false;
      if(path.charCodeAt(0) === 47 /* "/" */) { k = true; path = UCFullPaths[0].slice(0, -1) + path; }
      else k = path.indexOf("/") !== -1;
      let UCPath = path.toUpperCase();
      let w = k === true ? UCFullPaths.indexOf(UCPath) : UCPaths.indexOf(UCPath);
    ...
    function _inflateRawSync(payload, usz) {
      ...
      const InflRaw = new InflateRaw();
      const out = InflRaw._processChunk(payload.slice(payload.l), InflRaw._finishFlushFlag);
    ...
    function bit_swap_8(n) { const t = (((((n<<1)|(n<<11)) & 0x22110) | (((n<<5)|(n<<15)) & 0x88440))); return ((t>>16) | (t>>8) |t)&0xFF; }
    ...
    function read_bits_2(buf, bl) { const w = (bl&7); const h = (bl>>>3); return ((buf[h]|(w <= 6 ? 0 : buf[h+1]<<8))>>>w)& 0x03; }
    function read_bits_3(buf, bl) { const w = (bl&7); const h = (bl>>>3); return ((buf[h]|(w <= 5 ? 0 : buf[h+1]<<8))>>>w)& 0x07; }
    function read_bits_4(buf, bl) { const w = (bl&7); const h = (bl>>>3); return ((buf[h]|(w <= 4 ? 0 : buf[h+1]<<8))>>>w)& 0x0F; }
    function read_bits_5(buf, bl) { const w = (bl&7); const h = (bl>>>3); return ((buf[h]|(w <= 3 ? 0 : buf[h+1]<<8))>>>w)& 0x1F; }
    function read_bits_7(buf, bl) { const w = (bl&7); const h = (bl>>>3); return ((buf[h]|(w <= 1 ? 0 : buf[h+1]<<8))>>>w)& 0x7F; }

    /* works up to n = 3 * 8 + 1 = 25 */
    function read_bits_n(buf, bl, n) {
      const w = (bl&7); const h = (bl>>>3); const f = ((1<<n)-1);
      let v = buf[h] >>> w;
      if(n < 8 - w) return v & f;
      v |= buf[h+1]<<(8-w);
    ...
    function realloc(b, sz) {
      const L = b.length; const M = 2*L > sz ? 2*L : sz + 5; let i = 0;
      if(L >= sz) return b;
      if(has_buf) {
    ...
    function build_tree(clens, cmap, MAX) {
      let maxlen = 1; let w = 0; let i = 0; let j = 0; let ccode = 0; let L = clens.length;

      const bl_count  = use_typed_arrays ? new Uint16Array(32) : zero_fill_array(32);
    ...
    const fix_lmap = use_typed_arrays ? new Uint16Array(512) : zero_fill_array(512);
    const fix_dmap = use_typed_arrays ? new Uint16Array(32)  : zero_fill_array(32);
    if(!use_typed_arrays) {
    ...
    const dyn_lmap = use_typed_arrays ? new Uint16Array(32768) : zero_fill_array(32768);
    const dyn_dmap = use_typed_arrays ? new Uint16Array(32768) : zero_fill_array(32768);
    const dyn_cmap = use_typed_arrays ? new Uint16Array(128)   : zero_fill_array(128);
    let dyn_len_1 = 1; let dyn_len_2 = 1;

    /* 5.5.3 Expanding Huffman Codes */
    function dyn(data, boff) {
      /* nomenclature from RFC1951 refers to bit values; these are offset by the implicit constant */
      const _HLIT = read_bits_5(data, boff) + 257; boff += 5;
      const _HDIST = read_bits_5(data, boff) + 1; boff += 5;
      const _HCLEN = read_bits_4(data, boff) + 4; boff += 4;
      let w = 0;
      ...
      let maxlen = 1;
      const bl_count =  use_typed_arrays ? new Uint8Array(8) : zero_fill_array(8);
      const next_code = use_typed_arrays ? new Uint8Array(8) : zero_fill_array(8);
      ...
      for(i = 0; i < L; ++i) {
        ...
        if(cleni != 0) {
          ...
          for(let j = (1<<(7-cleni))-1; j>=0; --j) dyn_cmap[ccode|(j<<cleni)] = (cleni&7) | (i<<3);
        }
      }
      ...
      const h1 = hcodes.slice(0, _HLIT); const h2 = hcodes.slice(_HLIT);
    ...
    function inflate(data, usz) {
      ...
      let max_len_1 = 0; let max_len_2 = 0;
      ...
      while((header&1) == 0) {
        ...
        for(;;) { // while(true) is apparently out of vogue in modern JS circles
          ...
          if(((code>>>8)&0xFF) === 0) outbuf[woff++] = code;
            ...
            code -= 257;
            let len_eb = (code < 8) ? 0 : ((code-4)>>2); if(len_eb > 5) len_eb = 0;
    ...
    function parse_zip(file, options) {
      ...
      const FileIndex = []; const FullPaths = [];
      const o = {
        FileIndex,
        FullPaths
      };
      ...
      for(i = 0; i < fcnt; ++i) {
        ...
        const EF = parse_extra_field(blob.slice(blob.l+namelen, blob.l+namelen+efsz));
    ...
    function parse_local_file(blob, csz, usz, o, EF) {
      ...
      let name = ""; for(let i = 0; i < namelen; ++i) name += String.fromCharCode(blob[blob.l++]);
      if(efsz) {
    ...
    function write_zip(cfb, options) {
      const _opts = options || {};
      const out = []; const cdirs = [];
      let o = new_buf(1);
      const method = (_opts.compression ? 8 : 0); let flags = 0;
      const desc = false;
      if(desc) flags |= 8;
      let i = 0; let j = 0;

      let start_cd = 0; let fcnt = 0;
      const root = cfb.FullPaths[0]; let fp = root; let fi = cfb.FileIndex[0];
      const crcs = [];
      let sz_cd = 0;
    ...
    function cfb_del(cfb, name) {
      init_cfb(cfb);
      const file = CFB.find(cfb, name);
      if(file) for(let j = 0; j < cfb.FileIndex.length; ++j) if(cfb.FileIndex[j] == file) {
    ...
    function cfb_mov(cfb, old_name, new_name) {
      init_cfb(cfb);
      const file = CFB.find(cfb, old_name);
      if(file) for(let j = 0; j < cfb.FileIndex.length; ++j) if(cfb.FileIndex[j] == file) {
  ...
  function write_dl(fname, payload, enc) {
    ...
    const data = (enc == "utf8") ? utf8write(payload) : payload;
    if(typeof IE_SaveFile !== 'undefined') return IE_SaveFile(data, fname);
    if(typeof Blob !== 'undefined') {
      const blob = new Blob([blobify(data)], {type:"application/octet-stream"});
      if(typeof navigator !== 'undefined' && navigator.msSaveBlob) return navigator.msSaveBlob(blob, fname);
      if(typeof saveAs !== 'undefined') return saveAs(blob, fname);
      if(typeof URL !== 'undefined' && typeof document !== 'undefined' && document.createElement && URL.createObjectURL) {
        const url = URL.createObjectURL(blob);
        if(typeof chrome === 'object' && typeof (chrome.downloads||{}).download === "function") {
          if(URL.revokeObjectURL && typeof setTimeout !== 'undefined') setTimeout(() => { URL.revokeObjectURL(url); }, 60000);
    ...
    if(typeof $ !== 'undefined' && typeof File !== 'undefined' && typeof Folder !== 'undefined') try { // extendscript
      // $FlowIgnore
      const out = File(fname); out.open("w"); out.encoding = "binary";
  ...
  function read_binary(path) {
    ...
    if(typeof $ !== 'undefined' && typeof File !== 'undefined' && typeof Folder !== 'undefined') try { // extendscript
      // $FlowIgnore
      const infile = File(path); infile.open("r"); infile.encoding = "binary";
      const data = infile.read(); infile.close();
      return data;
    } catch(e) { if(!e.message || !e.message.match(/onstruct/)) throw e; }
    throw new Error(`Cannot access file ${  path}`);
  }
  function keys(o) {
    const ks = Object.keys(o); const o2 = [];
    for(let i = 0; i < ks.length; ++i) if(Object.prototype.hasOwnProperty.call(o, ks[i])) o2.push(ks[i]);
    return o2;
  }

  function evert_key(obj, key) {
    const o = ([]); const K = keys(obj);
    for(let i = 0; i !== K.length; ++i) if(o[obj[K[i]][key]] == null) o[obj[K[i]][key]] = K[i];
    return o;
  }
  ...
  function dup(o) {
    ...
    const out = {};
    for(const k in o) if(Object.prototype.hasOwnProperty.call(o, k)) out[k] = dup(o[k]);
    return out;
  }
  ...
  function fuzzydate(s) {
    const o = new Date(s); const n = new Date(NaN);
    const y = o.getYear(); const m = o.getMonth(); const d = o.getDate();
    if(isNaN(d)) return n;
  ...
  function split_regex(str, re, def) {
    ...
    const p = str.split(re); const o = [p[0]];
  ...
  function safegetzipfile(zip, file) {
    ...
    const f = file.toLowerCase(); const g = f.replace(/\//g,'\\');
    for(let i=0; i<k.length; ++i) {
  ...
  function zipentries(zip) {
    const k = zip.FullPaths || keys(zip.files); const o = [];
  ...
  let tagregex=/<[\/\?]?[a-zA-Z0-9:_-]+(?:\s+[^"\s?>\/]+\s*=\s*(?:"[^"]*"|'[^']*'|[^'">\s=]+))*\s?[\/\?]?>/g;
  ...
  function parsexmltag(tag, skip_root, skip_LC) {
    ...
    const m = tag.match(attregexg); let j=0; let v=""; let i=0; let q=""; let cc=""; let quot = 1;
  ...
  const unescapexml = (function() {
    ...
    const encregex = /&(?:quot|apos|gt|lt|amp|#x?([\da-fA-F]+));/ig; const coderegex = /_x([\da-fA-F]{4})_/ig;
    return function unescapexml(text) {
      const s = `${text  }`; const i = s.indexOf("<![CDATA[");
      if(i == -1) return s.replace(encregex, ($$, $1) => encodings[$$]||String.fromCharCode(parseInt($1,$$.indexOf("x")>-1?16:10))||$$).replace(coderegex,(m,c) => String.fromCharCode(parseInt(c,16)));
      const j = s.indexOf("]]>");
  ...
  function escapexml(text){
    const s = `${text  }`;
    return s.replace(decregex, (y) => rencoding[y]).replace(charegex,(s) => `_x${  (`000${s.charCodeAt(0).toString(16)}`).slice(-4)  }_`);
  }
  ...
  function escapehtml(text){
    const s = `${text  }`;
    return s.replace(decregex, (y) => rencoding[y]).replace(/\n/g, "<br/>").replace(htmlcharegex,(s) => `&#x${  (`000${s.charCodeAt(0).toString(16)}`).slice(-4)  };`);
  }

  function escapexlml(text){
    const s = `${text  }`;
    return s.replace(decregex, (y) => rencoding[y]).replace(htmlcharegex,(s) => `&#x${  (s.charCodeAt(0).toString(16)).toUpperCase()  };`);
  }
  ...
  var utf8write = function(orig) {
    const out = []; let i = 0; let c = 0; let d = 0;
    while(i < orig.length) {
      c = orig.charCodeAt(i++);
      switch(true) {
  ...
  if(has_buf) {
    const utf8readb = function utf8readb(data) {
      const out = Buffer.alloc(2*data.length); let w; let i; let j = 1; let k = 0; let ww=0; let c;
    ...
    const utf8readc = function utf8readc(data) { return Buffer_from(data, 'binary').toString('utf8'); };
  ...
  const vtregex = (function(){ const vt_cache = {};
    return function vt_regex(bt) {
      if(vt_cache[bt] !== undefined) return vt_cache[bt];
      return (vt_cache[bt] = new RegExp(`<(?:vt:)?${  bt  }>([\\s\\S]*?)</(?:vt:)?${  bt  }>`, 'g') );
    };})();
  const vtvregex = /<\/?(?:vt:)?variant>/g; const vtmregex = /<(?:vt:)([^>]*)>([\s\S]*)</;
  ...
  function wxt_helper(h) { return keys(h).map((k) => ` ${  k  }="${  h[k]  }"`).join(""); }
  ...
  function write_double_le(b, v, idx) {
    const bs = ((((v < 0) || (1/v == -Infinity)) ? 1 : 0) << 7); let e = 0; let m = 0;
    const av = bs ? (-v) : v;
  ...
  var __toBuffer = function(bufs) { const x=[]; const w=10240; for(let i=0;i<bufs[0].length;++i) if(bufs[0][i]) for(let j=0,L=bufs[0][i].length;j<L;j+=w) x.push.apply(x, bufs[0][i].slice(j,j+w)); return x; };
  const ___toBuffer = __toBuffer;
  var __utf16le = function(b,s,e) { const ss=[]; for(let i=s; i<e; i+=2) ss.push(String.fromCharCode(__readUInt16LE(b,i))); return ss.join("").replace(chr0,''); };
  const ___utf16le = __utf16le;
  let __hexlify = function(b,s,l) { const ss=[]; for(let i=s; i<s+l; ++i) ss.push((`0${  b[i].toString(16)}`).slice(-2)); return ss.join(""); };
  const ___hexlify = __hexlify;
  let __utf8 = function(b,s,e) { const ss=[]; for(let i=s; i<e; i++) ss.push(String.fromCharCode(__readUInt8(b,i))); return ss.join(""); };
  const ___utf8 = __utf8;
  let __lpstr = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? __utf8(b, i+4,i+4+len-1) : "";};
  const ___lpstr = __lpstr;
  let __cpstr = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? __utf8(b, i+4,i+4+len-1) : "";};
  const ___cpstr = __cpstr;
  let __lpwstr = function(b,i) { const len = 2*__readUInt32LE(b,i); return len > 0 ? __utf8(b, i+4,i+4+len-1) : "";};
  const ___lpwstr = __lpwstr;
  let __lpp4; let ___lpp4;
  __lpp4 = ___lpp4 = function lpp4_(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? __utf16le(b, i+4,i+4+len) : "";};
  let __8lpp4 = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? __utf8(b, i+4,i+4+len) : "";};
  const ___8lpp4 = __8lpp4;
  let __double; let ___double;
  __double = ___double = function(b, idx) { return read_double_le(b, idx);};
  let is_buf = function is_buf_a(a) { return Array.isArray(a); };
  ...
  if(has_buf) {
    ...
    __lpstr = function lpstr_b(b, i) { if(!Buffer.isBuffer(b)) return ___lpstr(b, i); const len = b.readUInt32LE(i); return len > 0 ? b.toString('utf8',i+4,i+4+len-1) : "";};
    __cpstr = function cpstr_b(b, i) { if(!Buffer.isBuffer(b)) return ___cpstr(b, i); const len = b.readUInt32LE(i); return len > 0 ? b.toString('utf8',i+4,i+4+len-1) : "";};
    __lpwstr = function lpwstr_b(b, i) { if(!Buffer.isBuffer(b)) return ___lpwstr(b, i); const len = 2*b.readUInt32LE(i); return b.toString('utf16le',i+4,i+4+len-1);};
    __lpp4 = function lpp4_b(b, i) { if(!Buffer.isBuffer(b)) return ___lpp4(b, i); const len = b.readUInt32LE(i); return b.toString('utf16le',i+4,i+4+len);};
    __8lpp4 = function lpp4_8b(b, i) { if(!Buffer.isBuffer(b)) return ___8lpp4(b, i); const len = b.readUInt32LE(i); return b.toString('utf8',i+4,i+4+len);};
  ...
  if(typeof cptable !== 'undefined') {
    ...
    __lpstr = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? cptable.utils.decode(current_ansi, b.slice(i+4, i+4+len-1)) : "";};
    __cpstr = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? cptable.utils.decode(current_codepage, b.slice(i+4, i+4+len-1)) : "";};
    __lpwstr = function(b,i) { const len = 2*__readUInt32LE(b,i); return len > 0 ? cptable.utils.decode(1200, b.slice(i+4,i+4+len-1)) : "";};
    __lpp4 = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? cptable.utils.decode(1200, b.slice(i+4,i+4+len)) : "";};
    __8lpp4 = function(b,i) { const len = __readUInt32LE(b,i); return len > 0 ? cptable.utils.decode(65001, b.slice(i+4,i+4+len)) : "";};
  }

  var __readUInt8 = function(b, idx) { return b[idx]; };
  var __readUInt16LE = function(b, idx) { return (b[idx+1]*(1<<8))+b[idx]; };
  const __readInt16LE = function(b, idx) { const u = (b[idx+1]*(1<<8))+b[idx]; return (u < 0x8000) ? u : ((0xffff - u + 1) * -1); };
  var __readUInt32LE = function(b, idx) { return b[idx+3]*(1<<24)+(b[idx+2]<<16)+(b[idx+1]<<8)+b[idx]; };
  var __readInt32LE = function(b, idx) { return (b[idx+3]<<24)|(b[idx+2]<<16)|(b[idx+1]<<8)|b[idx]; };
  const __readInt32BE = function(b, idx) { return (b[idx]<<24)|(b[idx+1]<<16)|(b[idx+2]<<8)|b[idx+3]; };

  function ReadShift(size, t) {
    let o=""; let oI; let oR; const oo=[]; let w; let vv; let i; let loc;
    switch(t) {
    case 'dbcs':
      loc = this.l;
    ...
    default:
      ...
      case 4: case -4:
        ...
        oR = __readUInt32LE(this, this.l); this.l += 4;  return oR;
      case 8: case -8:
        if(t === 'f') {
  ...
  const __writeUInt32LE = function(b, val, idx) { b[idx] = (val & 0xFF); b[idx+1] = ((val >>> 8) & 0xFF); b[idx+2] = ((val >>> 16) & 0xFF); b[idx+3] = ((val >>> 24) & 0xFF); };
  const __writeInt32LE  = function(b, val, idx) { b[idx] = (val & 0xFF); b[idx+1] = ((val >> 8) & 0xFF); b[idx+2] = ((val >> 16) & 0xFF); b[idx+3] = ((val >> 24) & 0xFF); };
  const __writeUInt16LE = function(b, val, idx) { b[idx] = (val & 0xFF); b[idx+1] = ((val >>> 8) & 0xFF); };
  ...
  function recordhopper(data, cb, opts) {
    if(!data) return;
    let tmpbyte; let cntbyte; let length;
    prep_blob(data, data.l || 0);
    const L = data.length; let RT = 0; let tgt = 0;
    while(data.l < L) {
  ...
  function buf_array() {
    const bufs = []; const blksz = has_buf ? 256 : 2048;
    ...
    const push = function ba_push(buf) { endbuf(); curbuf = buf; if(curbuf.l == null) curbuf.l = curbuf.length; next(blksz); };

    return ({ next, push, end, _bufs:bufs });
  }

  function write_record(ba, type, payload, length) {
    const t = +XLSBRE[type]; let l;
  ...
  const make_offcrypto = function(O, _crypto) {
    ...
    O.rc4 = function(key, data) {
      ...
      let c = 0; let i = 0; let j = 0; let t = 0;
      ...
      // $FlowIgnore
      i = j = 0; const out = Buffer(data.length);
  ...
  function decode_col(colstr) { const c = unfix_col(colstr); let d = 0; let i = 0; for(; i !== c.length; ++i) d = 26*d + c.charCodeAt(i) - 64; return d - 1; }
  function encode_col(col) { if(col < 0) throw new Error(`invalid column ${  col}`); let s=""; for(++col; col; col=Math.floor((col-1)/26)) s = String.fromCharCode(((col-1)%26) + 65) + s; return s; }
  function fix_col(cstr) { return cstr.replace(/^([A-Z])/,"$$$1"); }
  function unfix_col(cstr) { return cstr.replace(/^\$([A-Z])/,"$1"); }

  function split_cell(cstr) { return cstr.replace(/(\$?[A-Z]*)(\$?\d*)/,"$1,$2").split(","); }
  function decode_cell(cstr) { const splt = split_cell(cstr); return { c:decode_col(splt[0]), r:decode_row(splt[1]) }; }
  function encode_cell(cell) { return encode_col(cell.c) + encode_row(cell.r); }
  function decode_range(range) { const x =range.split(":").map(decode_cell); return {s:x[0],e:x[x.length-1]}; }
  ...
  function write_XLWideString(data, o) {
    let _null = false; if(o == null) { _null = true; o = new_buf(4+2*data.length); }
  ...
  function parse_RichStr(data, length) {
    ...
    if((flags & 1) !== 0) { /* fRichStr */
      ...
      for(let i = 0; i != dwSizeStrRun; ++i) rgsStrRun.push(parse_StrRun(data));
      z.r = rgsStrRun;
    }
  ...
  function write_RichStr(str, o) {
    /* TODO: formatted string */
    let _null = false; if(o == null) { _null = true; o = new_buf(15+4*str.t.length); }
    o.write_shift(1,0);
  ...
  function write_BrtCommentText(str, o) {
    /* TODO: formatted string */
    let _null = false; if(o == null) { _null = true; o = new_buf(23+4*str.t.length); }
    o.write_shift(1,1);
    write_XLWideString(str.t, o);
    o.write_shift(4,1);
  ...
  const write_XLSBCodeName = write_XLWideString;
  ...
  function write_XLNullableWideString(data, o) {
    let _null = false; if(o == null) { _null = true; o = new_buf(127); }
  ...
  function parse_RkNumber(data) {
    ...
    const fX100 = (b[0] & 1); const fInt = (b[0] & 2);
    data.l+=4;
    b[0] &= 0xFC; // b[0] &= ~3;
  ...
  function write_RkNumber(data, o) {
    ...
    let fX100 = 0; let fInt = 0; const d100 = data * 100;
  ...
  function parse_ClipboardFormatOrString(o, w) {
    // $FlowIgnore
    const ClipFmt = {2:"BITMAP",3:"METAFILEPICT",8:"DIB",14:"ENHMETAFILE"};
  ...
  (function() {
    for(const y in SpecialProperties) if(Object.prototype.hasOwnProperty.call(SpecialProperties, y))
      DocSummaryPIDDSI[y] = SummaryPIDSI[y] = SpecialProperties[y];
  })();

  const DocSummaryRE = evert_key(DocSummaryPIDDSI, "n");
  const SummaryRE = evert_key(SummaryPIDSI, "n");
  ...
  function rgbify(arr) { return arr.map((x) => [(x>>16)&255,(x>>8)&255,x&255]); }
  ...
  const CT_LIST = (function(){
    ...
    keys(o).forEach((k) => { ["xlsm", "xlam"].forEach((v) => { if(!o[k][v]) o[k][v] = o[k].xlsx; }); });
    keys(o).forEach((k)=> { keys(o[k]).forEach((v) => { ct2type[o[k][v]] = k; }); });
    return o;
  })();

  const type2ct/* {[string]:Array<string>} */ = evert_arr(ct2type);
  ...
  const CTYPE_XML_ROOT = writextag('Types', null, {
    'xmlns': XMLNS.CT,
    'xmlns:xsd': XMLNS.xsd,
    'xmlns:xsi': XMLNS.xsi
  });
  ...
  const CTYPE_DEFAULTS = [
    ...
    ['pdf', 'application/pdf'],
    ['rels', type2ct.rels[0]]
  ].map((x) => writextag('Default', null, {'Extension':x[0], 'ContentType': x[1]}));
  ...
  function parse_rels(data, currentFilePath) {
    ...
    (data.match(tagregex)||[]).forEach((x) => {
      ...
      if (y[0] === '<Relationship') {
        const rel = {}; rel.Type = y.Type; rel.Target = y.Target; rel.Id = y.Id; rel.TargetMode = y.TargetMode;
  ...
  const RELS_EXTERN = [RELS.HLINK, RELS.XPATH, RELS.XMISS];
  ...
  function write_manifest(manifest) {
    ...
    for(let i = 0; i < manifest.length; ++i) o.push(`  <manifest:file-entry manifest:full-path="${  manifest[i][0]  }" manifest:media-type="${  manifest[i][1]  }"/>\n`);
  ...
  const CORE_PROPS_REGEX = (function() {
    ...
    for(let i = 0; i < CORE_PROPS.length; ++i) {
      ...
      const g = `(?:${ f[0].slice(0,f[0].indexOf(":")) }:)${ f[0].slice(f[0].indexOf(":")+1)}`;
  ...
  function parse_core_props(data) {
    ...
    for(let i = 0; i < CORE_PROPS.length; ++i) {
      const f = CORE_PROPS[i]; const cur = data.match(CORE_PROPS_REGEX[i]);
  ...
  function write_core_props(cp, _opts) {
    const opts = _opts || {};
    const o = [XML_HEADER, CORE_PROPS_XML_ROOT]; const p = {};
    ...
    for(let i = 0; i != CORE_PROPS.length; ++i) {
      const f = CORE_PROPS[i];
      let v = opts.Props && opts.Props[f[1]] != null ? opts.Props[f[1]] : cp ? cp[f[1]] : null;
      if(v === true) v = "1";
      else if(v === false) v = "0";
  ...
  function load_props_pairs(HP, TOP, props, opts) {
    ...
    else for(let j = 0; j < HP.length; ++j) v = v.concat(HP[j].map((hp) => ({v:hp})));
    const parts = (typeof TOP === "string") ? parseVector(TOP, opts).map((x) => x.v) : TOP;
    let idx = 0; let len = 0;
    if(parts.length > 0) for(let i = 0; i !== v.length; i += 2) {
      len = +(v[i+1].v);
      switch(v[i].v) {
      case "Worksheets":
      case "工作表":
  ...
  function write_ext_props(cp) {
    ...
    o[o.length] = (W('TitlesOfParts', W('vt:vector', cp.SheetNames.map((s) => `<vt:lpstr>${  escapexml(s)  }</vt:lpstr>`).join(""), {size: cp.Worksheets, baseType:"lpstr"})));
  ...
  function parse_cust_props(data, opts) {
    ...
    if(m) for(let i = 0; i != m.length; ++i) {
      const x = m[i]; const y = parsexmltag(x);
      switch(y[0]) {
      case '<?xml': break;
      ...
      default: if (x.indexOf('<vt:') === 0) {
        const toks = x.split('>');
        const type = toks[0].slice(4); const text = toks[1];
  ...
  function xlml_write_custprops(Props, Custprops) {
    ...
    if(Custprops) keys(Custprops).forEach((k) => {
  ...
  function parse_FILETIME(blob) {
    const dwLowDateTime = blob.read_shift(4); const dwHighDateTime = blob.read_shift(4);
  ...
  function write_FILETIME(time) {
    ...
    let l = t % Math.pow(2,32); let h = (t - l) / Math.pow(2,32);
    l *= 1e7; h *= 1e7;
    const w = (l / Math.pow(2,32)) | 0;
    if(w > 0) { l %= Math.pow(2,32); h += w; }
    const o = new_buf(8); o.write_shift(4, l); o.write_shift(4, h); return o;
  }
  ...
  function parse_VtVecUnalignedLpstrValue(blob) {
    ...
    const ret = [];
    for(let i = 0; i != length; ++i) ret[i] = blob.read_shift(0, 'lpstr-cp').replace(chr0,'');
    return ret;
  }
  ...
  function parse_VtVecHeadingPairValue(blob) {
    ...
    const out = [];
    for(let i = 0; i != cElements / 2; ++i) out.push(parse_VtHeadingPair(blob));
    return out;
  }
  ...
  function parse_TypedPropertyValue(blob, type, _opts) {
    const t = blob.read_shift(2); let ret; const opts = _opts||{};
    blob.l += 2;
    if(type !== VT_VARIANT)
  ...
  function write_TypedPropertyValue(type, value) {
    const o = new_buf(4); let p = new_buf(4);
  ...
  const XLSPSSkip = [ "CodePage", "Thumbnail", "_PID_LINKBASE", "_PID_HLINKS", "SystemIdentifier", "FMTID" ].concat(PseudoPropsPairs);
  ...
  function write_PropertySet(entries, RE, PIDSI) {
    const hdr = new_buf(8); const piao = []; const prop = [];
    let sz = 8; let i = 0;

    let pr = new_buf(8); let pio = new_buf(8);
  ...
  function parse_PropertySetStream(file, PIDSI, clsid) {
    ...
    let NumSets; let FMTID0; let FMTID1; let Offset0; let Offset1 = 0;
  ...
  function write_PropertySetStream(entries, clsid, RE, PIDSI, entries2, clsid2) {
    const hdr = new_buf(entries2 ? 68 : 48);
    const bufs = [hdr];
    ...
    const ps0 = write_PropertySet(entries, RE, PIDSI);
    bufs.push(ps0);

    if(entries2) {
      const ps1 = write_PropertySet(entries2, null, null);
  ...
  function writezeroes(n, o) { if(!o) o=new_buf(n); for(let j=0; j<n; ++j) o.write_shift(1, 0); return o; }

  function parslurp(blob, length, cb) {
    const arr = []; const target = blob.l + length;
  ...
  function parse_Bes(blob) {
    const v = blob.read_shift(1); const t = blob.read_shift(1);
  ...
  function parse_XLUnicodeRichExtendedString(blob) {
    ...
    const cch = blob.read_shift(2); const flags = blob.read_shift(1);
    const /* fHighByte = flags & 0x1, */ fExtSt = flags & 0x4; const fRichSt = flags & 0x8;
    const width = 1 + (flags & 0x1); // 0x0 -> utf8, 0x1 -> dbcs
    let cRun = 0; let cbExtRst;
    const z = {};
  ...
  function parse_URLMoniker(blob) {
    const len = blob.read_shift(4); const start = blob.l;
    let extra = false;
    if(len > 24) {
      /* look ahead */
      blob.l += len - 24;
  ...
  function parse_HyperlinkMoniker(blob, length) {
    const clsid = blob.read_shift(16); length -= 16;
    switch(clsid) {
  ...
  function parse_Hyperlink(blob, length) {
    ...
    const flags = blob.read_shift(2);
    blob.l += 2;
    let displayName; let targetFrameName; let moniker; let oleMoniker; let Loc=""; let guid; let fileTime;
  ...
  function write_Hyperlink(hl) {
    const out = new_buf(512); let i = 0;
    let {Target} = hl;
    let F = Target.indexOf("#") > -1 ? 0x1f : 0x17;
    switch(Target.charAt(0)) { case "#": F=0x1c; break; case ".": F&=~2; break; }
    out.write_shift(4,2); out.write_shift(4, F);
    let data = [8,6815827,6619237,4849780,83]; for(i = 0; i < data.length; ++i) out.write_shift(4, data[i]);
    if(F == 0x1C) {
  ...
  /* 2.5.178 LongRGBA */
  function parse_LongRGBA(blob) { const r = blob.read_shift(1); const g = blob.read_shift(1); const b = blob.read_shift(1); const a = blob.read_shift(1); return [r,g,b,a]; }

  /* 2.5.177 LongRGB */
  function parse_LongRGB(blob, length) { const x = parse_LongRGBA(blob, length); x[3] = 0; return x; }
  ...
  function parse_XTI(blob, length, opts) {
    ...
    const iSupBook = blob.read_shift(w); const itabFirst = blob.read_shift(w,'i'); const itabLast = blob.read_shift(w,'i');
  ...
  function parse_WsBool(blob, length, opts) {
    const flags = opts && opts.biff == 8 || length == 2 ? blob.read_shift(2) : (blob.l += length, 0);
  ...
  function parse_Window1(blob) {
    const xWn = blob.read_shift(2); const yWn = blob.read_shift(2); const dxWn = blob.read_shift(2); const dyWn = blob.read_shift(2);
    const flags = blob.read_shift(2); const iTabCur = blob.read_shift(2); const iTabFirst = blob.read_shift(2);
    const ctabSel = blob.read_shift(2); const wTabRatio = blob.read_shift(2);
  ...
  function write_Window2(view) {
    const o = new_buf(18); let f = 0x6b6;
  ...
  function write_Font(data, opts) {
    ...
    const b5 = (opts && (opts.biff == 5)); const w = (b5 ? (15 + name.length) : (16 + 2 * name.length));
    const o = new_buf(w);
  ...
  function parse_Dimensions(blob, length, opts) {
    ...
    const r = blob.read_shift(w); const R = blob.read_shift(w);
    const c = blob.read_shift(2); const C = blob.read_shift(2);
    blob.l = end;
  ...
  function parse_RK(blob) {
    const rw = blob.read_shift(2); const col = blob.read_shift(2);
  ...
  function parse_CellStyleXF(blob, length, style, opts) {
    const o = {};
    const a = blob.read_shift(4); const b = blob.read_shift(4);
    const c = blob.read_shift(4); const d = blob.read_shift(2);
  ...
  function parse_Lbl(blob, length, opts) {
    ...
    let npflen = target - blob.l; if(opts && opts.biff == 2) --npflen;
  ...
  function parse_ExternSheet(blob, length, opts) {
    ...
    const o = []; const target = blob.l + length; let len = blob.read_shift(opts.biff > 8 ? 4 : 2);
  ...
  function parse_NoteSh(blob, length, opts) {
    ...
    const row = blob.read_shift(2); const col = blob.read_shift(2);
    const flags = blob.read_shift(2); const idObj = blob.read_shift(2);
  ...
  function parse_ShtProps(blob, length, opts) {
    ...
    const d = blob.read_shift(1); blob.l += 3;
  ...
  function parse_ImData(blob) {
    ...
    const o = {fmt:cf, env, len:lcb, data:blob.slice(blob.l,blob.l+lcb)};
    blob.l += lcb;
    return o;
  }
  ...
  const DBF = (function() {
    ...
    function dbf_to_aoa(buf, opts) {
      ...
      let /* filedate = new Date(), */ nrow = 0; let fpos = 0;
      ...
      const hend = fpos - 10 - (vfp ? 264 : 0); const ww = l7 ? 32 : 11;
      ...
      while(nrow-- > 0) {
        ...
        for(C = 0; C != fields.length; ++C) {
          const dd = d.slice(d.l, d.l+fields[C].len); d.l+=fields[C].len;
          prep_blob(dd, 0);
    ...
    function sheet_to_dbf(ws, opts) {
      ...
      const headers = aoa[0]; const data = aoa.slice(1);
      let i = 0; let j = 0; let hcnt = 0; let rlen = 1;
      ...
      for(i = 0, j = 0; i < headers.length; ++i) {
        ...
        const hf = ba.next(32);
        const _f = (`${headers[i].slice(-10)  }\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00`).slice(0, 11);
  ...
  const SYLK = (function() {
    ...
    const sylk_char_regex = new RegExp(`\u001BN(${  keys(sylk_escapes).join("|").replace(/\|\|\|/, "|\\||").replace(/([?()+])/g,"\\$1")  }|\\|)`, "gm");
    const sylk_char_fn = function(_, $1){ const o = sylk_escapes[$1]; return typeof o === "number" ? _getansi(o) : o; };
    const decode_sylk_char = function($$, $1, $2) { const newcc = (($1.charCodeAt(0) - 0x20)<<4) | ($2.charCodeAt(0) - 0x30); return newcc == 59 ? $$ : _getansi(newcc); };
    sylk_escapes["|"] = 254;
    /* TODO: find an actual specification */
    ...
    function sylk_to_aoa_str(str, opts) {
      const records = str.split(/[\n\r]+/); let R = -1; let C = -1; let ri = 0; let rj = 0; let arr = [];
      const formats = [];
      let next_cell_format = null;
      const sht = {}; const rowinfo = []; const colinfo = []; let cw = [];
      let Mval = 0; let j;
      if(+opts.codepage >= 0) set_cp(+opts.codepage);
      for (; ri !== records.length; ++ri) {
        Mval = 0;
        const rstr=records[ri].trim().replace(/\x1B([\x20-\x2F])([\x30-\x3F])/g, decode_sylk_char).replace(sylk_char_regex, sylk_char_fn);
        const record=rstr.replace(/;;/g, "\u0000").split(";").map((x) => x.replace(/\u0000/g, ";"));
        const RT=record[0]; var val;
    ...
    function sylk_to_sheet(d, opts) {
      ...
      const aoa = aoasht[0]; const ws = aoasht[1];
      const o = aoa_to_sheet(aoa, opts);
      keys(ws).forEach((k) => { o[k] = ws[k]; });
      return o;
    }
    ...
    function sheet_to_sylk(ws, opts) {
      const preamble = ["ID;PWXL;N;E"]; const o = [];
      const r = safe_decode_range(ws['!ref']); let cell;
  ...
  const DIF = (function() {
    ...
    function dif_to_aoa_str(str, opts) {
      const records = str.split('\n'); let R = -1; let C = -1; let ri = 0; let arr = [];
      for (; ri !== records.length; ++ri) {
        if (records[ri].trim() === 'BOT') { arr[++R] = []; C = 0; continue; }
        if (R < 0) continue;
        const metadata = records[ri].trim().split(",");
        const type = metadata[0]; const value = metadata[1];
        ++ri;
  ...
  const ETH = (function() {
    ...
    function sheet_to_eth_data(ws) {
      if(!ws || !ws['!ref']) return "";
      const o = []; let oo = []; let cell; let coord = "";
  ...
  const PRN = (function() {
    ...
    function guess_sep(str) {
      let cnt = {}; let instr = false; let end = 0; let cc = 0;
      ...
      cc.sort((a, b) => a[0] - b[0] || guess_sep_weights[a[1]] - guess_sep_weights[b[1]]);
    ...
    function dsv_to_sheet_str(str, opts) {
      ...
      let R = 0; let C = 0; let v = 0;
      let start = 0; let end = 0; const sepcc = sep.charCodeAt(0); let instr = false; let cc=0;
      str = str.replace(/\r\n/mg, "\n");
    ...
    function prn_to_sheet(d, opts) {
      let str = ""; const bytes = opts.type == 'string' ? [0,0,0,0] : firstbyte(d, opts);
      switch(opts.type) {
  ...
  function read_wb_ID(d, opts) {
    const o = opts || {}; const OLD_WTF = !!o.WTF; o.WTF = true;
    try {
  ...
  const WK_ = (function() {
    ...
    function lotus_to_workbook_buf(d, opts) {
      ...
      const o = opts || {};
      if(DENSE != null && o.dense == null) o.dense = DENSE;
      let s = ((o.dense ? [] : {})); let n = "Sheet1"; let sidx = 0;
      const sheets = {}; const snames = [n];
    ...
    function parse_NUMBER_17(blob, length) {
      ...
      const s = e & 0x8000; e = (e&0x7FFF) - 16446;
  ...
  function parse_rpr(rpr) {
    const font = {}; const m = rpr.match(tagregex); let i = 0;
    let pass = false;
  ...
  const parse_rs = (function() {
    const tregex = matchtag("t"); const rpregex = matchtag("rPr");
    /* 18.4.4 r CT_RElt */
    function parse_r(r) {
    ...
    const rregex = /<(?:\w+:)?r>/g; const rend = /<\/(?:\w+:)?r>/;
  ...
  /* 18.4.8 si CT_Rst */
  const sitregex = /<(?:\w+:)?t[^>]*>([^<]*)<\/(?:\w+:)?t>/g; const sirregex = /<(?:\w+:)?r>/;
  ...
  function write_sst_xml(sst, opts) {
    ...
    for(let i = 0; i != sst.length; ++i) { if(sst[i] == null) continue;
      const s = sst[i];
      let sitag = "<si>";
      if(s.r) sitag += s.r;
      else {
  ...
  const write_BrtSSTItem = write_RichStr;

  function write_sst_bin(sst) {
    const ba = buf_array();
    write_record(ba, "BrtBeginSst", write_BrtBeginSst(sst));
    for(let i = 0; i < sst.length; ++i) write_record(ba, "BrtSSTItem", write_BrtSSTItem(sst[i]));
    /* FRTSST */
  ...
  function parse_EncryptionVerifier(blob, length) {
    const o = {}; const tgt = blob.l + length;
  ...
  function parse_EncInfoAgl(blob) {
    const KeyData = ["saltSize","blockSize","keyBits","hashSize","cipherAlgorithm","cipherChaining","hashAlgorithm","saltValue"];
    blob.l+=4;
    const xml = blob.read_shift(blob.length - blob.l, 'utf8');
    const o = {};
    xml.replace(tagregex, (x) => {
      const y = parsexmltag(x);
      switch(strip_ns(y[0])) {
      case '<?xml': break;
      case '<encryption': case '</encryption>': break;
      case '<keyData': KeyData.forEach((k) => { o[k] = y[k]; }); break;
  ...
  function parse_RC4CryptoHeader(blob, length) {
    const o = {};
    const vers = o.EncryptionVersionInfo = parse_CRYPTOVersion(blob, 4); length -= 4;
    if(vers.Minor != 2) throw new Error(`unrecognized minor version code: ${  vers.Minor}`);
    if(vers.Major > 4 || vers.Major < 2) throw new Error(`unrecognized major version code: ${  vers.Major}`);
    o.Flags = blob.read_shift(4); length -= 4;
    const sz = blob.read_shift(4); length -= 4;
  ...
  function crypto_CreatePasswordVerifier_Method1(Password) {
    let Verifier = 0x0000; let PasswordArray;
    const PasswordDecoded = _JS2ANSI(Password);
    const len = PasswordDecoded.length + 1; let i; let PasswordByte;
    let Intermediate1; let Intermediate2; let Intermediate3;
  ...
  const crypto_CreateXorArray_Method1 = (function() {
    ...
    const InitialCode = [0xE1F0, 0x1D0F, 0xCC9C, 0x84C0, 0x110C, 0x0E10, 0xF1CE, 0x313E, 0x1872, 0xE139, 0xD40F, 0x84F9, 0x280C, 0xA96A, 0x4EC3];
    const XorMatrix = [0xAEFC, 0x4DD9, 0x9BB2, 0x2745, 0x4E8A, 0x9D14, 0x2A09, 0x7B61, 0xF6C2, 0xFDA5, 0xEB6B, 0xC6F7, 0x9DCF, 0x2BBF, 0x4563, 0x8AC6, 0x05AD, 0x0B5A, 0x16B4, 0x2D68, 0x5AD0, 0x0375, 0x06EA, 0x0DD4, 0x1BA8, 0x3750, 0x6EA0, 0xDD40, 0xD849, 0xA0B3, 0x5147, 0xA28E, 0x553D, 0xAA7A, 0x44D5, 0x6F45, 0xDE8A, 0xAD35, 0x4A4B, 0x9496, 0x390D, 0x721A, 0xEB23, 0xC667, 0x9CEF, 0x29FF, 0x53FE, 0xA7FC, 0x5FD9, 0x47D3, 0x8FA6, 0x0F6D, 0x1EDA, 0x3DB4, 0x7B68, 0xF6D0, 0xB861, 0x60E3, 0xC1C6, 0x93AD, 0x377B, 0x6EF6, 0xDDEC, 0x45A0, 0x8B40, 0x06A1, 0x0D42, 0x1A84, 0x3508, 0x6A10, 0xAA51, 0x4483, 0x8906, 0x022D, 0x045A, 0x08B4, 0x1168, 0x76B4, 0xED68, 0xCAF1, 0x85C3, 0x1BA7, 0x374E, 0x6E9C, 0x3730, 0x6E60, 0xDCC0, 0xA9A1, 0x4363, 0x86C6, 0x1DAD, 0x3331, 0x6662, 0xCCC4, 0x89A9, 0x0373, 0x06E6, 0x0DCC, 0x1021, 0x2042, 0x4084, 0x8108, 0x1231, 0x2462, 0x48C4];
    const Ror = function(Byte) { return ((Byte/2) | (Byte*128)) & 0xFF; };
    const XorRor = function(byte1, byte2) { return Ror(byte1 ^ byte2); };
  ...
  const crypto_DecryptData_Method1 = function(password, Data, XorArrayIndex, XorArray, O) {
  ...
  const crypto_MakeXorDecryptor = function(password) {
    let XorArrayIndex = 0; const XorArray = crypto_CreateXorArray_Method1(password);
    return function(Data) {
  ...
  function parse_FilePassHeader(blob, length, oo) {
    const o = oo || {}; o.Info = blob.read_shift(2); blob.l -= 2;
    if(o.Info === 1) o.Data = parse_RC4Header(blob, length);
    else o.Data = parse_RC4CryptoHeader(blob, length);
    return o;
  }
  function parse_FilePass(blob, length, opts) {
    const o = ({ Type: opts.biff >= 8 ? blob.read_shift(2) : 0 }); /* wEncryptionType */
  ...
  function rgb2HSL(rgb) {
    ...
    const M = Math.max(R, G, B); const m = Math.min(R, G, B); const C = M - m;
    if(C === 0) return [0, 0, R];

    let H6 = 0; let S = 0; const L2 = (M + m);
    S = C / (L2 > 1 ? 2 - L2 : L2);
    switch(M){
  ...
  function hsl2RGB(hsl){
    ...
    const C = S * 2 * (L < 0.5 ? L : 1 - L); const m = L - C/2;
    const rgb = [m,m,m]; const h6 = 6*H;
  ...
  const DEF_MDW = 6; const MAX_MDW = 15; const MIN_MDW = 1; let MDW = DEF_MDW;
  ...
  function find_mdw_colw(collw) {
    let delta = Math.abs(collw - cycle_width(collw)); let _MDW = MDW;
  ...
  const DEF_PPI = 96; const PPI = DEF_PPI;
  ...
  function parse_numFmts(t, styles, opts) {
    ...
    for(i=0; i < m.length; ++i) {
      ...
      case '<numFmt': {
        const f=unescapexml(utf8read(y.formatCode)); let j=parseInt(y.numFmtId,10);
  ...
  function write_numFmts(NF) {
    const o = ["<numFmts>"];
    [[5,8],[23,26],[41,44],[/* 63 */50,/* 66],[164, */392]].forEach((r) => {
      for(let i = r[0]; i <= r[1]; ++i) if(NF[i] != null) o[o.length] = (writextag('numFmt',null,{numFmtId:i,formatCode:escapexml(NF[i])}));
    });
  ...
  const cellXF_bool = [ "applyAlignment", "applyBorder", "applyFill", "applyFont", "applyNumberFormat", "applyProtection", "pivotButton", "quotePrefix" ];
  function parse_cellXfs(t, styles, opts) {
    styles.CellXf = [];
    let xf;
    let pass = false;
    (t[0].match(tagregex)||[]).forEach((x) => {
      const y = parsexmltag(x); let i = 0;
      switch(strip_ns(y[0])) {
  ...
  const STYLES_XML_ROOT = writextag('styleSheet', null, {
    'xmlns': XMLNS.main[0],
    'xmlns:vt': XMLNS.vt
  });
  ...
  function write_sty_xml(wb, opts) {
    const o = [XML_HEADER, STYLES_XML_ROOT]; let w;
  ...
  const rev_XLSBFillPTNames = (evert(XLSBFillPTNames));
  /* TODO: gradient fill representation */
  const parse_BrtFill = parsenoop;
  function write_BrtFill(fill, o) {
    if(!o) o = new_buf(4*3 + 8*7 + 16*1);
    let fls = rev_XLSBFillPTNames[fill.patternType];
  ...
  function write_FMTS_bin(ba, NF) {
    ...
    let cnt = 0;
    [[5,8],[23,26],[41,44],[/* 63 */50,/* 66],[164, */392]].forEach((r) => {
      for(let i = r[0]; i <= r[1]; ++i) if(NF[i] != null) ++cnt;
    });

    if(cnt == 0) return;
    write_record(ba, "BrtBeginFmts", write_UInt32LE(cnt));
    [[5,8],[23,26],[41,44],[/* 63 */50,/* 66],[164, */392]].forEach((r) => {
      for(let i = r[0]; i <= r[1]; ++i) if(NF[i] != null) write_record(ba, "BrtFmt", write_BrtFmt(i, NF[i]));
    });
  ...
  function write_CELLXFS_bin(ba, data) {
    ...
    data.forEach((c) => { write_record(ba, "BrtXF", write_BrtXF(c,0)); });
    /* 1*65430(BrtXF *FRT) */
  ...
  function parse_Theme(blob, length, opts) {
    ...
    blob.l = end;
    let zip; try { zip = new jszip(data); } catch(e) { return; }
  ...
  function parse_xlink_bin(data, rel, name, _opts) {
    ...
    let pass = false; const end = false;
  ...
  function parse_comments_xml(data, opts) {
    ...
    const authors = [];
    const commentList = [];
    const authtag = data.match(/<(?:\w+:)?authors>([\s\S]*)<\/(?:\w+:)?authors>/);
    if(authtag && authtag[1]) authtag[1].split(/<\/\w*:?author>/).forEach((x) => {
      if(x === "" || x.trim() === "") return;
      const a = x.match(/<(?:\w+:)?author[^>]*>(.*)/);
      if(a) authors.push(a[1]);
    });
    const cmnttag = data.match(/<(?:\w+:)?commentList>([\s\S]*)<\/(?:\w+:)?commentList>/);
    if(cmnttag && cmnttag[1]) cmnttag[1].split(/<\/\w*:?comment>/).forEach((x) => {
      if(x === "" || x.trim() === "") return;
      const cm = x.match(/<(?:\w+:)?comment[^>]*>/);
      if(!cm) return;
      const y = parsexmltag(cm[0]);
      const comment = ({ author: y.authorId && authors[y.authorId] || "sheetjsghost", ref: y.ref, guid: y.guid });
  ...
  function write_comments_xml(data) {
    ...
    const iauthor = [];
    o.push("<authors>");
    data.forEach((x) => { x[1].forEach((w) => { const a = escapexml(w.a);
  ...
  function make_vba_xls(cfb) {
    ...
    cfb.FullPaths.forEach((p, i) => {
      ...
      const newpath = p.replace(/^[^\/]*/,"R").replace(/\/_VBA_PROJECT_CUR\u0000*/, "");
  ...
  function fill_vba_xls(cfb, vba) {
    vba.FullPaths.forEach((p, i) => {
      if(i == 0) return;
      const newpath = p.replace(/[^\/]*[\/]/, "/_VBA_PROJECT_CUR/");
      if(newpath.slice(-1) !== "/") CFB.utils.cfb_add(cfb, newpath, vba.FileIndex[i].content);
    });
  }

  const VBAFMTS = [ "xlsb", "xlsm", "xlam", "biff8", "xla" ];
  ...
  var rc_to_a1 = (function(){
    ...
    function rcfunc($$,$1,$2,$3) {
      ...
      let R = $2.length>0?parseInt($2,10)|0:0; let C = $3.length>0?parseInt($3,10)|0:0;
  ...
  function shift_formula_str(f, delta) {
    return f.replace(crefregex, ($0, $1, $2, $3, $4, $5) => $1+($2=="$" ? $2+$3 : encode_col(decode_col($3)+delta.c))+($4=="$" ? $4+$5 : encode_row(decode_row($5) + delta.r)));
  }

  function shift_formula_xlsx(f, range, cell) {
    const r = decode_range(range); const {s} = r; const c = decode_cell(cell);
  ...
  function parse_RgceArea_BIFF2(blob) {
    const r=parse_ColRelU(blob, 2); const R=parse_ColRelU(blob, 2);
  ...
  function parse_RgceAreaRel(blob, length, opts) {
    ...
    const r=blob.read_shift(opts.biff == 12 ? 4 : 2); const R=blob.read_shift(opts.biff == 12 ? 4 : 2);
    const c=parse_ColRelU(blob, 2);
    const C=parse_ColRelU(blob, 2);
    return { s:{r, c:c[0], cRel:c[1], rRel:c[2]}, e:{r:R, c:C[0], cRel:C[1], rRel:C[2]} };
  }
  ...
  function parse_RgceElfLoc(blob) {
    ...
    return {r, c:c & 0xFF, fQuoted:!!(c & 0x4000), cRel:c>>15, rRel:c>>15 };
  }
  ...
  function parse_RgceLocRel(blob, length, opts) {
    ...
    const cRel = (cl & 0x4000) >> 14; const rRel = (cl & 0x8000) >> 15;
    cl &= 0x3FFF;
  ...
  function parse_RgceLocRel_BIFF2(blob) {
    ...
    const rRel = (rl & 0x8000) >> 15; const cRel = (rl & 0x4000) >> 14;
    rl &= 0x3FFF;
  ...
  function parse_PtgAttrChoose(blob, length, opts) {
    ...
    for(let i = 0; i <= offset; ++i) o.push(blob.read_shift(opts && opts.biff == 2 ? 1 : 2));
    return o;
  }
  ...
  function parse_PtgAttrSpaceType(blob) {
    const type = blob.read_shift(1); const cch = blob.read_shift(1);
    return [type, cch];
  }
  ...
  function parse_PtgFuncVar(blob, length, opts) {
    ...
    const cparams = blob.read_shift(1); const tab = opts && opts.biff <= 3 ? [(type == 0x58 ? -1 : 0), blob.read_shift(1)]: parsetab(blob);
  ...
  function parse_PtgExtraMem(blob, cce, opts) {
    ...
    const out = [];
    for(let i = 0; i != count; ++i) out.push(((opts.biff == 12) ? parse_UncheckedRfX : parse_Ref8U)(blob, 8));
    return out;
  }
  ...
  (function(){for(const y in PtgDupes) PtgTypes[y] = PtgTypes[PtgDupes[y]];})();
  ...
  function get_ixti_raw(supbooks, ixti, opts) {
    ...
    case 0x0401:
      ...
      return XTI[1] == XTI[2] ? o : `${o  }:${  supbooks.SheetNames[XTI[2]]}`;
    case 0x3A01: return supbooks[XTI[0]].slice(1).map((name) => name.Name).join(";;"); // return "SH33TJSERR8";
    default:
  ...
  function stringify_formula(formula/* Array<any> */, range, cell, supbooks, opts) {
    ...
    const stack = []; let e1; let e2; let  c; let ixti=0; let nameidx=0; let r; let sname="";
    if(!formula[0] || !formula[0][0]) return "";
    let last_sp = -1; let sp = "";
  ...
  function parse_ArrayParsedFormula(blob, length, opts) {
    const target = blob.l + length; const len = opts.biff == 2 ? 1 : 2;
    let rgcb; const cce = blob.read_shift(len); // length of rgce
  ...
  function parse_SharedParsedFormula(blob, length, opts) {
    const target = blob.l + length;
    let rgcb; const cce = blob.read_shift(2); // length of rgce
    const rgce = parse_Rgce(blob, cce, opts);
    if(cce == 0xFFFF) return [[],parsenoop(blob, length-2)];
  ...
  function write_FormulaValue(value) {
    if(value == null) {
      ...
      return o;
    } if(typeof value === "number") return write_Xnum(value);
    return write_Xnum(0);
  }
  ...
  function ods_to_csf_formula(f) {
    ...
    f = f.replace(/\[((?:\.[A-Z]+[0-9]+)(?::\.[A-Z]+[0-9]+)?)\]/g, ($$, $1) => $1.replace(/\./g,""));
  ...
  function get_cell_style(styles, cell, opts) {
    ...
    let i = 0x3c; const len = styles.length;
    if(z == null && opts.ssf) {
  ...
  function write_ws_xml_merges(merges) {
    ...
    for(let i = 0; i != merges.length; ++i) o += `<mergeCell ref="${  encode_range(merges[i])  }"/>`;
    return `${o  }</mergeCells>`;
  }
  ...
  function write_ws_xml_sheetpr(ws, wb, idx, opts, o) {
    let needed = false;
    const props = {}; const payload = null;
    if(opts.bookType !== 'xlsx' && wb.vbaraw) {
      let cname = wb.SheetNames[idx];
  ...
  const sheetprot_deffalse = ["objects", "scenarios", "selectLockedCells", "selectUnlockedCells"];
  const sheetprot_deftrue = [
  ...
  function parse_ws_xml_cols(columns, cols) {
    ...
    for(let coli = 0; coli != cols.length; ++coli) {
      ...
      let colm=parseInt(coll.min, 10)-1; const colM=parseInt(coll.max,10)-1;
  ...
  function write_ws_xml_autofilter(data, ws, wb, idx) {
    let ref = typeof data.ref === "string" ? data.ref : encode_range(data.ref);
  ...
  function parse_ws_xml_sheetviews(data, wb) {
    ...
    (data.match(sviewregex)||[]).forEach((r, i) => {
  ...
  function write_ws_xml_cell(cell, ref, ws, opts) {
    ...
    let vv = "";
    const oldt = cell.t; const oldv = cell.v;
    ...
    let v = writetag('v', escapexml(vv)); const o = ({r:ref});
    /* TODO: cell style */
    ...
    if(cell.f) {
      const ff = cell.F && cell.F.slice(0, ref.length) == ref ? {t:"array", ref:cell.F} : null;
  ...
  var parse_ws_xml_data = (function() {
    const cellregex = /<(?:\w+:)?c[ >]/; const rowregex = /<\/(?:\w+:)?row>/;
    const rregex = /r=["']([^"']*)["']/; const isregex = /<(?:\w+:)?is>([\S\s]*?)<\/(?:\w+:)?is>/;
    const refregex = /ref=["']([^"']*)["']/;
    const match_v = matchtag("v"); const match_f = matchtag("f");

    return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles) {
      let ri = 0; let x = ""; let cells = []; let cref = []; let idx=0; let i=0; let cc=0; let d=""; let p;
      let tag; let tagr = 0; let tagc = 0;
      let sstr; let ftag;
      let fmtid = 0; let fillid = 0;
      const do_format = Array.isArray(styles.CellXf); let cf;
      const arrayf = [];
      const sharedf = [];
      const dense = Array.isArray(s);
      const rows = []; let rowobj = {}; let rowrite = false;
  ...
  function write_ws_xml_data(ws, opts, idx, wb) {
    const o = []; let r = []; const range = safe_decode_range(ws['!ref']); let cell=""; let ref; let rr = ""; const cols = []; let R=0; let C=0; const rows = ws['!rows'];
    const dense = Array.isArray(ws);
    let params = ({r:rr}); let row; let height = -1;
  ...
  function write_ws_xml(idx, opts, wb, rels) {
    ...
    const s = wb.SheetNames[idx]; let sidx = 0; let rdata = "";
    let ws = wb.Sheets[s];
    if(ws == null) ws = {};
    let ref = ws['!ref'] || 'A1';
  ...
  const write_BrtWsDim = write_UncheckedRfX;
  ...
  function write_BrtWsProp(str, o) {
    ...
    for(let i = 0; i < 3; ++i) o.write_shift(1,0);
    write_BrtColor({auto:1}, o);
    o.write_shift(-4,-1);
    o.write_shift(-4,-1);
  ...
  function write_BrtHLink(l, rId) {
    const o = new_buf(50+4*(l[1].Target.length + (l[1].Tooltip || "").length));
  ...
  function parse_BrtMargins(data) {
    const margins = ({});
    BrtMarginKeys.forEach((k) => { margins[k] = parse_Xnum(data, 8); });
    return margins;
  }
  function write_BrtMargins(margins, o) {
    if(o == null) o = new_buf(6*8);
    default_margins(margins);
    BrtMarginKeys.forEach((k) => { write_Xnum((margins)[k], o); });
    return o;
  }
  ...
  function parse_ws_bin(data, _opts, idx, rels, wb, themes, styles) {
    ...
    const state = [];
    let pass = false; let end = false;
    let row; let p; let cf; let R; let C; let addr; let sstr; let rr; let cell;
    const merges = [];
    opts.biff = 12;
    opts['!row'] = 0;

    let ai = 0; let af = false;
    ...
    supbooks.SheetNames = wb.SheetNames || wb.Sheets.map((x) => x.name);
    if(!opts.supbooks) {
      opts.supbooks = supbooks;
      if(wb.Names) for(let i = 0; i < wb.Names.length; ++i) supbooks[0][i+1] = wb.Names[i];
    }

    const colinfo = []; const rowinfo = [];
    let seencol = false;
    ...
    recordhopper(data, (val, R_n, RT) => {
      ...
      case 0x000B: /* 'BrtFmlaError' */
        ...
        if(opts.cellDates && cf && p.t == 'n' && SSF.is_date(SSF._table[cf.numFmtId])) {
          const _d = SSF.parse_date_code(p.v); if(_d) { p.t = 'd'; p.v = new Date(_d.y, _d.m-1,_d.d,_d.H,_d.M,_d.S,_d.u); }
        }
        break;
  ...
  function write_CELLTABLE(ba, ws, idx, opts) {
    const range = safe_decode_range(ws['!ref'] || "A1"); let ref; let rr = ""; const cols = [];
    write_record(ba, 'BrtBeginSheetData');
    const dense = Array.isArray(ws);
    let cap = range.e.r;
    if(ws['!rows']) cap = Math.max(range.e.r, ws['!rows'].length - 1);
    for(let R = range.s.r; R <= cap; ++R) {
      rr = encode_row(R);
      /* [ACCELLTABLE] */
      /* BrtRowHdr */
      write_row_header(ba, ws, range, R);
      if(R <= range.e.r) for(let C = range.s.c; C <= range.e.c; ++C) {
        /* *16384CELL */
  ...
  function write_MERGECELLS(ba, ws) {
    ...
    ws['!merges'].forEach((m) => { write_record(ba, 'BrtMergeCell', write_BrtMergeCell(m)); });
  ...
  function write_COLINFOS(ba, ws) {
    ...
    ws['!cols'].forEach((m, i) => { if(m) write_record(ba, 'BrtColInfo', write_BrtColInfo(i, m)); });
  ...
  function write_ws_bin(idx, opts, wb, rels) {
    const ba = buf_array();
    const s = wb.SheetNames[idx]; const ws = wb.Sheets[s] || {};
    let c = s; try { if(wb && wb.Workbook) c = wb.Workbook.Sheets[idx].CodeName || c; } catch(e) {}
  ...
  function parse_Cache(data) {
    ...
    (data.match(/<c:pt idx="(\d*)">(.*?)<\/c:pt>/mg)||[]).forEach((pt) => {
    ...
    const nf = unescapexml((data.match(/<c:formatCode>([\s\S]*?)<\/c:formatCode>/) || ["","General"])[1]);

    (data.match(/<c:f>(.*?)<\/c:f>/mg)||[]).forEach((F) => { f = F.replace(/<.*?>/g,""); });
  ...
  function parse_chart(data, name, opts, rels, wb, csheet) {
    ...
    let C = 0; let R = 0; let col = "A";
    const refguess = {s: {r:2000000, c:2000000}, e: {r:0, c:0} };

    /* 21.2.2.120 numCache CT_NumData */
    (data.match(/<c:numCache>[\s\S]*?<\/c:numCache>/gm)||[]).forEach((nc) => {
  ...
  function push_defaults_array(target, defaults) {
    for(let j = 0; j != target.length; ++j) { const w = target[j];
      for(let i=0; i != defaults.length; ++i) { const z = defaults[i];
  ...
  function check_wb_names(N, S, codes) {
    N.forEach((n,i) => {
      check_ws_name(n);
      for(let j = 0; j < i; ++j) if(n == N[j]) throw new Error(`Duplicate Sheet Name: ${  n}`);
      if(codes) {
        const cn = (S && S[i] && S[i].CodeName) || n;
  ...
  function check_wb(wb) {
    ...
    for(let i = 0; i < wb.SheetNames.length; ++i) check_ws(wb.Sheets[wb.SheetNames[i]], wb.SheetNames[i], i);
  ...
  function parse_wb_xml(data, opts) {
    ...
    let pass = false; let xmlns = "xmlns";
    let dname = {}; let dnstart = 0;
    data.replace(tagregex, (x, idx) => {
      const y = parsexmltag(x);
      switch(strip_ns(y[0])) {
      case '<?xml': break;
  ...
  const WB_XML_ROOT = writextag('workbook', null, {
    'xmlns': XMLNS.main[0],
    // 'xmlns:mx': XMLNS.mx,
  ...
  function write_BOOKVIEWS(ba, wb) {
    ...
    let i = 0; let vistab = -1; let hidden = -1;
...
*/
  ...
  function xlml_parsexmltag(tag, skip_root) {
    ...
    const z = ([]); if(!skip_root) z[0] = words[0];
    if(words.length === 1) return z;
    const m = tag.match(attregexg2); let y; let j; let w; let i;
    if(m) for(i = 0; i != m.length; ++i) {
  ...
  function safe_format_xlml(cell, nf, o) {
    ...
    try {
      ...
      if(o.cellDates && cell.t == 'n' && SSF.is_date(z)) {
        const _d = SSF.parse_date_code(cell.v); if(_d) { cell.t = 'd'; cell.v = new Date(_d.y, _d.m-1,_d.d,_d.H,_d.M,_d.S,_d.u); }
      }
    } catch(e) { if(o.WTF) throw e; }
  }
  ...
  function parse_xlml_data(xml, ss, data, cell, base, styles, csty, row, arrayf, o) {
    let nf = "General"; let sid = cell.StyleID; const S = {}; o = o || {};
    const interiors = [];
    let i = 0;
  ...
  function parse_xlml_xml(d, _opts) {
    ...
    const opening = str.slice(0, 1024).toLowerCase(); let ishtml = false;
    if(opening.indexOf("<?xml") == -1) ["html", "table", "head", "meta", "script", "style", "div"].forEach((tag) => { if(opening.indexOf(`<${  tag}`) >= 0) ishtml = true; });
    if(ishtml) return HTML_.to_workbook(str, opts);
    let Rn;
    const state = []; let tmp;
    if(DENSE != null && opts.dense == null) opts.dense = DENSE;
    const sheets = {}; const sheetnames = []; let cursheet = (opts.dense ? [] : {}); let sheetname = "";
    let table = {}; let cell = ({}); let row = {};// eslint-disable-line no-unused-vars
    let dtag = xlml_parsexmltag('<Data ss:Type="String">'); let didx = 0;
    let c = 0; let r = 0;
    let refguess = {s: {r:2000000, c:2000000}, e: {r:0, c:0} };
    const styles = {}; let stag = {};
    let ss = ""; let fidx = 0;
    let merges = [];
    const Props = {}; const Custprops = {}; let pidx = 0; let cp = [];
    let comments = []; let comment = ({});
    let cstys = []; let csty; let seencol = false;
    let arrayf = [];
    let rowinfo = []; let rowobj = {}; let cc = 0; let rr = 0;
    const Workbook = ({ Sheets:[], WBProps:{date1904:false} }); let wsprops = {};
    xlmlregex.lastIndex = 0;
    str = str.replace(/<!--([\s\S]*?)-->/mg,"");
    while((Rn = xlmlregex.exec(str))) switch(Rn[3]) {
    case 'Data':
      if(state[state.length-1][1]) break;
      if(Rn[1]==='/') parse_xlml_data(str.slice(didx, Rn.index), ss, dtag, state[state.length-1][0]=="Comment"?comment:cell, {c,r}, styles, cstys[c], row, arrayf, opts);
    ...
    case 'Column':
      ...
      if(!seencol && csty.wpx > 10) {
        ...
        for(let _col = 0; _col < cstys.length; ++_col) if(cstys[_col]) process_col(cstys[_col]);
      }
  ...
  function write_sty_xlml(wb, opts) {
    /* Styles */
    const styles = ['<Style ss:ID="Default" ss:Name="Normal"><NumberFormat/></Style>'];
    opts.cellXfs.forEach((xf, id) => {
      const payload = [];
      payload.push(writextag('NumberFormat', null, {"ss:Format": escapexml(SSF._table[xf.numFmtId])}));

      const o = {"ss:ID": `s${  21+id}`};
  ...
  function write_ws_xlml_wsopts(ws, opts, idx, wb) {
    ...
    if(ws['!protect']) {
      ...
      ].forEach((x) => { if(ws['!protect'][x[0]]) o.push(`<${x[1]}/>`); });
    }
  ...
  function write_ws_xlml_cell(cell, ref, ws, opts, idx, wb, addr){
    ...
    if(cell.F && cell.F.slice(0, ref.length) == ref) {
      const end = decode_cell(cell.F.slice(ref.length + 1));
    ...
    const _v = (cell.v != null ? p : "");
    let m = cell.t == 'z' ? "" : (`<Data ss:Type="${  t  }">${  _v  }</Data>`);
  ...
  function write_ws_xlml_table(ws, opts, idx, wb) {
    if(!ws['!ref']) return "";
    const range = safe_decode_range(ws['!ref']);
    const marr = ws['!merges'] || []; let mi = 0;
    const o = [];
    if(ws['!cols']) ws['!cols'].forEach((n, i) => {
      process_col(n);
      const w = !!n.width;
    ...
    for(let R = range.s.r; R <= range.e.r; ++R) {
      ...
      for(let C = range.s.c; C <= range.e.c; ++C) {
        ...
        if(skip) continue;
        const addr = {r:R,c:C};
        const ref = encode_cell(addr); const cell = dense ? (ws[R]||[])[C] : ws[ref];
  ...
  function write_ws_xlml(idx, opts, wb) {
    ...
    let t = ws ? write_ws_xlml_names(ws, opts, idx, wb) : "";
...
*/
  function slurp(R, blob, length, opts) {
    ...
    prep_blob(b, 0);
    let ll = 0; b.lens = [];
    for(let j = 0; j < bufs.length; ++j) { b.lens.push(ll); ll += bufs[j].length; }
  ...
  function parse_workbook(blob, options) {
    ...
    let cur_sheet = "";
    let Preamble = {};
    let lastcell; let last_cell = ""; let cc; let cmnt; let rngC; let rngR;
    const sharedf = {};
    let arrayf = [];
    let temp_val;
    let country;
    ...
    let palette = [];
    const Workbook = ({ Sheets:[], WBProps:{date1904:false}, Views:[{}] }); let wsprops = {};
    ...
    let merges = [];
    let objects = [];
    let colinfo = []; let rowinfo = [];
    // eslint-disable-next-line no-unused-vars
    let defwidth = 0; let defheight = 0; // twips / MDW respectively
    let seencol = false;
    ...
    let BIFF2Fmt = 0; const BIFF2FmtTable = [];
    ...
    while(blob.l < blob.length - 1) {
      ...
      if(R && R.f) {
        ...
        last_Rn = R.n;
        if(R.r === 2 || R.r == 12) {
          const rt = blob.read_shift(2); length -= 2;
        ...
        case 'Formula': {
          ...
          if(options.cellFormula) {
            ...
            if(_f && _f[0] && _f[0][0] && _f[0][0][0] == 'PtgExp') {
              const _fr = _f[0][0][1][0]; const _fc = _f[0][0][1][1];
    ...
    wb.SheetNames=keys(Directory).sort((a,b) => Number(a) - Number(b)).map((x)=> Directory[x].name);
    if(!options.bookSheets) wb.Sheets=Sheets;
    if(wb.Sheets) FilterDatabases.forEach((r,i) => { wb.Sheets[wb.SheetNames[i]]['!autofilter'] = r; });
  ...
  function parse_xls_props(cfb, props, o) {
    ...
    if(DSI && DSI.size > 0) try {
      const DocSummary = parse_PropertySetStream(DSI, DocSummaryPIDDSI, PSCLSID.DSI);
  ...
  function write_xls_props(wb, cfb) {
    const DSEntries = []; const SEntries = []; const CEntries = [];
    let i = 0; let Keys;
    if(wb.Props) {
      Keys = keys(wb.Props);
      // $FlowIgnore
  ...
  function write_FMTS_biff8(ba, NF, opts) {
    if(!NF) return;
    [[5,8],[23,26],[41,44],[/* 63 */50,/* 66],[164, */392]].forEach((r) => {
      for(let i = r[0]; i <= r[1]; ++i) if(NF[i] != null) write_biff_rec(ba, "Format", write_Format(i, NF[i], opts));
    });
  }
  ...
  function write_CELLXFS_biff8(ba, opts) {
    for(let i = 0; i < 16; ++i) write_biff_rec(ba, "XF", write_XF({numFmtId:0, style:true}, 0, opts));
    opts.cellXfs.forEach((c) => {
  ...
  function write_biff8_global(wb, bufs, opts) {
    ...
    const b8 = opts.biff == 8; const b5 = opts.biff == 5;
  ...
  function write_biff8_buf(wb, opts) {
    ...
    for(let i = 0; i < wb.SheetNames.length; ++i) bufs[bufs.length] = write_ws_biff8(i, o, wb);
  ...
  var HTML_ = (function() {
    function html_to_sheet(str, _opts) {
      ...
      let i = mtch.index; let j = mtch2 && mtch2.index || str.length;
      const rows = split_regex(str.slice(i, j), /(:?<tr[^>]*>)/i, "<tr>");
      let R = -1; let C = 0; let RS = 0; let CS = 0;
  ...
  function parse_dom_table(table, _opts) {
    ...
    const sheetRows = opts.sheetRows || 10000000;
    const range = {s:{r:0,c:0},e:{r:0,c:0}};
    const merges = []; let midx = 0;
    const rowinfo = [];
    let _R = 0; let R = 0; let _C = 0; let C = 0; let RS = 0; let CS = 0;
    for(; _R < rows.length && R < sheetRows; ++_R) {
      ...
      for(_C = C = 0; _C < elts.length; ++_C) {
        ...
        const v = htmldecode(elt.innerHTML);
  ...
  const parse_content_xml = (function() {
    ...
    const parse_text_p = function(text) {
      ...
      const fixed = text
        .replace(/[\t\r\n]/g, " ").trim().replace(/ +/g, " ")
        .replace(/<text:s\/>/g," ")
        .replace(/<text:s text:c="(\d+)"\/>/g, ($$,$1) => Array(parseInt($1,10)+1).join(" "))
        .replace(/<text:tab[^>]*\/>/g,"\t")
    ...
    return function pcx(d, _opts) {
      ...
      const state = []; let tmp;
      let tag;
      let NFtag = {name:""}; let NF = ""; let pidx = 0;
      let sheetag;
      let rowtag;
      ...
      let Rn; let q;
      let ctag = ({value:""});
      let textp = ""; let textpidx = 0; let textptag;
      let textR = [];
      let R = -1; let C = -1; const range = {s: {r:1000000,c:10000000}, e: {r:0, c:0}};
      let row_ol = 0;
      const number_format_map = {};
      let merges = []; let mrange = {}; let mR = 0; let mC = 0;
      let rowinfo = []; let rowpeat = 1; let colpeat = 1;
      const arrayf = [];
      const WB = {Names:[]};
      let atag = ({});
      let _Ref = ["", ""];
      let comments = []; let comment = ({});
      let creator = ""; let creatoridx = 0;
      let isstub = false; let intable = false;
      let i = 0;
      xlmlregex.lastIndex = 0;
  ...
  /* OpenDocument */
  const write_styles_ods = (function() {
    const payload = `<office:document-styles ${  wxt_helper({
  ...
  const write_content_ods = (function() {
    /* 6.1.2 White Space Characters */
    const write_text_p = function(text) {
      return escapexml(text)
        .replace(/  +/g, ($$)=> `<text:s text:c="${$$.length}"/>`)
        .replace(/\t/g, "<text:tab/>")
        .replace(/\n/g, "<text:line-break/>")
    ...
    const write_ws = function(ws, wb, i) {
      /* Section 9 Tables */
      const o = [];
      o.push(`      <table:table table:name="${  escapexml(wb.SheetNames[i])  }">\n`);
      let R=0; let C=0; const range = decode_range(ws['!ref']);
      const marr = ws['!merges'] || []; let mi = 0;
      const dense = Array.isArray(ws);
      ...
      for(; R <= range.e.r; ++R) {
        ...
        for(; C <= range.e.c; ++C) {
          let skip = false; const ct = {}; let textp = "";
          for(mi = 0; mi != marr.length; ++mi) {
            if(marr[mi].s.c > C) continue;
            if(marr[mi].s.r > R) continue;
            if(marr[mi].e.c < C) continue;
          ...
          const ref = encode_cell({r:R, c:C}); const cell = dense ? (ws[R]||[])[C]: ws[ref];
          if(cell && cell.f) {
          ...
          let text_p = write_text_p(textp);
          if(cell.l && cell.l.Target) {
            let _tgt = cell.l.Target; _tgt = _tgt.charAt(0) == "#" ? `#${  csf_to_ods_3D(_tgt.slice(1))}` : _tgt;
    ...
    const write_automatic_styles_ods = function(o) {
    ...
    return function wcx(wb, opts) {
      ...
      for(let i = 0; i != wb.SheetNames.length; ++i) o.push(write_ws(wb.Sheets[wb.SheetNames[i]], wb, i, opts));
      o.push('    </office:spreadsheet>\n');
      o.push('  </office:body>\n');
  ...
  function write_obj_str(factory) {
    return function write_str(wb, o) {
      const idx = write_sheet_index(wb, o.sheet);
      return factory.from_sheet(wb.Sheets[wb.SheetNames[idx]], o, wb);
    };
  }

  const write_htm_str = write_obj_str(HTML_);
  const write_csv_str = write_obj_str({from_sheet:sheet_to_csv});
  const write_slk_str = write_obj_str(typeof SYLK !== "undefined" ? SYLK : {});
  const write_dif_str = write_obj_str(typeof DIF !== "undefined" ? DIF : {});
  const write_prn_str = write_obj_str(typeof PRN !== "undefined" ? PRN : {});
  const write_rtf_str = write_obj_str(typeof RTF !== "undefined" ? RTF : {});
  const write_txt_str = write_obj_str({from_sheet:sheet_to_txt});
  const write_dbf_buf = write_obj_str(typeof DBF !== "undefined" ? DBF : {});
  const write_eth_str = write_obj_str(typeof ETH !== "undefined" ? ETH : {});
  ...
  function safe_parse_wbrels(wbrels, sheets) {
    if(!wbrels) return 0;
    try {
      wbrels = sheets.map((w) => { if(!w.id) w.id = w.strRelID; return [w.name, wbrels['!id'][w.id].Target, get_sheet_type(wbrels['!id'][w.id].Type)]; });
    } catch(e) { return null; }
  ...
  function safe_parse_sheet(zip, path, relsPath, sheet, idx, sheetRels, sheets, stype, opts, wb, themes, styles) {
    try {
      ...
      let comments = [];
      if(sheetRels && sheetRels[sheet]) keys(sheetRels[sheet]).forEach((n) => {
  ...
  function parse_zip(zip, opts) {
    ...
    /* var externbooks = */dir.links.map((link) => {
      try {
        const rels = parse_rels(getzipstr(zip, get_rels_path(strip_front_slash(link))), link);
        return parse_xlink(getzipdata(zip, strip_front_slash(link)), rels, link, opts);
      } catch(e) {}
    });

    const wb = parse_wb(getzipdata(zip, strip_front_slash(dir.workbooks[0])), dir.workbooks[0], opts);

    let props = {}; let propdata = "";
  ...
  function parse_xlsxcfb(cfb, _opts) {
    const opts = _opts || {};
    let f = 'Workbook'; let data = CFB.find(cfb, f);
    try {
  ...
  function write_zip(wb, opts) {
    ...
    const wbext = opts.bookType == "xlsb" ? "bin" : "xml";
    const vbafmt = VBAFMTS.indexOf(opts.bookType) > -1;
    const ct = new_ct();
    ...
    for(rId=1;rId <= wb.SheetNames.length; ++rId) {
      ...
      if(ws) {
        ...
        if(comments && comments.length > 0) {
          const cf = `xl/comments${  rId  }.${  wbext}`;
  ...
  function read_plaintext_raw(data, o) {
    let str = ""; const bytes = firstbyte(data, o);
    switch(o.type) {
  ...
  function readSync(data, opts) {
    ...
    let d = data; let n = [0,0,0,0]; let str = false;
    let o = opts||{};
    ...
    if(o.type == 'array' && typeof Uint8Array !== 'undefined' && data instanceof Uint8Array && typeof ArrayBuffer !== 'undefined') {
      // $FlowIgnore
      const ab=new ArrayBuffer(3); const vu=new Uint8Array(ab); vu.foo="bar";
      // $FlowIgnore
  ...
  function write_zip_type(wb, opts) {
    ...
    const out = z.FullPaths ? CFB.write(z, {fileType:"zip", type: {"nodebuffer": "buffer", "string": "binary"}[oopts.type] || oopts.type}) : z.generate(oopts);
    /* jshint -W083 */
  ...
  function write_string_type(out, opts, bom) {
    ...
    case "buffer": {
      ...
      return write_string_type(o, {type:'binary'}).split("").map((c) => c.charCodeAt(0));
    }
    }
    throw new Error(`Unrecognized type ${  opts.type}`);
  }
  ...
  function write_stxt_type(out, opts) {
    ...
    case "buffer": {
      ...
      return out.split("").map((c) => c.charCodeAt(0));
    }
    }
    throw new Error(`Unrecognized type ${  opts.type}`);
  }
  ...
  function writeSync(wb, opts) {
    ...
    const o = opts||{};
    if(o.cellStyles) { o.cellNF = true; o.sheetStubs = true; }
    if(o.type == "array") { o.type = "binary"; const out = (writeSync(wb, o)); o.type = "array"; return s2ab(out); }
  ...
  function resolve_book_type(o) {
    ...
    const ext = o.file.slice(o.file.lastIndexOf(".")).toLowerCase();
  ...
  function writeFileAsync(filename, wb, opts, cb) {
    ...
    resolve_book_type(o);
    o.type = 'buffer';
    let _cb = cb; if(!(_cb instanceof Function)) _cb = (opts);
    return _fs.writeFile(filename, writeSync(wb, o), _cb);
  }
  function make_json_row(sheet, r, R, cols, header, hdr, dense, o) {
    const rr = encode_row(R);
    const {defval} = o; const raw = o.raw || !Object.prototype.hasOwnProperty.call(o, "raw");
    let isempty = true;
    const row = (header === 1) ? [] : {};
    if(header !== 1) {
    ...
    if(!dense || sheet[R]) for (let C = r.s.c; C <= r.e.c; ++C) {
  ...
  function sheet_to_json(sheet, opts) {
    ...
    let val = {t:'n',v:0}; let header = 0; let offset = 1; const hdr = []; let v=0; let vv="";
    ...
    const cols = [];
    const out = [];
    let outi = 0; let counter = 0;
    const dense = Array.isArray(sheet);
    let R = r.s.r; let C = 0; let CC = 0;
  ...
  function make_csv_row(sheet, r, R, cols, fs, rs, FS, o) {
    let isempty = true;
    const row = []; let txt = ""; const rr = encode_row(R);
    for(let C = r.s.c; C <= r.e.c; ++C) {
      if (!cols[C]) continue;
      ...
      if(val == null) txt = "";
        ...
        for(let i = 0, cc = 0; i !== txt.length; ++i) if((cc = txt.charCodeAt(i)) === fs || cc === rs || cc === 34) {txt = `"${  txt.replace(qreg, '""')  }"`; break; }
        if(txt == "ID") txt = '"ID"';
  ...
  function sheet_to_csv(sheet, opts) {
    ...
    const FS = o.FS !== undefined ? o.FS : ","; const fs = FS.charCodeAt(0);
    ...
    for(let C = r.s.c; C <= r.e.c; ++C) if (!((colinfo[C]||{}).hidden)) cols[C] = encode_col(C);
    for(let R = r.s.r; R <= r.e.r; ++R) {
  ...
  function sheet_to_txt(sheet, opts) {
    ...
    const o = cptable.utils.encode(1200, s, 'str');
  ...
  function sheet_to_formulae(sheet) {
    let y = ""; let x; let val="";
    if(sheet == null || sheet["!ref"] == null) return [];
    const r = safe_decode_range(sheet['!ref']); let rr = ""; const cols = []; let C;
    const cmds = [];
    const dense = Array.isArray(sheet);
  ...
  function sheet_add_json(_ws, js, opts) {
    ...
    js.forEach((JS, R) => {
      keys(JS).forEach((k) => {
        ...
        let t = 'z';
        let z = "";
        const ref = encode_cell({c:_C + C,r:_R + R + offset});
  ...
  (function(utils) {
    ...
    function add_consts(R/* Array<any> */) { R.forEach((a)=> { utils.consts[a[0]] = a[1]; }); }
    ...
    utils.book_append_sheet = function(wb, ws, name) {
      if(!name) for(let i = 1; i <= 0xFFFF; ++i) if(wb.SheetNames.indexOf(name = `Sheet${  i}`) == -1) break;
    ...
    utils.sheet_set_array_formula = function(ws, range, formula) {
      ...
      for(let R = rng.s.r; R <= rng.e.r; ++R) for(let C = rng.s.c; C <= rng.e.c; ++C) {
        const cell = ws_get_cell_stub(ws, R, C);
        cell.t = 'n';
        cell.F = rngstr;
        delete cell.v;
  ...
  if(has_buf && typeof require !== 'undefined') (function() {
    ...
    const write_csv_stream = function(sheet, opts) {
    ...
    const write_html_stream = function(ws, opts) {
      const stream = Readable();

      const o = opts || {};
      const header = o.header != null ? o.header : HTML_.BEGIN;
      const footer = o.footer != null ? o.footer : HTML_.END;
      stream.push(header);
      const r = decode_range(ws['!ref']);
      o.dense = Array.isArray(ws);
    ...
    const write_json_stream = function(sheet, opts) {
...
else if(typeof define === 'function' && define.amd) define('xlsx', () => { if(!XLSX.version) make_xlsx_lib(XLSX); return XLSX; });
else make_xlsx_lib(XLSX);
/* exported XLS, ODS */
const XLS = XLSX; const ODS = XLSX;
