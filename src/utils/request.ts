...
instance.interceptors.response.use(
    response => {
        ...
        if (response.data.code === 401 && interceptCount === 0) {
            ...
            if (response.config.url.substring(0, response.config.url.indexOf("?")) !== "/bn-amm/gem001b/queryAlcAlarmByParam" && whiteListError.indexOf(response.config.url) === -1) {
                ...
                // }
                // 第三版右侧错误提示信息
                if (response.data.msg !== "内部服务器错误" && response.data.msg !== "服务未找到" && /[\u4e00-\u9fa5]/.test(response.data.msg)) {
                    Notification.error({
                        title: "",
                        message: response.data.msg,
                        duration: 1000,
    ...
    err => {
        ...
        if (!config || !config.retry) {
            if (response?.config.headers.unDisplayErrTip) {
                ...
                logoutBox();
            } else if (config.url.substring(0, config.url.indexOf("?")) !== "/bn-amm/gem001b/queryAlcAlarmByParam" && whiteListError.indexOf(config.url) === -1) {
                // 过滤掉右上角小铃铛1min轮询接口,和报警信息列表接口的报错信息
                ...
                if (err.response?.status === 404) {
                    ...
                    const message = err.response?.data?.errorInfo?.message || err.response?.data?.msg || err.response?.statusText || "接口报错";
                    if (response.data.msg !== "内部服务器错误" && response.data.msg !== "服务未找到" && /[\u4e00-\u9fa5]/.test(response.data.msg)) {
                        Notification.error({
                            title: "",
                            message: message,
