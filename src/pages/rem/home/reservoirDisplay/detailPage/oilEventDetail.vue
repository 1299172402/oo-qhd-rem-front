<template>
  <div class="app-container" style="height: 100%">
    ...
    <page-panel :header-title=title style="height: calc(100% - 145px)" :show-btn="true">
      <el-form>
        <el-form-item>
          <el-button style="float:right" type="primary" icon="el-icon-download" @click="downtable">下载
          </el-button>
        </el-form-item>
      </el-form>
      <el-table
      ...
      >
        <el-table-column label="序号" width="50px" type="index" :index="Nindex">
        </el-table-column>
        ...
        <el-table-column label="大事类型" min-width="130px" prop="appendixValueName" align="center">
          <template slot-scope="scope">
                        <span
                            v-if="scope.row.appendixValueName !== null && scope.row.appendixValueName !== ''">{{
                            scope.row.appendixValueName
                          }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="大事简要" min-width="130px" prop="chronicle" align="center">
          <template slot-scope="scope">
                        <span
                            v-if="scope.row.chronicle !== null && scope.row.chronicle !== ''">{{
                            scope.row.chronicle
                          }}</span>
            <span v-else>-</span>
          </template>
...
import {
  queryOilFieldIncident,
  queryOilFieldIncidentDownloadFile,
  queryOilFieldIncidentType
} from "@/api/rem/reservoirbillboards";
...
export default {
  ...
  methods: {
    Nindex(index) {
      // 当前页数 - 1 * 每页数据条数 + 1
      const page = this.queryData.page // 当前页码
      const pagesize = this.queryData.pageSize // 每页条数
