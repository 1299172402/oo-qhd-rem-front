<template>
  <el-row class="mt-2" style="height: 100%;">
    <page-panel header-title="生产指标">
      ...
      />
      <echarts ref="echartDown" :chart-data="option" :height="echartsHeight" />
    </page-panel>
  </el-row>
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    // const heightP1 = this.$refs.zmain.$el.clientHeight;
    // this.$nextTick(() => {
  ...
  methods: {
    ...
    getArterialPressureLine(linearChart) {
      const xData = [];
      const yData = [];
      const lineData = linearChart.linearData;
     ...
     */
    doDownLoad() {
      const res = this.$refs.echartDown.chart.getDataURL(
        {
          type: "png",
          pixelRatio: 14,
          backgroundColor: "#022644"
