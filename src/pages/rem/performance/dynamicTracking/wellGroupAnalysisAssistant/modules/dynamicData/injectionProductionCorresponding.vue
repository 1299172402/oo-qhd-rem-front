  ...
  <div class="z-main">
    <div class="z-search" style="display: flex; align-items: center">
      ...
      <el-radio-group v-model="wellValue" @input="changeWell" style="margin-left: 20px">
        <el-radio-button v-for="well in wellList" :key="well.wellId" :label="well.wellId">{{
          well.wellNo
        }}</el-radio-button>
      </el-radio-group>
    </div>
    <div class="z-echarts">
      <div v-for="(chart, index) in wellOptions" :key="index">
        <page-panel
          :headerTitle="chart.wellNo + '对应曲线图'"
          style="height: 600px; margin-top: 0; margin-bottom: 10px"
          show-btn
        >
          ...
          if (resList.length) {
            resList.forEach((resItem, index) => {
              ...
              if ((resItem.charts.length && resItem.wellType == "002002001") || resItem.wellType == "003002001") {
                ...
                resItem.charts.length &&
                (resItem.wellType == "002003001" || resItem.wellType == "003003001")
              ) {
