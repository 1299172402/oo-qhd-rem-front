...
<template>
    <div class="z-main">
        <div class="z-search">
            ...
            <span>时间：</span>
            <el-date-picker v-model="yearTime" type="month" placeholder="选择年月" value-format="yyyy-MM" style="margin-right:20px;"></el-date-picker>
...
<script>
    ...
    import { dynamicDataAccumulatedWaterOilRatioDistribution } from '@/api/oilDeposit/rem-01/fielddynamicanalysis.js';
    ...
    export default {
        ...
        methods: {
            async doSearch() {
                ...
                //获取图片组信息
                await dynamicDataAccumulatedWaterOilRatioDistribution(request).then((res) => {
                    if (res.data.code == 200) {
            ...
            OnChangeImage() {
                ...
                dynamicDataAccumulatedWaterOilRatioDistribution(request).then((res) => {
                    if (res.data.code == 200) {
                        if (res.data.data.layerPics) {
                            if (res.data.data.layerPics.length > 0) {
                                let imageData = res.data.data.layerPics[0];
