...
export default {
  ...
  data() {
    return {
      ...
      option: {
        grid: [
          {x: '5%', y: '20%', width: '80%', height: '65%', containLabel: true},
        ],
        legend: {
          left: 0,
          textStyle: {
    ...
    async doSearch() {
      ...
      await dynamicDataYieldComponentCurve(request).then((res) => {
        if (res.data.code == 0) {
          ...
          if (res.data.data.outputComposition)
            this.tableDataStruct = res.data.data.outputComposition;
          else {
            this.tableDataStruct = [];
          }
          if (res.data.data.measureOutputComposition)
            this.tableDataProduct = res.data.data.measureOutputComposition;
          else
