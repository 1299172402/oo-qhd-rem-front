...
<template>
  <div class="mt-2">
    ...
    <el-row class="mt-2" style="height: 600px;overflow:auto;">
      <!--<img style="height: 700px" :src="src">-->
      <el-image :src="image">
        <div slot="error">
...
import {reservoirDataSandThicknessMap} from "@/api/oilDeposit/rem-01/fielddynamicanalysis.js";
...
export default {
  ...
  data() {
    return {
      radio: 1,
      src: '../../static/img/blockAnalysisAided/reservoirData/sandstone.jpg',
      //选中层位
      selectPosition: '',
      //层位所选择内容信息
      position: [],
  ...
  methods: {
    async doSearch(){
       ...
       * 获取图片组信息
       */
      await reservoirDataSandThicknessMap(request).then((res)=>{
        if(res.data.code==0){
    ...
    OnChangeImage(){
      ...
      reservoirDataSandThicknessMap(request).then((res)=>{
        if(res.data.code==0){
          if(res.data.data.layerPics){
            if(res.data.data.layerPics.length>0){
              let imageData = res.data.data.layerPics[0];
