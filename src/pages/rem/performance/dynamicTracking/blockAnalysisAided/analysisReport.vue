  ...
  <div class="app-container">
    <headerSearch style="height: 80px">
      <div class="g-row-flex-V g-w100 g-h100">
        ...
        <div style="margin-left: 10px">
          <el-button icon="el-icon-search" style="margin-left: 20px; width: 90px" type="primary" @click="doSearch"
            >搜索</el-button
          >
        ...
        <div style="margin-left: 8px; margin-right: 20px">
          <pagePanel headerTitle="主要开发矛盾洞察" style="margin-top: 0; min-height: 500px">
            <el-row :gutter="30" style="text-align: center; height: calc(100% - 55px)">
              <el-col :span="6">
                <el-button class="commonBtn" style="width: 100%; cursor: inherit; margin-bottom: 5px"
                  >开采现状(地层压力)分析</el-button
                >
                <el-row>
                  <el-col v-for="(item, index) in indexChangeTrendList" :key="index" :span="24">
                    <el-button
                      class="z-button"
                      style="height: 34px !important; line-height: 8px"
                      :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                      @click.stop="selRadioIterm(item.code, 'indexChangeTrendList')"
                    >
              ...
              <el-col :span="6">
                ...
                <el-row style="width: 100%">
                  <el-radio-group
                    v-model="indexChangeTrend"
                    @change="
                      (val) => {
                        selRadioIterm(val, 'stabilityFoundationAnalysisList');
                      }
                    "
                  >
                    <el-radio-button
                      v-for="(item, index) in stabilityFoundationAnalysisList"
                      :key="index"
                      :label="item.code"
                      :class="item.value > 0 ? 'checkButton about1' : 'checkButton'"
                      style="width: 100%"
                    >
                      {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                    </el-radio-button>
              ...
              <el-col :span="6">
                ...
                <el-row>
                  <el-radio-group
                    ...
                    @change="
                      (val) => {
                        selRadioIterm(val, 'injectionProductionBalanceAnalysisList');
                      }
                    "
                  >
                    <el-radio-button
                      v-for="(item, index) in injectionProductionBalanceAnalysisList"
                      :key="index"
                      :label="item.code"
                      :class="item.value > 0 ? 'checkButton about1' : 'checkButton'"
                      style="width: 100%"
              ...
              <el-col :span="6" style="height: 100%">
                ...
                <el-row style="height: 60%">
                  <el-radio-group
                    ...
                    @change="
                      (val) => {
                        selRadioIterm(val, 'recoveryAnalysisList');
                      }
                    "
                  >
                    <el-radio-button
                      v-for="(item, index) in recoveryAnalysisList"
                      :key="index"
                      :label="item.code"
                      :class="item.value > 0 ? 'checkButton about1' : 'checkButton'"
                      style="width: 100%"
            ...
            <el-row style="height: 100%">
              ...
              <el-col :span="10" style="height: 100%">
                ...
                <el-col :span="4" style="margin-top: 10px; display: flex">
                  <el-button class="commonBtn" style="margin-left: auto !important" @click="switchToAnaylsis"
                    >区块分析</el-button
                  >
                </el-col>
              </el-col>
                  ...
                  <div class="z_schedule">
                    ...
                    <div class="z_proess">
                      <span class="z_proess_sp1" :style="{ width: indexChangeTrendNum.zczb + '%' }">
                        <b style="cursor: pointer" @click="indexChangeTrendSwitch = true">{{
                          indexChangeTrendNum.zcnum
                        }}</b>
                      </span>
                      <span class="z_proess_sp2"></span>
                    </div>
                    <span class="sp2"
                      >异常井：<b style="cursor: pointer" @click="indexChangeTrendSwitch = false">{{
                        indexChangeTrendNum.ycnum
                      }}</b></span
                    >
                  </div>
                  <el-row :gutter="10">
                    <el-col v-for="(item, index) in indexChangeTrendList" :key="index" :span="24">
                      <el-button
                        ...
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="
                          !item.name.includes('正常') && (item.value != 0 || item.isShow) && !indexChangeTrendSwitch
                        "
                        @click.stop="selRadioIterm(item.code, 'indexChangeTrendList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      ...
                      <el-button
                        class="z-button"
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="item.name.includes('正常') && indexChangeTrendSwitch"
                        @click.stop="selRadioIterm(item.code, 'indexChangeTrendList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                  ...
                  <div class="z_schedule">
                    ...
                    <div class="z_proess">
                      <span class="z_proess_sp1" :style="{ width: stabilityFoundationAnalysisNum.zczb + '%' }">
                        <b style="cursor: pointer" @click="stabilityFoundationAnalysisSwitch = true">{{
                          stabilityFoundationAnalysisNum.zcnum
                        }}</b>
                      </span>
                      <span class="z_proess_sp2"></span>
                    </div>
                    <span class="sp2"
                      >异常井组：<b style="cursor: pointer" @click="stabilityFoundationAnalysisSwitch = false">{{
                        stabilityFoundationAnalysisNum.ycnum
                      }}</b></span
                    >
                  </div>
                  <el-row :gutter="10">
                    <el-col v-for="(item, index) in stabilityFoundationAnalysisList" :key="index" :span="24">
                      <el-button
                        ...
                        v-if="
                          ...
                          !stabilityFoundationAnalysisSwitch
                        "
                        @click.stop="selRadioIterm(item.code, 'stabilityFoundationAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                      <el-button
                        class="z-button"
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="item.name.includes('正常') && stabilityFoundationAnalysisSwitch"
                        @click.stop="selRadioIterm(item.code, 'stabilityFoundationAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                  ...
                  <div class="z_schedule">
                    ...
                    <div class="z_proess">
                      <span class="z_proess_sp1" :style="{ width: injectionProductionBalanceAnalysisNum.zczb + '%' }">
                        <b style="cursor: pointer" @click="injectionProductionBalanceAnalysisSwitch = true">{{
                          injectionProductionBalanceAnalysisNum.zcnum
                        }}</b>
                      </span>
                      <span class="z_proess_sp2"></span>
                    </div>
                    <span class="sp2"
                      >异常井组：<b style="cursor: pointer" @click="injectionProductionBalanceAnalysisSwitch = false">{{
                        injectionProductionBalanceAnalysisNum.ycnum
                      }}</b></span
                    >
                  </div>
                  <el-row :gutter="10">
                    <el-col v-for="(item, index) in injectionProductionBalanceAnalysisList" :key="index" :span="24">
                      <el-button
                        ...
                        v-if="
                          !item.name.includes('正常') &&
                          (item.value != 0 || item.isShow) &&
                          !injectionProductionBalanceAnalysisSwitch
                        "
                        @click.stop="selRadioIterm(item.code, 'injectionProductionBalanceAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                      <el-button
                        class="z-button"
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="item.name.includes('正常') && injectionProductionBalanceAnalysisSwitch"
                        @click.stop="selRadioIterm(item.code, 'injectionProductionBalanceAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                  ...
                  <div class="z_schedule">
                    ...
                    <div class="z_proess">
                      <span class="z_proess_sp1" :style="{ width: recoveryAnalysisNum.zczb + '%' }">
                        <b style="cursor: pointer" @click="recoveryAnalysisSwitch = true">{{
                          recoveryAnalysisNum.zcnum
                        }}</b>
                      </span>
                      <span class="z_proess_sp2"></span>
                    </div>
                    <span class="sp2"
                      >异常井区：<b style="cursor: pointer" @click="recoveryAnalysisSwitch = false">{{
                        recoveryAnalysisNum.ycnum
                      }}</b></span
                    >
                  </div>
                  <el-row :gutter="10">
                    <el-col v-for="(item, index) in recoveryAnalysisList" :key="index" :span="24">
                      <el-button
                        ...
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="
                          !item.name.includes('正常') && (item.value != 0 || item.isShow) && !recoveryAnalysisSwitch
                        "
                        @click.stop="selRadioIterm(item.code, 'recoveryAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
                      <el-button
                        class="z-button"
                        :class="[item.value > 0 ? 'about1' : '', item.code == indexChangeTrend ? 'selectButton' : '']"
                        v-if="item.name.includes('正常') && recoveryAnalysisSwitch"
                        @click.stop="selRadioIterm(item.code, 'recoveryAnalysisList')"
                      >
                        {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                      </el-button>
            ...
            <pagePanelNew headerTitle="" style="height: 100%" showBtn @zoom-out-com="zoomOutCom">
              <div style="height: 100%">
                <div style="display: flex; margin-bottom: 10px">
                  ...
                  <!-- minIo上传 -->
                  <el-button v-if="isUpdateFile" type="primary" icon="el-icon-upload2" @click="ljpmUploadDialogLast"
                    >上传底图</el-button
                  >
                  <!-- 区块分析 -->
                  <el-button class="commonBtn" style="margin-left: auto !important" @click="switchToAnaylsis"
              ...
              <el-col :span="24" style="height: 100%">
                ...
                >
                  <div
                    ...
                    :key="index"
                    :class="[$store.state.setting.mode == 'light' ? 'z-well0' : 'z-well']"
                    @click="getBorepipeTypeApi(item.well)"
                  >
                    <span>{{ item.well }}</span>
    ...
    getStableBaseAnalysis(request) {
      return stableBaseAnalysis(request).then((data) => {
        if (data.data.data != null) {
          ...
          this.stabilityFoundationAnalysisNum.zczb =
            (this.stabilityFoundationAnalysisNum.zcnum / this.stabilityFoundationAnalysisNum.allnum) * 100;
      ...
      return proInjectionBalanceAnalysis(request).then((data) => {
        if (data.data.data != null) {
          ...
          if (
            ...
            this.injectionProductionBalanceAnalysisNum.zczb =
              (this.injectionProductionBalanceAnalysisNum.zcnum / this.injectionProductionBalanceAnalysisNum.allnum) *
              100;
          }
          this.injectionProductionBalanceAnalysisNum.yczb =
            (this.injectionProductionBalanceAnalysisNum.yczb / this.injectionProductionBalanceAnalysisNum.allnum) * 100;
        ...
        for (let j = 0; j < this.stabilityFoundationAnalysisList.length; j++) {
          ...
          if (val == t_data.code) {
            ...
            if (t_data.basis == null) {
              ...
              myData[i]["stabilityFoundationAnalysisMessage"] = messData
                ? messData.message
                  ? messData.message
                  : ""
                : "";
              ...
              if (t_data.basis == null) {
                ...
                myData[i]["stabilityFoundationAnalysisMessage"] = messData
                  ? messData.message
                    ...
                    : ""
                  : "";
                myData[i]["stabilityFoundationAnalysis"] = messData
                  ? messData.itemValue
                    ? messData.itemValue
                    : ""
                  : "";
        ...
        for (let j = 0; j < this.injectionProductionBalanceAnalysisList.length; j++) {
          ...
          if (val == t_data.code) {
            ...
            if (t_data.basis == null) {
              ...
              myData[i]["injectionProductionBalanceAnalysisMessage"] = messData
                ? messData.message
                  ? messData.message
                  : ""
                : "";
              myData[i]["injectionProductionBalanceAnalysis"] = messData
                ? messData.itemValue
                  ? messData.itemValue
                  : ""
                : "";
        ...
        if (res.data.code == 200) {
          ...
          if (data == 1) {
            ...
            this.$router.push({
              ...
              path: `/dynamicManagement/dynamicTrackingWaterAuxiliary/waterAuxiliaryAnalysis?wellNo=${wellNo}&activeName=productionDynamicData&currentModule=productionData`,
            });
          } else if (data == 2) {
            //油井
            this.$router.push({
              // path: `/dynamicManagement/dynamicTrackingOilAuxiliary/oilAnalysisReport?wellNo=${wellNo}`,
              path: `/dynamicManagement/dynamicTrackingOilAuxiliary/oilAuxiliaryAnalysis?wellNo=${wellNo}&activeName=productionDynamicData&currentModule=productionData`,
            });
          } else if (data == 3) {
            //井组
            this.$router.push({
