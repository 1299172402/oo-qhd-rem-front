        ...
        <headerSearch style="height: 80px">
            <div class="g-row-flex-V g-w100 g-h100">
                ...
                <el-upload ref="upload" style="margin-left: 20px" class="upload-demo" action=""
                    ...
                    :show-file-list="false" :on-success="handleSuccess" v-show="
                        canUpload &&
                        !ljpmTag &&
                ...
                <el-button style="margin-left: 20px" type="primary" icon="el-icon-download" @click="doDownLoad">下载
                </el-button>
            </div>
        </headerSearch>
        <!-- :style="{ height: calc(100% - 100px)}" -->
        <pagePanelNew headerTitle="油井辅助分析" show-btn
            :style="{ height: this.currentModule == 'oilReport' ? 'auto' : 'calc(100% - 100px)' }" class="g-w100">
                ...
                <el-tab-pane v-for="(item, index) in tabs" :key="index" :label="item.label" :name="item.name">
                    <div class="tab-view">
                        ...
                        <div class="select-view" v-if="currentModule == 'homeworkWellHistory'">
                            ...
                            <el-select v-model="majorEventsBrieflyValue" placeholder="请选择" filterable clearable
                                @change="majorEventsBrieflyChange">
                                <el-option v-for="(item, index) in majorEventsBrieflyList" :key="index"
                                    :label="item.chronicle" :value="item.chronicle"></el-option>
                            </el-select>
                        </div>
                    </div>
                </el-tab-pane>
            ...
            <keep-alive :include="[]" :max="10">
                <component :is="component" ref="componentCustom" :oil-feild-id="selectOilField"
                    :platform="selectPlatform" :well-id="selectWellId"
                    :majorEventsBrieflyValue="majorEventsBrieflyValue" @childPara="changeChildParam"></component>
            </keep-alive>
            <el-dialog custom-class="border" title="连井剖面图上传" :visible.sync="ljpmDialog" width="50%"
                ...
                <el-form ref="form" :model="ljUploadForm" label-width="80px">
                    <el-form-item label="图片上传">
                        <el-upload ref="ljpmUpload" class="upload-demo" action="" :on-preview="handlePreview"
                            :before-remove="beforeRemove" :on-change="ljpmChange" :on-exceed="handleExceed"
                            :file-list="ljpmFileList" :http-request="httpRequest" :auto-upload="false">
                    ...
                    <el-form-item label="井号">
                        <el-transfer filterable :titles="['未选中', '已选中']" :filter-method="filterMethod"
                            filter-placeholder="请输入" :props="props" v-model="ljUploadForm.chooseWell"
                            :data="ljpmWellData">
                        </el-transfer>
                    </el-form-item>
                </el-form>
