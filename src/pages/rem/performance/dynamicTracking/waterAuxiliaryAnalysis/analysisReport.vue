      ...
      <div class="app-container" v-if="!isNewformat">
        ...
        <pagePanelNew style="height: calc(100% - 100px)" class="g-w100">
          ...
          >
                      ...
                      <el-col :span="12">
                        ...
                        <el-col v-for="(item, index) in workingCondition" :key="`index2-${index}`" :span="24">
                          <el-button
                            class="z-button"
                            :class="[item.value > 0 ? 'about1' : '', item.code == selCode ? 'selectButton' : '']"
                            @click.stop="selRadioIterm(item.code, 'workingCondition')"
                          >
                            {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                          </el-button>
                        </el-col>
                      ...
                      <el-col :span="6">
                        ...
                        <el-col v-for="(item, index) in wellboreReason" :key="`index4-${index}`" :span="24">
                          <el-button
                            class="z-button"
                            :class="[item.value > 0 ? 'about1' : '', item.code == selCode ? 'selectButton' : '']"
                            @click.stop="selRadioIterm(item.code, 'wellboreReason')"
                          >
                            {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                          </el-button>
                        </el-col>
                      ...
                      <el-col :span="6">
                        ...
                        <el-col v-for="(item, index) in formationReason" :key="`index5-${index}`" :span="24">
                          <el-button
                            class="z-button"
                            :class="[item.value > 0 ? 'about1' : '', item.code == selCode ? 'selectButton' : '']"
                            @click.stop="selRadioIterm(item.code, 'formationReason')"
                          >
                            {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                          </el-button>
                        </el-col>
                      ...
                      <el-col :span="6">
                        ...
                        <el-col v-for="(item, index) in stopInjectionRecovery" :key="`index6-${index}`" :span="24">
                          <el-button
                            class="z-button"
                            :class="[item.value > 0 ? 'about1' : '', item.code == selCode ? 'selectButton' : '']"
                            @click.stop="selRadioIterm(item.code, 'stopInjectionRecovery')"
                          >
                            {{ item.name + (item.value > 0 ? "(" + item.value + ")" : "(0)") }}
                          </el-button>
                        </el-col>
            ...
            <div style="flex: 1; min-height: 680px">
              <pagePanel headerTitle="水井动态分析详情列表" style="margin-top: 0; height: 100%">
                <div style="display: flex; justify-content: flex-end">
                  <el-button
                    ...
                    @click="doDownExcel('#table1', '水井动态分析详情列表')"
                    >下载</el-button
                  >
                </div>
                <el-table
                ...
                >
                  ...
                  >
                      ...
                      <template slot-scope="{ row }">
                        <span style="display: flex; align-items: center; justify-content: center">
                          ...
                          <img
                            ...
                            v-if="row[item.code + 'Message'] && row[item.code + 'Message'].includes('上升')"
                            style="width: 20px; height: 20px"
                          />
                          <img
                            src="@/assets/rem/yieId/downTriangle.png"
                            v-if="row[item.code + 'Message'] && row[item.code + 'Message'].includes('下降')"
                            style="width: 20px; height: 20px"
                          />
                        </span>
                      </template>
                    ...
                    <el-table-column
                      v-for="(item, index) in productionProblemsTab"
                      :key="`index11-${index}`"
                      :prop="item.code"
                      :label="item.name"
                      align="center"
                  ...
                  <el-table-column prop="overUnderInjectionAnalysis" label="超欠注原因分析" align="center">
                    <el-table-column
                      v-for="(item, index) in overUnderInjectionAnalysisTab"
                      :key="`index12-${index}`"
                      :prop="item.code"
                      :label="item.name"
                      align="center"
      ...
      <div class="app-container2" v-if="isNewformat">
        ...
        <div class="app-container3">
          <div class="leftBox">
            ...
            -->
            <img src="@/assets/rem/performance/shuijing.gif" alt="" class="speed" />
            <img src="@/assets/rem/performance/bg.png" alt="" class="bg" v-if="$store.state.setting.mode == 'dark'" />
            <img src="@/assets/rem/performance/bg2.png" alt="" class="bg" v-else />
          </div>
          <div class="rightBox">
                  ...
                  <div class="z-content-n">
                    <div class="z-row-left">
                      ...
                      <div class="z_schedule">
                        ...
                        <span class="sp2"
                          >异常井：<b @click="trendOfIndicatorsSwitch = false">{{
                  ...
                  <div class="z-content-n">
                    <div class="z-row-left">
                      ...
                      <div class="z_schedule">
                        ...
                        <span class="sp2"
                          >异常井：<b @click="wellboreReasonSwitch = true">{{ wellboreReasonNum.ycnum }}</b></span
                        >
                      </div>
                    </div>
                    <div class="z-row-center">
                      <div
                        class="numBtn"
                        :class="[item.code == selCode ? 'numBtnBgActive' : '']"
                        v-for="(item, index) in wellboreReason"
                        :key="`index16-${index}`"
                        v-if="item.name != '正常' && (item.value != 0 || item.isShow) && !wellboreReasonSwitch"
                        @click="
                          (val) => {
                            selRadioIterm(item.code, 'wellboreReason');
                          }
                        "
                      >
                        <span class="sp1">{{ item.value }}</span>
                  ...
                  <div class="z-content-n">
                    <div class="z-row-left">
                      ...
                      <div class="z_schedule">
                        ...
                        <span class="sp2"
                          >异常井：<b @click="workingCondSwitch = false">{{ workingCondNum.ycnum }}</b></span
                        >
                      </div>
                    </div>
                    <div class="z-row-center">
                      <div
                        class="numBtn"
                        :class="[item.code == selCode ? 'numBtnBgActive' : '']"
                        v-for="(item, index) in workingCondition"
                        :key="`index19-${index}`"
                        v-if="
                          item.name != '正常' &&
                          item.name != '合格区' &&
                        ...
                        @click="
                          (val) => {
                            selRadioIterm(item.code, 'workingCondition');
                          }
                        "
                      >
                        <span class="sp1">{{ item.value }}</span>
                    ...
                    <div class="z-row-right" style="position: relative; top: 48px">
                      ...
                      <div class="num">
                        <span
                          ...
                          v-for="(item, index) in recommendedMeasuresOptions"
                          :key="`index21-${index}`"
                          v-if="item.name == '储层改造' || item.name == '酸化解堵' || item.name == '恢复注水'"
                          @click="selRadioIterm(item.code, 'recommendedMeasuresOptions')"
                        >
                          {{ item.name }}：
                          <b style="color: #ffc835; font-size: 14px">{{ item.value }}</b>
                  ...
                  <div class="z-content-n">
                    <div class="z-row-left">
                      ...
                      <div class="z_schedule">
                        ...
                        <span class="sp2"
                          >异常井：<b @click="zsqdSwitch = false">{{ zsqdNum.ycnum }}</b></span
                        >
                      </div>
                    </div>
                    <div class="z-row-center">
              ...
              <pagePanel header-title="水井动态分析详情列表" style="height: 100%">
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <template slot-scope="{ row }">
                        <span style="display: flex; align-items: center; justify-content: center">
                          ...
                          <img
                            src="@/assets/rem/yieId/upTriangle.png"
                            v-if="row[item.code] && (item.name.includes('偏高') || item.name.includes('变高'))"
                            style="width: 20px; height: 20px"
                          />
                          <img
                            src="@/assets/rem/yieId/downTriangle.png"
                            v-if="row[item.code] && (item.name.includes('偏低') || item.name.includes('变低'))"
                            style="width: 20px; height: 20px"
                          />
                        </span>
                      </template>
                    ...
                    <!-- <el-table-column :key="`index39-1`" prop="zsqdForm" label="注水强度" align="center">
                      <el-table-column :key="`index39-1-1`" prop="zsqdFormValue" label="数值" align="center">
                        <template slot-scope="{ row }">
                          <span>{{ row["zsqdFormValue"] ? row[item.code] : "-" }}</span>
                        </template>
                      </el-table-column>
                      <el-table-column :key="`index39-1-2`" prop="zsqdFormName" label="评价结论" align="center">
                        <template slot-scope="{ row }">
                          <span>{{ row["zsqdFormName"] ? row['zsqdFormName'] : "-" }}</span>
                        </template>
                      </el-table-column>
                    </el-table-column> -->
                  </el-table-column>
    ...
    getFieldsData() {
      QueryReservoirAnalyseUnit({ ogfId: this.selYtdm }).then((res) => {
        if (res.data.code == 200) {
          ...
          //默认选中第一个区块信息
          this.selectBlock = this.blocks[0].reservoirAnalyseUnitId;
      ...
      for (let i = 0; i < myData.length; i++) {
        ...
        for (let j = 0; j < this.zsqdForm.length; j++) {
          ...
          if (t_data.basis == null) {
            ...
            myData[i][t_data.code + "Message"] = messData && messData.message ? messData.message : "";
            myData[i][t_data.code] = messData && messData.itemValue ? messData.itemValue : "";
            // myData[i]["zsqdFormValue"] = messData && messData.name ? messData.name : "";
            // myData[i]["zsqdFormName"] = messData && messData.itemValue ? messData.itemValue : "";
            myData[i].id = Math.random() * 3;
          }
          //选中项目不需要测试
          if (val == t_data.code) {
