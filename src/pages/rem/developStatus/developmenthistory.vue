        ...
        <headerSearch style="height:80px;">
            <div class="g-row-flex-V g-w100 g-h100">
                ...
                <el-select v-model="oilfield" disabled @change="onFieldChange" style="margin-right:20px">
                    <el-option v-for="item in oiloptions" :key="item.oilFieldId" :label="item.name" :value="item.oilFieldId"></el-option>
                </el-select>
                <span>区块：</span>
        ...
        methods: {
            ...
            async initData() {
                ...
                await fetchOilFields().then((res) => {
                    if (res.data.code == 200) {
                        this.oiloptions = res.data.data.oilFields;
                        this.oiloptions1 = res.data.data.oilFields;
                        if (this.oiloptions.length == 0) {
                            this.oilfield = '';
                            this.oilfield1 = '';
                        } else {
                            this.oilfield = this.oiloptions[0].oilFieldId;
                            this.oilfield1 = this.oiloptions1[0].oilFieldId;
                        }
                    }
                });
                //区块
                ...
                await fetchFields(requestField).then((res) => {
                    if (res.data.code == 200) {
                        this.blockoptions = res.data.data.fields;
                        this.block = requestField.oilFieldId;
                    }
                });
                this.doSearch();
            },
            ...
            getFetchFields1(oilFieldId) {
                ...
                fetchFields(request).then((res) => {
                    if (res.data.code == 200) {
                        this.blockoptions1 = res.data.data.fields;
