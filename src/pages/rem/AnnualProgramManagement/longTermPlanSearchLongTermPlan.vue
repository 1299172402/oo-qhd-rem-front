...
<template>
  <el-container class="layout">
    <el-header style="margin: 0">
      <header-search style="height: 100%" height="auto">
        <div class="py-5" style="margin-top: 8px">
          <span>油田：</span>
    ...
    <el-main style="margin-top: -20px">
      <div style="height: 100%">
        <page-panel-new  @zoom-out-com="zoomOutComNew" class="tablebox">
          <span class="tableTitle">{{ oilFieldName }} 中长期计划表</span>
          <div style="margin: 30px 0"></div>
          <el-table :data="tableDataLS" id="tableData" height="calc(100% - 110px)" highlight style="width: 100%">
  ...
  methods: {
    ...
    getSearchLongTermPlan(oilFieldId, page, pageSize) {
      ...
      searchLongTermPlan(request).then((res) => {
        ...
        _this.tableData = [];
        /*//根据数组获得油田名称
              let oilFieldContent = this.oilField.find(
                  (item) => item.oilFieldId == this.selectOilField
              );
              //修改油田名称
              this.oilFieldName = oilFieldContent.name;*/
        //判断是否获得成功
