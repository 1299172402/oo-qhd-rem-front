...
<template>
  <div class="app-container">
      ...
      <div style="padding-top:12px;display: flex;align-items: center;">
        ...
        <div class="fl mg">
          ...
          <el-select class="f2" filterable>
            <el-option v-for="(item, index) in []" :key="index" :label="item.name" :value="item.code"></el-option>
          </el-select>
        </div>
        <div class="fl mg">
          <span>措施类型：</span>
    ...
    <pagePanelNew headerTitle="措施管理" style="height: calc(100% - 100px)">
      ...
      <div class="tableBox">
        <el-table id="csgl" :data="tableData1.slice((queryParams.page - 1) * queryParams.pageSize, queryParams.page * queryParams.pageSize)" height="calc(100% - 114px)"
          ...
          <el-table-column prop="measureName3" label="措施作业天数(计划/实际)" width="110">
            <template slot-scope="scope">
              <span v-if="scope.row.measureName3_1||scope.row.measureName3_2">{{scope.row.measureName3_1}}/{{scope.row.measureName3_2}}</span>
            </template>
          </el-table-column>
...
<script>
  ...
  export default {
    name:'planIndex',
    components:{fieldOperations},
    data() {
      return {
        mcWidth:'',
        days:['2022-01-01','2022-02-01','2022-03-01','2022-04-01','2022-05-01','2022-06-01','2022-07-01','2022-08-01','2022-09-01','2022-10-01','2022-11-01','2022-12-01',],
        ...
        if(this.tableData1.length){
          this.tableData1.forEach((el,i)=>{
            if(el.type!='date'){
              ...
              this.tableData1[i].mgleftwidth=(diffObject.month*78)+(diffObject.month*20)+(78/30*diffObject.day) -4 +'px';
            
              let sjwidth=(78/30*el.measureName3_1)+(this.diffMonth(el.sj_stardata,el.sj_enddata) * 17.5);
              let jhwidth=(78/30*el.measureName3_2)+(this.diffMonth(el.jh_stardata,el.jh_enddata) * 17.5);
