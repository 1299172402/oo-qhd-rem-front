      ...
      <div style="display: flex; align-items: center; flex-wrap: wrap">
        ...
        <div style="margin-right: 15px; margin-bottom: 10px">
          ...
          <el-select v-model="searchForm.planTypeCode" placeholder="请选择" style="width: 150px">
            <el-option
              v-for="(item, index) in planTypeSelect"
              :key="index"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <div style="margin-right: 15px; margin-bottom: 10px" v-if="pageType == '原油产量'">
          ...
          <el-select v-model="searchForm.theYieldComponentsValue" placeholder="请选择" style="width: 150px">
            <el-option
              v-for="(item, index) in theYieldComponentsSelect"
              :key="index"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <div style="margin-right: 15px; margin-bottom: 10px">
          ...
          <el-select v-model="searchForm.rollForecastVersion" placeholder="请选择" style="width: 150px">
            <el-option
              v-for="(item, index) in rollingForecastSelect"
              :key="index"
              :label="item.rollForecastName"
              :value="item.rollForecastVersion"
            ></el-option>
          </el-select>
        </div>
        <div style="margin-right: 15px; margin-bottom: 10px">
      ...
      <pagePanelNew class="pagePanelNew" id="pagePanelNew">
        <el-tabs class="g-pageHeader" v-model="pageType" topline v-if="isLoadChildCommon">
          <el-tab-pane label="原油产量" name="原油产量">
            <crudeOil
              v-if="pageType == '原油产量' && searchForm.theYieldComponentsValue == 1"
            ...
            ></basicYield>
            <measureProduction
              v-if="pageType == '原油产量' && searchForm.theYieldComponentsValue == 3"
              :searchForm="searchForm"
              ref="childComponent"
            ></measureProduction>
            <adjustingWellProduction
              v-if="pageType == '原油产量' && searchForm.theYieldComponentsValue == 4"
              :searchForm="searchForm"
              ref="childComponent"
            ></adjustingWellProduction>
            <devWellProduction
              v-if="pageType == '原油产量' && searchForm.theYieldComponentsValue == 5"
              :searchForm="searchForm"
              ref="childComponent"
            ></devWellProduction>
          </el-tab-pane>
  ...
  data() {
    return {
      ...
      searchForm: {
        ...
        selectDate: ["2024-01-01", "2024-12-31"], //日期
