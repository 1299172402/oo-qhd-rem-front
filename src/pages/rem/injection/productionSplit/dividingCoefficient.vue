<template>
  <div class="app-container">
    <headerSearch class="g-w100 g-h100">
      <div class="g-row-flex-V g-w100 g-h100" style="margin-top: 10px; margin-bottom: 10px">
        <div class="fl">
          <span>油田：</span>
        ...
        <span>井别：</span>
        <el-select v-model="params.wellCategory" class="f2" disabled>
    ...
    <pagePanel headerTitle="单井劈分系数结果" style="height: 100%">
    ...
    <div v-if="params.wellCategory === '01'">
      <el-dialog title="修改" :visible.sync="redactBut" width="900px" append-to-body>
        <el-form ref="form" label-width="100px" :model="form">
          <el-card class="box-card" v-for="(interval, index) in form.table" :key="interval.intervalId">
              ...
              <el-row style="width: 100%">
                ...
                <el-col :span="10">
                  <el-form-item label="备注">
                    <el-input style="width: 220px" v-model="interval.remark" placeholder="请输入备注" />
                  </el-form-item>
                </el-col>
              </el-row>
            </div>
          ...
          <el-card class="box-card" v-for="(interval, index) in form.table" :key="interval.intervalId">
            <div slot="header" class="clearfix">
              <el-row style="width: 100%">
                <el-col :span="2">{{ interval.intervNo }}</el-col>
                <el-col :span="10">
                  <el-form-item label="劈分系数" :prop="'table.' + index + '.oilRatio'" :rules="rules.oilRatio">
                    <el-input
                      style="width: 220px"
                      type="number"
...
export default {
  ...
  data() {
    ...
    return {
        ...
        ratio: [
          {
            ...
            required: true,
            pattern:
              /^((^[0-9]{1,2})\.([0-9]{1,2})$)|^((^[0-9]{0,2})$)|^(([0-9]{1,2})\.([0-9])([1-9])$)|^(([0-9]{1,2})$)/,
            message: "输入不正确",
          },
          {
            validator: ratio,
        ...
        layerData: [
          {
            ...
            required: true,
            pattern:
              /^((^[0-9]{1,2})\.([0-9]{1,2})$)|^((^[1-9]{1,2})$)|^(([0-9]{1,2})\.([0-9])([1-9])$)|^(([1-9]{1,2})$)/,
            message: "输入不正确",
          },
          {
            validator: ratio,
      ...
      tData: [
        {
          airPermeability: 2368,
          intervId: "708219D3CA0F4860800EBBEDECD7FEFE",
          intervInjRatio: null,
          intervNo: "P2",
          intervSplit: null,
          layerData: [
            {
              ...
              layerNo: "明化镇组下段Ⅰ油组1小层",
              oilRatio: null,
              oilSplitData: null,
              openStatus: false,
              ratio: 0,
              remark: null,
              stateDate: "2022-08-03",
              thicknessEffe: 18.5,
              waterRatio: null,
              waterSplitData: null,
  ...
  methods: {
    ...
    changeCellStyle(row, column, rowIndex, columnIndex) {
      //列的label的名称
      if (row.column.label === "劈分系数" || row.column.label === "备注") {
    ...
    // 保存
    submitSave(data) {
      return postdividingCoefficientSave(data)
        .then(() => {
          this.OilfieldBut();
    ...
    // 通过井号下拉获取表格数据
    OilfieldBut() {
      console.log(this.params.wellCategory);
