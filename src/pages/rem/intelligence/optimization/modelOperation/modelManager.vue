<template>
    <div style="height: 100%">
        <pagePanelNew style="height: 100%">
            <!-- 模型管理  油藏优化模型 -->
            <el-row style="width:100%; margin-top: 0px" type="flex">
                ...
                <span style="font-size: 14px;color: white;margin-top: 7px;margin-left: 95px">模型创建时间 : </span>
                ...
                <el-button type="primary" @click="returnMainScreen"
                           style="">返回主界面</el-button>
            </el-row>
            <pagePanel headerTitle="油藏优化模型" :show-btn="true" style="height: 100% ;margin-top: 15px;height: 700px">
                <!-- 表格-->
                <el-table  highlight style="margin-top: -10px;margin-left: 1px;" height="calc(100% - 10px)"
...
export default {
    ...
    methods:{
        // 模型管理页面返回主界面
        returnMainScreen(){
            this.$router.push({ name: 'ModelMain' })
        },
        //分页序号连续
        table_index(index){
        ...
        lookResClick(id) {
            ...
            //跳转页面
            this.$router.push({ name: 'ModelMain', params: { modelParams: id } })
        },
        handleTime() {
            ...
            this.$refs.modelForm.validate((valid) => {
                if (valid) {
                    ...
                    GddModelBasic(modelBasicVo).then(res => {
                        ...
                        //------------------------------- 有问题
                        this.$router.push({ name: 'ModelMain', params: { modelParams: res.result.modelBasicId } })
                    }).catch(error => {
                        console.log(error)
                    });
                } else {
