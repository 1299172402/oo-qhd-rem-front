  ...
  <div style="height:100%;">
    <div class="titleBox">
      <el-tabs v-model="activeName" class="g-pageHeader" @tab-click="setWidth">
        <el-tab-pane label="定产配注" name="first" />
        <el-tab-pane label="智能配注" name="second">
          <!-- <a href="http://sea-oil-web-qhd32-6znyt.tjdevapp.cnooc/"></a> -->
        </el-tab-pane>
      </el-tabs>
      <el-button
        v-if="activeName =='second' && $route.query.page"
        style="position: absolute;z-index:50;right: 20px;top:110px"
        type="primary"
        @click="gogo"
      >
        返回
    ...
    <header-search v-if="activeName=='first'" style="height: auto;display: grid">
      <div v-if="activeName == 'first'" style="margin-top:20px;margin-bottom:20px;">
        ...
        <el-button class="commonBtn" icon="el-icon-refresh" @click="refresh">
          重置
        </el-button>
        <el-button
          v-if="$route.query.page"
          type="primary"
          style="position: absolute;right:2%;top:110px"
          @click="$router.push({
        ...
        <el-button type="primary" @click="upModelForRedis">
          模型更新
        </el-button>
        <span class="fangan">
    ...
    <page-panel-new v-if="activeName == 'first'" style="height: auto">
      <el-row :gutter="20" style="height:800px">
        <el-col :span="6" style="height:100%">
          <page-panel
            header-title="单井月度配产计划表"
            :show-btn="true"
        ...
        <el-col :span="18" style="height:100%">
          <page-panel :header-title="title2" style="height:calc(100% - 10px);" :show-btn="true">
            <div class="buttonBox">
              <el-row :gutter="20">
                <el-col :span="22">
                  <el-button class="button" icon="el-icon-search" @click="assessBut">
                    可行性评估
                  </el-button>
                  <!-- @click="modify = true" -->
                  <el-button type="primary" icon="el-icon-edit" @click="modify = true">
                    修改
                  </el-button>
                  <el-button
                    type="primary"
            ...
            >
              ...
              >
                ...
                <el-table-column
                  ...
                  align="center"
                />
                <el-table-column prop="injection" label="注采比" align="center" />
                <el-table-column
                  prop="layerNo"
                  min-width="200"
                  label="层段号"
                ...
                >
                  <template slot-scope="scope">
                    <span
                      v-if="scope.row.layerNo !== null && scope.row.layerNo !== ''"
                    >{{ scope.row.layerNo }}</span>
                    <span v-else>-</span>
                  </template>
                </el-table-column>
                <el-table-column
    ...
    <model-operation v-else ref="modelOpreation" style="height: 100%" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getSpanArr(data) {
      this.mergeArr.forEach((key, index1) => {
        ...
        data.forEach((item, index) => {
          ...
          } else if (item[key] === data[index - 1][key]) {
    ...
    // 单井月度配注计划表
    queryTableData(arr, ifrefsh) {
      const params = {
        blockId: this.queryData.blockId,
        dateTime: this.queryData.dateTime,
        ifRefsh: this.ifrefsh
      };
