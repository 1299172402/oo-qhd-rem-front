 ...
 * @returns {any[]}
 */
export function getScatterDataSet({ dateTime = [], placeholderColor, borderColor }) {
  const divideNum = 40;
  const timeArray = [];
  const inArray = [];
  const outArray = [];
  for (let i = 0; i < divideNum; i++) {
    ...
    if (dateTime.length > 0) {
      ...
      const endTime = new Date(dateTime[0]).getTime() + dateAbsTime * i;
...
/** 折线图数据 */
export function getFolderLineDataSet({ dateTime = [], placeholderColor, borderColor }) {
