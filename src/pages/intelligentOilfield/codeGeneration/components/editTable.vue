      ...
      <el-tab-pane label="字段信息" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          ...
          <el-table-column label="Java类型" min-width="11%">
            <template slot-scope="scope">
              <el-select v-model="scope.row.javaType" clearable>
          ...
          <el-table-column label="显示类型" min-width="12%">
            <template slot-scope="scope">
              <el-select v-model="scope.row.htmlType" clearable>
  ...
  created() {
    ...
    if (tableId) {
      ...
      getDictOptionselect().then(response => {
        this.dictOptions = response.data.data;
      });
      /** 查询菜单下拉列表 */
      getMenuTreeselect().then(response => {
        this.menus = this.handleTree(response.data.data, "menuId");
      });
    }
  },
  mounted() {
    ...
    Sortable.create(el, {
      ...
      onEnd: evt => {
        ...
        this.columns.forEach(index => {
          this.columns[index].sort = parseInt(index,10) + 1;
        });
  ...
  methods: {
    ...
    submitForm() {
      ...
      Promise.all([basicForm, genForm].map(this.getFormPromise)).then(res => {
        ...
        if (validateResult) {
          const genTable = { ...basicForm.model, ...genForm.model};
