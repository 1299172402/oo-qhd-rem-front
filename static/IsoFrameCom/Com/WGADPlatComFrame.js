...
define(['WGAD/Scripts/WGADPlatConfig/WGADPlatConfig',
    ...
    Com.WGADPlatComFrame = function (viewSetting,contentWindow)
    {
                            ...
                            case WGADPlatGraph2D.EWGObjectType.wgtFrameExploitation:
                                {
                                    ...
                                    exp.OffX = itemObject.OffX;
                                    exp.OffY = itemObject.OffY;
        ...
        this.ClearView = function () {
            if (this.View.MapData.GetActivateLayerList() != null && this.View.MapData.GetActivateLayerList().length > 0) {
                //处理所有的layer
                console.log(this.View.MapData.GetActivateLayerList());
                this.View.MapData.GetActivateLayerList().forEach(item=>{
                    item.RemoveAllChildrens();
                })
            }
            //原方法仅处理了单个layer 在存在填充颜色时 会造成颜色叠加 增加上面一个if 处理所有layer
            if (this.View.MapData.GetActivateLayer() != null) {
